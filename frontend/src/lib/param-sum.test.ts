...
describe("sumParamField", () => {
  ...
  it("subprocess 노드는 subprocess_refs의 sp값 사용", () => {
    const g = graph(
      ...
      { 7: { designated: true, department: null, assignee: null, system: null, duration: "0.30", url: null, url_label: null, headcount: null, cost_krw: null, cost_usd: null } },
    );
    expect(sumParamField(g, "duration")).toBe("1.30");
  });
  it("십진수는 스케일 정수 합산 — 부동소수 오차 없음", () => {
    const g = graph([node("a", { cost_krw: "0.1" }), node("b", { cost_krw: "0.2" })]);
  ...
  it("기여값 0개면 빈 문자열", () => {
    expect(sumParamField(graph([node("a")]), "cost_usd")).toBe("");
  });
