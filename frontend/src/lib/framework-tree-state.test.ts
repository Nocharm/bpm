...
describe("fetchRootChildren", () => {
  it("루트 로드 - parentId 없이 listCategoryNodes 호출, children_loaded로 리듀스", async () => {
    const rootNodes: CategoryNode[] = [
      { id: 1, code: "A", name: "Root A", level: 1, sort_order: 0, child_count: 2, map_count: 5, linkage_map_id: null, can_edit_linkage: false, l5_count: 0, canvas_state: null, admin: null, slot_pending_count: 0 },
    ];
...
describe("펼침 캐시", () => {
  it("펼침 시 자식+맵을 1회만 fetch - 접었다 재펼침해도 캐시라 재요청 없음", async () => {
    const children: CategoryNode[] = [
      { id: 11, code: "A1", name: "Sub A1", level: 2, sort_order: 0, child_count: 0, map_count: 2, linkage_map_id: null, can_edit_linkage: false, l5_count: 0, canvas_state: null, admin: null, slot_pending_count: 0 },
    ];
  ...
  it("펼침 중(fetch 미완료) 접었다 재펼침해도 fetch는 1회만 발생한다 - loadingIds 가드 회귀", async () => {
    const children: CategoryNode[] = [
      { id: 21, code: "B1", name: "Sub B1", level: 2, sort_order: 0, child_count: 0, map_count: 1, linkage_map_id: null, can_edit_linkage: false, l5_count: 0, canvas_state: null, admin: null, slot_pending_count: 0 },
    ];
...
describe("hidden·빈 카테고리 표시", () => {
  it("hidden>0과 빈 카테고리(map_count=0·child_count=0) 노드를 상태에 그대로 보존한다", async () => {
    const emptyNode: CategoryNode = {
      ...
      map_count: 0,
      linkage_map_id: null,
      can_edit_linkage: false, l5_count: 0, canvas_state: null, admin: null, slot_pending_count: 0,
    };
...
describe("collectCascadeTargets", () => {
  const makeNode = (id: number, mapCount: number): CategoryNode => ({
    id, code: `C${id}`, name: `Cat ${id}`, level: 2, sort_order: id, child_count: 0, map_count: mapCount, linkage_map_id: null, can_edit_linkage: false, l5_count: 0, canvas_state: null, admin: null, slot_pending_count: 0,
  });
