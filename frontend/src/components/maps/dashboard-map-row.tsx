// 대시보드 컴팩트 맵 행 — 섹션 안의 평면 행(구분선). 제목 톤다운·SP 아이콘(맵 카드와 동일)·상태는 점 인디케이터만·우측 메타,
// hover 시 열기 버튼이 지연 후 폭을 벌리며 등장(우측 항목이 밀림, 맵 카드 열기 필과 같은 디자인). 클릭은 선택.
"use client";
...
export function DashboardMapRow({ map, meta, onSelect }: DashboardMapRowProps) {
  ...
  return (
    ...
    >
      <span className="min-w-0 flex-1 truncate text-caption text-ink-secondary">{map.name}</span>
