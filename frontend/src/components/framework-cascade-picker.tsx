...
export function FrameworkCascadePicker({
  selectedId, onSelect, selectableLevels = [5], maxLevel = 5, height = 320, dataIdPrefix,
      ...
      <li key={node.id} data-tree-node data-id={`${dataIdPrefix}-node-${node.id}`} className="flex flex-col">
        ...
        >
          ...
          <button
            ...
            onClick={() => (canPick ? onSelect(isSelected ? null : node) : canExpand && handleToggle(node.id))}
          >
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-md border border-hairline bg-surface" style={{ height }} data-id={`${dataIdPrefix}`}>
      <label className="m-2 flex min-w-0 items-center gap-2 rounded-sm border border-hairline bg-surface px-2.5 py-1.5 text-caption text-ink">
