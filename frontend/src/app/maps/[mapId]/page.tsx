...
const EXTENT_MARGIN = 600; // 패닝/노드 사방 대칭 여백 — 콘텐츠 bbox + 이 여백(무한 캔버스처럼 자유 패닝)
const MIN_ZOOM = 0.2; // 최소 줌 — translateExtent 패닝 여백(pane/MIN_ZOOM) 계산이 이 값과 반드시 일치해야 줌아웃 centering이 방지됨
...
function MapEditor({ mapId }: { mapId: number }) {
  ...
  >(() => {
    ...
    // 노드 드래그 범위: 콘텐츠 bbox + 사방 대칭 여백.
    const node: [[number, number], [number, number]] = [
      [minX - EXTENT_MARGIN, minY - EXTENT_MARGIN],
      [maxX + EXTENT_MARGIN, maxY + EXTENT_MARGIN],
    ];
    // 패닝 범위: 콘텐츠 사방으로 최소 (minZoom 뷰포트 크기)만큼 대칭 여백 — ① 줌아웃해도 뷰포트가
    // extent를 못 넘어 d3-zoom centering이 안 일어나고(콘텐츠가 가운데로 튀지 않음), ② fitView 가운데
    // 정렬 후 살짝 팬해도 여백 안이라 스냅 없음(무한 캔버스처럼 자유 패닝).
    const padX = Math.max(EXTENT_MARGIN, paneWidth / MIN_ZOOM);
    const padY = Math.max(EXTENT_MARGIN, paneHeight / MIN_ZOOM);
