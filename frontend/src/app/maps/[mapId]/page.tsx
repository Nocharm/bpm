...
const EXTENT_MARGIN = 600; // 우/하단 패닝 여백 — 콘텐츠 성장 여유 (콘텐츠 bbox + 이 여백)
const EXTENT_TOPLEFT_MARGIN = 120; // 좌/상단 패닝 여백 — 작게(좌상단 고정 느낌, 위/왼쪽 무한 패닝 방지)
const MIN_ZOOM = 0.2; // 최소 줌 — translateExtent 우하단 확장 계산이 이 값과 반드시 일치해야 좌상단 고정이 유지됨
...
function MapEditor({ mapId }: { mapId: number }) {
  ...
  // 노드 위치(nodeExtent)·패닝(translateExtent) 허용 범위 = 콘텐츠 bbox + 여백. 좌상단은 콘텐츠에 바짝 고정.
  ...
  >(() => {
    ...
    // 패닝 범위: translateExtent가 줌아웃 뷰포트보다 작으면 d3-zoom이 콘텐츠를 가운데 정렬해
    // 좌상단 고정이 풀린다(좌상단 여백이 자람). minZoom에서의 뷰포트 크기(pane/MIN_ZOOM) 이상으로
    // 우하단을 늘려 centering을 원천 차단 → 좌상단은 모든 줌에서 topLeft에 고정, 여백은 우하단에만.
