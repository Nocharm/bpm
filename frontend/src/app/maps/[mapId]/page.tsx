...
const GROUP_NEST_STEP = 16; // 중첩 한 단계당 상위 박스에 더해지는 패딩 — 하위 박스를 감싸 보이게
...
function buildGraph(nodes: AppNode[], edges: Edge[], groups: GraphGroup[]): Graph {
  ...
  // 노드가 직접 속한 그룹 + 그 상위 그룹(중첩)까지 보존 — 직접 멤버 없는 상위 그룹이 사라지지 않게
  const leafGroupIds = new Set<string>();
  for (const node of nodes) {
    if (node.data.groupId) {
      leafGroupIds.add(node.data.groupId);
    }
  }
  const keepSet = collectKeptGroups(groups, leafGroupIds);
  const keptGroups = groups.filter((group) => keepSet.has(group.id));
  ...
  return {
    ...
    groups: keptGroups.map((group) => ({
      id: group.id,
      // 상위 그룹이 보존된 그룹 집합에 있을 때만 유지(고아 참조 제거)
      parent_group_id:
        group.parent_group_id && groupIds.has(group.parent_group_id)
...
function MapEditor({ mapId }: { mapId: number }) {
  ...
  const createGroupFromSelection = useCallback(() => {
    ...
    // 선택 노드가 모두 같은 기존 그룹 소속이면 그 그룹의 하위 그룹으로 중첩(부분집합 → 하위 그룹핑)
    const commonGroup = first.data.groupId;
    const nestParent =
      commonGroup && selected.every((node) => node.data.groupId === commonGroup)
        ? commonGroup
        : null;
    setGroups((cur) => [
      ...cur,
  ...
  // 그룹 해제(disband) — 직접 멤버와 하위 그룹을 해제 그룹의 상위로 끌어올린 뒤 그룹 제거(중첩 일관 유지).
  // leaveGroup(선택 멤버만 이탈)과 구분.
  const disbandGroup = useCallback(
    (groupId: string) => {
      ...
      pushHistory();
      const parent = groupsRef.current.find((group) => group.id === groupId)?.parent_group_id ?? null;
      setNodes((current) =>
        current.map((node) =>
          node.data.groupId === groupId ? { ...node, data: { ...node.data, groupId: parent } } : node,
        ),
      );
      setGroups((current) =>
        current
          ...
          .map((group) =>
            group.parent_group_id === groupId ? { ...group, parent_group_id: parent } : group,
          ),
      );
      scheduleAutoSave();
    },
  ...
  // 그룹 멤버(서브트리: 자기+하위 그룹 전원) 노드 색 일괄 변경
  const applyGroupColor = useCallback(
    (groupId: string, color: string) => {
      pushHistory();
      const subtree = groupSubtreeIds(groupsRef.current, groupId);
      setNodes((current) =>
        current.map((node) =>
          node.data.groupId && subtree.has(node.data.groupId)
  ...
  // 그룹 박스 — 서브트리(자기+하위 그룹) 멤버 bbox로 산정. 중첩 높이만큼 패딩을 더해 상위 박스가 하위 박스를 감싸게.
  const groupBoxes = useMemo(() => {
    const heights = computeGroupHeights(groups);
    return groups.flatMap((group) => {
      const subtree = groupSubtreeIds(groups, group.id);
      const members = nodes.filter(
        (node) => node.data.groupId !== null && subtree.has(node.data.groupId),
      );
      if (members.length === 0) {
        return [];
      }
      ...
      // 중첩 깊을수록(상위 그룹) 패딩↑ → 하위 박스를 시각적으로 포함
      const pad = GROUP_PAD + (heights.get(group.id) ?? 0) * GROUP_NEST_STEP;
      return [
        {
          id: group.id,
          label: group.label,
  ...
  // 일괄 편집 대상 그룹의 서브트리(자기+하위 그룹) — 멤버 산정용
  const bulkSubtree = useMemo(
    () => (bulkEditGroupId ? groupSubtreeIds(groups, bulkEditGroupId) : new Set<string>()),
    [bulkEditGroupId, groups],
  );
        ...
        >
          ...
          {bulkEditGroupId && (
            <GroupBulkModal
              ...
              members={nodes
                .filter((n) => n.data.groupId !== null && bulkSubtree.has(n.data.groupId))
                .map((n) => ({
                  id: n.id,
                  label: n.data.label,
