...
> ⑪(dev, 2026-09-07) **L5 퍼블리시 거버넌스(confirmed 계약·게이트 6종·레벨 위임) · 슬롯 거버넌스(`fw_slot` 승인·캔버스 자동 재지정·임포트 플레이스홀더, `docs/qa/2026-09-fw-slot-governance-qa.md`) · 라이브러리 필터(부서 트리 플라이아웃·역할 필·미등록 스위치)**. ⑫(dev, 2026-09-19) **홈 업무 체계 뷰 = L5 포커스 드릴다운**(`components/maps/framework-drill.tsx`, 브레드크럼 + 형제(1):하위(2) 두 열, 소속 맵은 우측 요약, `/categories/nodes`에 `l5_count`·`canvas_state`·`admin`·`slot_pending_count`) + **탐색 모달**(`framework-explorer-modal.tsx`, 계단식 트리 ↔ ERD식 다이어그램 `lib/framework-diagram.ts`, 우클릭 메뉴). ⑬(dev, 2026-09-23) AI L5 2라운드 — 관리 패널 타일 진입·AiButton·플랜 카드 FLIP·주관식 타이핑·연결 캔버스(세션 canvas↔relations)·피드백 엔드포인트·IO 배열·질문 섹션·러너 병렬·비교 diff 토큰/인스펙터 폭·L5 드롭존
