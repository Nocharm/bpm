...
> ⑪(dev, 2026-09-07) **L5 퍼블리시 거버넌스(confirmed 계약·게이트 6종·레벨 위임) · 슬롯 거버넌스(`fw_slot` 승인·캔버스 자동 재지정·임포트 플레이스홀더, `docs/qa/2026-09-fw-slot-governance-qa.md`) · 라이브러리 필터(부서 트리 플라이아웃·역할 필·미등록 스위치)**. ⑫(feat/fw-l5-list, 2026-09-18) **홈 업무 체계 뷰 = L5 포커스 드릴다운**(`components/maps/framework-drill.tsx`, 브레드크럼+형제 칩, 소속 맵은 우측 요약, `/categories/nodes`에 `l5_count`·`canvas_state`·`admin`·`slot_pending_count`).
