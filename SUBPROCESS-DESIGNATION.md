...
| F8 | 좌측 사이드바 노드 검색 — 플레이스홀더 축약("노드 검색 (/)") + 단축키 Ctrl+K → `/` (레전드·매뉴얼 문서 동기 갱신) | 검토 대기 | `d8f3cae` |
| F9 | `.env MANUAL_URL` + 에디터 툴바 행 우측 끝 "매뉴얼 편집 사이트 열기" 버튼(BookOpen, 새 탭 — 미설정 시 숨김) | 검토 대기 | |
| F10 | 매뉴얼 다중 문서(공지사항식) — 제목 자동 추출·한/영 선택 저장·뷰어 제목 드롭다운 목록(맵 제목 버튼식)·언어 전환 시 동일 순번 문서 유지 | 검토 대기 | `61202ea` |
| F11 | 매뉴얼 UI 폴리시 — 목록 삭제 버튼 호버 노출·타이틀 드롭다운 테두리(맵식)+긴 제목 말줄임·검색창 본문 최상단 가운데 이동 | 검토 대기 | `f1a26d3` |
| F12 | 아웃라인 노드 검색 — 플레이스홀더의 "(/)" 제거, 우측 `/` 키캡 버튼(공용 SearchBox와 동일: 클릭=포커스, 입력 중=X 지우기) | 검토 대기 | |

### F9·F10 시현
- F9: `.env`(backend/.env 또는 루트)에 `MANUAL_URL=<주소>` 설정 후 백엔드 재기동 → 에디터 편집 모드 툴바(정렬 행) 우측 끝 BookOpen 버튼 → 새 탭.
- F10 관리: admin.sys → 설정 → 콘텐츠 → 매뉴얼 — 문서 목록(언어 뱃지·자동 제목·삭제)·새 문서(포맷+언어 세그먼트)·게시. 제목은 본문 첫 헤딩 자동 추출.
- F10 뷰어: /manual — 제목이 드롭다운(현재 한/영에 맞는 목록), 상단 한/EN 토글 시 같은 순번 문서 유지(예: ko 2번째 "서브프로세스 활용" → EN "Using Subprocesses"). 문서 0건이면 번들 매뉴얼 fallback.
