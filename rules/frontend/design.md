...
- **예외: L5 다크 셸** — L5 연계 캔버스가 차콜 배경일 때 에디터 크롬을 `.bpm-l5-dark`(globals.css) 토큰 플립으로 동조. OS 다크모드가 아닌 L5 한정 무대 연출이며, body 포털(모달·드롭다운·토스트)은 라이트 유지. 크롬에 새 색을 쓸 땐 토큰만 쓰면 자동 적용 — 컴포넌트별 다크 분기 금지.
