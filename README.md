...
- 노드(단계)·엣지(선후 흐름)로 프로세스를 시각 편집 — React Flow 캔버스. 이름·라벨은 여러 줄 입력(`Shift`/`Alt+Enter`),
  연결선 **선 모양(곡선/꺾은선/직선)은 엣지별로 저장**(맵 단위 일괄 변경 + 확인 모달)
...
- 버전 생성(복제)·라벨링·나란히 비교
- **버전 승인 워크플로** — Draft→제출(Pending)→승인(만장일치)→게시(Published), 반려·회수(기록 유지), 게시 시 순차 버전번호(v1,v2…)·직전 게시본 만료(Expired)·만료본 재게시. 제출·승인·게시·철회 **단계별 코멘트**(반려 사유 알림 동봉·코멘트 이력 모달), 제출 시 **공개 범위 변경 동봉**(오너 전용)
- **점유권(checkout)** — 드래프트 편집은 보유자 1인만, 지정 인계 전용(직접 이전 / 요청→승인). 요청자 복수·승인 시 나머지 자동거절·요청 철회, 출처(누구에게서·언제) 표시
- **권한(RBAC)** — 맵별 owner/editor/viewer(유저·부서·그룹 grant), 공개/비공개 가시성, 지정 승인자, 유저 그룹 라이프사이클. 강등·가시성 전환은 **승인 요청 라이프사이클**(대기 표시·철회·직접 적용 시 종결), 협업자 변경은 **스택 저장**(Save 일괄+직전 1회 Undo), 결재 대기 통합 목록·인박스 대기 배지
- **직원 디렉터리·조직 정보** — n8n HR 웹훅 자동 동기화(퇴직자 자동 제외, AD는 직급 보강 전용), 조직 기준 departments(EDW 직책 부서장), 인물 카드(한/영 이름·직급·보직·메신저 링크)·조직 정보 모달
- **업무체계(Framework) 뷰**(시범) — 전사 프로세스 분류 트리로 맵 탐색, 관리자 카테고리 관리·웹 대량 임포트
...
| `backend/` | FastAPI + Python **3.11**(배포 이미지 기준) | 맵/버전/노드/엣지·승인 워크플로·권한 API |
| `nginx/` | nginx | 리버스 프록시 — `/` → frontend, `/api` → backend |
| `docker-compose.yml` | + PostgreSQL 16 | 서버 배포 (포트 **9900**) |
...
> 서버의 엣지 nginx(443/80)와 충돌하지 않도록 앱은 9900에 노출한다. 줄바꿈은 `.gitattributes`로 LF 고정 — Windows PC를 경유해도 안전. 자세한 운영 제약은 `CLAUDE.md`의 Operations 섹션 참고.
