...
## 사내 로컬(Windows) — Postgres 전환 + 서버 덤프 복원

sqlite 대신 **로컬 Postgres**에 서버 실데이터를 복원해 테스트하는 절차. 사내 로컬(Windows)은 Docker를 쓸 수 없어
**Postgres를 네이티브(설치 프로그램)로 설치**해 둔 상태를 전제로 한다. DB 선택은 전적으로 `.env`의 `DATABASE_URL`로 갈린다.

> ⚠️ 복원한 서버 데이터가 든 DB에는 **`reset_db`를 절대 실행하지 말 것**(`drop_all`로 전부 삭제됨). 시드는 빈 DB에서만.

### 1) (참고) 서버에서 덤프 — 사내서버 환경만, 이미 완료

서버 DB는 compose 내부 `db:5432`라 외부에서 직접 못 닿는다. 서버(사내 `182.199.63.71`)에 SSH로 들어가 컨테이너에서
`pg_dump`로 떠서 로컬로 내려받는다(이미 수행 완료 — 재현용 기록).

```bash
# === 서버 (linux) — 계정 h_jin.jang, 서버 182.199.63.71 ===
ssh h_jin.jang@182.199.63.71
cd <compose 디렉터리>                                   # docker-compose.yml 위치
# custom 포맷(권장, pg_restore로 복원):
docker compose exec -T db pg_dump -U processmap -Fc processmap > bpm.dump
# 또는 plain SQL:
#   docker compose exec -T db pg_dump -U processmap processmap > bpm.sql
exit
scp h_jin.jang@182.199.63.71:~/bpm.dump .              # 로컬로 다운로드
```

### 2) 로컬 Postgres에 복원

네이티브 Postgres의 슈퍼유저(`postgres`)로 덤프 소유자(`processmap`)와 DB를 만든 뒤 복원한다.
`psql`/`pg_restore`는 셸 무관 동일 — 아래는 PowerShell 기준(bash도 동일 명령).

```powershell
# === PowerShell (Windows) ===
# 덤프 소유자 role + DB 생성 (최초 1회) — 비밀번호는 아래 .env와 일치시킬 것
psql -U postgres -c "CREATE ROLE processmap LOGIN PASSWORD 'change-me';"
psql -U postgres -c "CREATE DATABASE processmap OWNER processmap;"

# 복원 — custom 포맷(.dump)이면 pg_restore:
pg_restore -U postgres -d processmap --no-owner --clean --if-exists bpm.dump
# plain SQL(.sql)이면:  psql -U postgres -d processmap -f bpm.sql
```
...
`backend/.env`(git 미추적)에 `DATABASE_URL`을 Postgres로 지정한다. user/password/db는 2)에서 만든 값과 일치시킨다.
...
- 기동 시 `init_models`가 **누락 테이블·컬럼을 멱등 보강**한다: 새 테이블(`checkout_requests`)은 `create_all`이,
  기존 `map_versions`에 없는 **`version_number` 컬럼은 `_add_missing_columns`가 자동 추가**(nullable, 기존 행 생존).
  덤프가 라이프사이클 이전 스키마여도 publish/workflow가 500 나지 않는다.
- 확인: `psql -U postgres -d processmap -c "\d map_versions"` 에 `version_number`가 보이면 정상.
- 기존 published 행은 `version_number`가 NULL이며, 다음 게시 때 채번된다(과거 번호 소급 채번은 하지 않음).

> **mac/Homebrew 대체 경로**: mac에서 검증할 땐 Docker 대신 `brew install postgresql@16` → `brew services start postgresql@16`
> 로 네이티브 Postgres를 띄우고 위 2)~4)를 동일하게 적용한다(`DATABASE_URL` 그대로).
