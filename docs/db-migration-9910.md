...
전례: 1차 업데이트의 `docs/db-migration-9800.md`(운영 9900 → 검증 9800)와 같은 메커니즘 —
서버 실측값(컨테이너명·compose 병합 함정)은 그 문서에서 확인된 값을 그대로 쓴다.
브라우저 검증 시나리오: `DEV-SERVER-TEST-PLAN.md`.
...
- [ ] **Keycloak 리다이렉트 허용** — realm `ai-portal` 클라이언트에 9910 origin이 없으면 로그인 불가:
  ...
  (이미 와일드카드거나 1차 때 포트별로 넣었다면 9910만 추가. 화면 위치는 `docs/deploy.md` §1.)
- [ ] **서브넷 분리** — 사내 71번 서버 compose는 오버라이드 병합이 누적이라(9800 문서 §2 실측)
...
**전체 시나리오는 `DEV-SERVER-TEST-PLAN.md`** (§1 스모크 → §3 A~F → §4 서버 전용 함정).
...
- §4-1 스키마 확인 → `DEV-SERVER-TEST-PLAN.md` §1 스모크 재확인.
