...
- [ ] **R2-13 에디터 승인 탭 결재 섹션**: 에디터 > Map > 승인 탭 접이식 "결재 대기" 섹션(기본 접힘, R6에서 최상단으로 이동) — pending 수 배지, 펼치면 설정 화면과 동일한 4종 목록·행별 결정 버튼, 결정 시 워크플로 상태 갱신.
...
- [ ] **R6-4 승인 탭 순서 + 드래프트 CTA 두 상태**: 에디터 우측 Approval 탭 → 순서가 ① 결재 대기 섹션(최상단, 기본 접힘) ② **드래프트 CTA 행** ③ 워크플로(기본 펼침) ④ SP ⑤ 타임라인 순 → **드래프트 有일 때** CTA는 "승인을 위해 드래프트로 전환"(`PencilLine` 아이콘), **無일 때** "승인을 위해 드래프트 생성"(`Plus` 아이콘) → **현재가 draft 상태**이면 CTA 미노출. (O/E)
