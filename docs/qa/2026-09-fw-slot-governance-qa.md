...
| 16 | 배정 모달 | 대기 배너 + 요청자 철회 버튼, 대기 중 버튼 비활성 | 수동 | ✅ 배너 "… is waiting for approval" + 진행 필 "0/1 sides" + "requested by" 요청자 필 + Withdraw(트랙 F 시인성 개선 후 문구·구조), 연결/해제 버튼 disabled, 철회 후 pending null(`a2-pending-banner`, `a3-after-withdraw`, 개선 후 `verify-f/f1-3-pending-banner`); move는 "1/2 sides"(`b2-move-banner-1of2`). 2026-09-07: 철회권 = 요청자·현재 오너·sysadmin(`can_withdraw`), 요청자가 아니면 "submitted by another user" 필 — pytest `test_current_owner_and_sysadmin_can_withdraw_after_ownership_transfer` |
