...
- [ ] 맵 선택 → 상세 카드에 카테고리 경로 pill(**최상단 전용 행** — 긴 경로여도 이름이 한 줄 유지) + Input/Output 표시. 미연결 맵은 유령 필이 기존 우측 위치
- [ ] (오너 계정) 경로 pill 클릭 → 연결/이양 모달 — **조직도식 트리**로 표시, 현재 지정 경로는 미리 펼쳐지고 **가장 하위(리프) 카테고리만 선택 가능**(상위 클릭=펼침/접힘만), 선택 행은 accent 강조+체크, **미선택이면 연결 버튼 비활성**
