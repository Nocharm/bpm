...
- [ ] (두 뷰 공통 + 나의 부서) 맵 4개+ 목록은 **3.5개 높이로 잘리고**(잘린 영역은 마우스 올린 채 휠로 내부 스크롤) 아래 풀폭 쉐브론 "Show all (n)" → 전체 펼침/"Collapse" 재접힘, 새로고침에도 상태 유지
- [ ] 부서/카테고리 펼침 시 영역이 **자라나는 애니메이션**(즉시 뿅 아님), 틴트 박스 헤더는 **스크롤해도 상단 고정(sticky)** — 전체 펼침 상태면 헤더 우측에 "Collapse" 버튼
