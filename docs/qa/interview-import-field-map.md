...
| `kind: loop` | 엣지. **Start 배선 판정에서 제외**(End 판정엔 포함) | 엣지 |
...
| 산출 | 규칙 |
|---|---|
| 노드 좌표 · 엣지 변(side) | 노드·엣지를 다 만든 뒤 **가로 자동정렬**(rank + 교차 감소 + 주 흐름 직선화 + 역행 엣지 top 핸들). 랭크 간격은 **그 구간을 지나는 엣지 라벨 폭**에서 나온다(라벨이 노드를 덮지 않게). L5 연계 캔버스는 **새로 만들 때만** 적용(보강은 기존 노드 불변) |
