...
- **Four steps.** (1) **Plan**: given an optional brief and attachments, AI proposes a list of L6 task cards to register under that L5. Edit, add, or remove cards, then press **Lock**; the list is fixed and questionnaire prep starts in the background for each card. (2) **Questionnaire**: answer an AI-generated, mostly multiple-choice questionnaire for each L6 card. Every question must be answered before you can submit (a blank free-text answer is filled with the suggested value automatically), and submitting immediately draws that card's flow in the background while the next questionnaire is prepared. A submitted card cannot be reopened. (3) **Connections**: once every L6 is drawn, AI proposes the flow between L6s (start point and connectors). The large preview on the left is the L5 linkage canvas; in the right panel you can **Preview** each L6 card, **Edit answers and redraw** (reopens the questionnaire with the previous answers; resubmitting redraws only that card), and edit the entry point and edge table, then press **Confirm connections**. (4) **Register**: the assembled document goes through the same screen as interview import. Entering this step **runs the Dry run automatically** so the report appears right away, and **Back to connections** returns you to the previous step at any time before Apply.
...
- **"Suggestion applied" badge.** On the answer review screen, this badge on a free-text question means it was submitted blank and the AI's suggested value was used as is. Free-text answers default to the suggestion; press **Write my own** to open the input.
