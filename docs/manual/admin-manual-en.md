...
The entry card above the interview import card (sysadmin only) fills an L5 through a conversation with AI instead of a document. The card has two modes. **Existing L5**: pick a level-5 category in the cascading tree (same UI as the hierarchy explorer, with search) and press **Fill L5 with AI**. **New L5**: pick the L4 that will be the parent, type the new L5 name, and press **Create L5 and start**; the category is created and the campaign session opens right away. The category management tree above uses the same cascading UI now (level pills, guide lines, search to jump to a row).
