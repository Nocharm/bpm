...
- **Node edit dialog**: below the title and description the fields sit as **tiles** (the same design as the subprocess designation dialog, 512 px wide): type and color tiles, **attributes** (department and assignee as row tiles spanning both columns, where the department shows its leaf as a pill that opens the org info modal and each assignee is a person pill whose hover or click opens the person card with name, id, leaf department, and org path; plus system, link, GMP), the seven **per-run metrics** (cost is a single tile; pick the unit with the ₩/$ tabs at the front of the popover, the chosen unit shows as a pill on the tile, and the other currency is cleared on save), and **I/O & conditions** (input and output show only their item count and are edited item by item in the popover - data forms are per item too - plus the start/end conditions). Clicking a tile opens an **input popover right where you clicked**: `Enter` applies (`Ctrl`/`⌘+Enter` in multi-line editors), `Esc` cancels, and clicking outside applies only when something changed. The popover's bottom button is a lone **Cancel** while nothing changed; once something did it turns into **Save** with a ▾ menu (Save / Save and close / Close without saving). The department list puts **your own department chain first** with a **My Dept** pill on your own row (the same pill as the owning-department picker on the new-map dialog), and changing the department shows a notice that the assignees will be cleared. Values applied in popovers and title/description typing are **written to the node right away** and show up in the inspector on the right at once; edits made in the inspector likewise flow into the open dialog's tiles and popovers - the bottom of the dialog is just **Done** (`Esc`), and `Ctrl+Z` undoes. Hovering the system tile turns the tile white with an emphasized border and cross-fades its icon into the **interview note** icon (with a one-time accent flash) so you can read, edit, and apply the note; a tile that has a note shows a **small dot** on the icon corner even before you hover (the dot fades while hovered so the swapped icon stands out). The system popover carries a note field too (the subprocess designation dialog's duration, touch time, system, and annual volume tiles have the same hover note icon and dot). The **+** button in the input/output popover footer opens a two-item menu, **Add new** / **Import from node…** (the same import window as the inspector card; locked while the popover has unapplied changes). The **previous / next connected nodes** float outside the dialog as small cards on the left and right of the canvas (color dot and label on the first line, the type on the second); hovering one widens it toward the dialog and makes it a little taller (a real size change, not a scale transform, so text stays crisp) and the cards above and below spread apart. Clicking one plays a swap: the clicked card grows into the center while the center card shrinks toward the opposite dock, then the dialog switches to that node. The inspector's BPM attribute rows now use the same icon + label form as the other cards (the system row's icon swaps to the interview note icon on hover), and on a subprocess node the annual volume / FTE rows put the reference icon after the label and the inputs flush right at equal width. Read-only (viewer, locked, published) shows only filled tiles statically; only the input/output tiles stay clickable to view their lists. A subprocess node's dialog shows the attributes, four per-run metrics, and I/O & conditions inherited from the linked map as read-only tiles and edits only **annual volume and FTE** (the popover notes the designated reference). While typing a name, `Enter` commits and **`Shift+Enter` (or `Alt+Enter`) inserts a line break**. The same rule holds in the canvas, the inspector, and the node edit dialog, and the break shows on the canvas too.
- **Color**: preset swatches or a custom hex color (`#RRGGBB`).
- **BPM attributes**: **Assignee** (picked from the org directory), **Role**, **Department** (auto-set from the assignee), **System**, and the **per-run metrics** (see section 3).
- **Role**: independent of the assignee's real name, one **role name** that performs the step ("Purchasing officer", "QA reviewer"). Enter it in the inspector's Role row, in the tile next to the assignee in the node edit dialog and the subprocess designation dialog (assignee 2 : role 1 ratio; when filled, only the chip shows without a label), or in the Role mode of group bulk edit; the admin-managed **role list** appears as autocomplete. Typing one of its aliases converts to the canonical spelling, and a value not in the list is saved as typed. Canvas nodes normally show the **role** and switch to the **assignee name chip** (monochrome) about a second after hover or selection; the role also appears in the library, the preview peek, the home map detail, and Excel. Unlike the assignee, a role is not checked against the org directory, so it never raises an orphan warning.
- **System and managed lists**: the System field is also checked against the admin-managed **system list**: typing an alias (e.g. `sap`) saves the canonical spelling (`SAP ERP`), and a value not in the list is classified as **Other** while the typed text is kept as a **source note** (system note). If a different note already exists you choose **Replace / Append / Cancel**. On the canvas the system line shows the source note squeezed to one line when the value is Other; the full text is in the node edit dialog.
- **Orphaned department/assignee warning**: on a node whose department or assignee is no longer in the org chart (active-employee directory), the department/assignee icons on the attribute line turn the **warning color** and a badge at the node's bottom right shows the count. Hovering the badge breaks down the reason: **Not listed** (missing from the directory), **No view access** (a person who cannot view this map), **Department changed** (differs from the assignee's current department). The inspector's department/assignee pills, the node edit dialog, and the home map detail show the same verdict, and admins clean up stale references in bulk at Settings → Directory → **Orphaned refs** (the Role field is not checked).
...
- **I/O & Conditions**: **Input** and **Output** hold multiple items (one per row). Each row is laid out as **number / data-form pill / item name (+ required/optional flag)** columns, and long names are truncated with an ellipsis (the same in the inspector and the tile popover). Click the dashed placeholder in the form column to set that item's **data form**: an autocomplete list matches by extension, English program name, or Korean (`.xlsx` / `Excel` / `엑셀` all find the same entry), navigated with ↑/↓ and picked with Enter or Space. A value outside the list is added via the **Add "…"** row at the bottom. A set form shows as a small pill (catalog entries carry their icon); click the pill to change it, ✕ to remove. **Start / End conditions** are free text. On the canvas, the I/O & Conditions boxes appear below a node **while it is selected** (what shows is picked in the Map tab's Node display).
...
A **Subprocess** node embeds another process as a single step. It is a reference, not a copy.

- Subprocess nodes are created from the **process library** (library button in the top bar, or `S`): drag a map onto the canvas, or press **Add to map** in its preview peek. The top-bar **map-name dropdown** also offers **Add as link node** for an existing map. New links **follow the latest published version by default**; you can pin a specific version instead. When a newer published version appears, the node offers **Update to latest**.
...
- By default only maps **designated as subprocesses** appear in the library picker. The map's owner designates it in **Map Settings → Subprocess designation** with representative attributes (department required; assignee, **Role**, system, link, the seven per-run metrics, and input/output optional; click a tile to edit it in a popover right there; assignee and role share one 2:1 row; see section 5 of the Getting Around manual), and these show live on every node linking the map. There is no separate description; the **map description** is used as is.
...
- **Preview peek**: **click a map row in the library (or hover it for 2.5 s)** to open a **preview peek of its published version**. The header shows the map name, owner, and published version on the left and the **framework path** on the right (click it for the framework tree flyout and "Search other frameworks"). The flow preview on the left (same type colors as the canvas) **zooms with the wheel** (100–300%, anchored at the cursor) and pans by dragging, with **Add to map** (accent) and **Open this map** buttons floating over it. The right side has two tabs, **Node / Details**. The **Node tab** is a live mock-up of the node you'd add (key attributes, metric chips, GMP, conditions, and the I/O checklist; hovering switches it to the current map's display settings), and you can **drag the mock-up straight onto the canvas**. The **Details tab** lists the designated attributes (including the **Role** next to the assignee), subprocess metrics, conditions / I/O / GMP, the published version (v-number), publish time, and the framework path (empty values are toned-down dashes). Library rows show the designated role as well. Maps you can't view show a lock notice but can still be added. Hovering a **department chip** in the list opens an org-info card next to the cursor.
...
- **Expand in place (inline)**: right-click a subprocess node and choose **Expand subprocess**, or press the same button on the action bar under the selected node (while the node is designated and unlocked), to embed the child map's published version **read-only in a tinted region** on the current canvas. The region's header shows the map name with an open icon and, when the linked map belongs to the framework, the **full five-level classification path label** (click it to open the classification peek). Hovering the region highlights it gently, and right-clicking inside offers **Open linked map** (with a confirmation gate) and **Collapse**. With nested expansions the menu targets the **innermost map**, its first line shows that map's name, and clicking the name in a region header collapses it and then opens the menu. `Tab`/`Shift+Tab` traversal includes the expanded child nodes in flow order and crosses region boundaries (focus never leaves the canvas while a node is selected), and child GMP pills render read-only.
- **Double-click:** double-clicking a subprocess node opens the **node edit dialog**, as on any other node. The stacked deep-view overlay is gone, and the child map's content is viewed only through the inline expansion above (expanded children are **read-only**).
...
- **Framework pill**: when the linked map is registered in the business **Framework**, a folder-shaped pill sits next to the node's name. Click it (or hover 3 s) to open the classification-path peek, and its **"Search other frameworks"** footer leads to the full framework browse modal. The peek forgives brief mouse departures, closing only after a 0.4 s grace period.
- If you lack permission on the linked map, the node shows **No access**.
- **Inspector Subprocess tab**: opening a map that is designated as a subprocess adds a **Subprocess tab** to the inspector: **Designation** (version, designated at, by) → **Linked from** (reverse references) → **Designation values** (the designation dialog's tiles stacked in **one column** for the narrow inspector: department (leaf pill, click for the org info modal), assignee, and role, system, link, per-run metrics, input/output item counts; the tiles are static, and the input/output tiles open a view-only list). Unset values stay as grey **Not set** tiles so you can see what is missing, and a field that only has an interview note shows the note as a provisional value (dashed border, italic, tagged **from interview note**). Parameters that carry an interview note (duration, touch time, system, annual volume) show a small dot on the icon corner, swap their icon for a note icon on hover, and open the original wording on click. With the published version open, the owner or an admin gets an **Edit** button in the section header that opens the designation dialog. With any other version open (a draft, say) a **Not the published version** watermark sits over the values and a **Go to published map** button (accent outline) takes the Edit button's place, since designation values belong to the published version. The map's own **Subprocess designation card** also carries a **Linked from** accordion (reverse references, the maps linking this one), so a designated map shows where it is used right on the card.
...
- In the **process library panel** (library button in the top bar), the **Filter** button opens a popover with an **"Unregistered maps"** switch on top (click anywhere on the row), **Department** rows (the check in front of the name appears when you hover the row), and **Role** pills (Owner / Editor / Viewer; the icon turns into a check when selected; no selection means all roles). Turning on "Unregistered maps" makes unregistered maps appear with a **"Not registered"** badge. Active filters show as removable pills next to the Filter button, **"Clear filters"** resets them all, and the choice **persists per browser**.
- **Department filter**: the org tree runs to hundreds of departments, so the popover lists only **your own department chain** (root → your department, indented, with a **"My dept"** mark on your own row) plus any department you already picked. **"All departments…"** opens a flyout beside the popover with the **whole org tree**: expand and collapse branches with the chevron or by double-clicking a row (a single click toggles the check; a branch with a single child keeps drilling in on its own), **search** by department name (English or Korean; rows show the name in your UI language), right-click a row for **Select / Expand / Department info**, and press **"Find my department"** to clear the search, open your ancestors one level at a time and scroll to your department. Picking a department also covers **every department under it**.
...
- **Approver landing and Compare with published**: if a pending version is waiting for your decision, opening the map lands on it (an explicit `?version=` wins); on any other version the top banner shows an **Awaiting your approval: {version}** link and the approval-workflow section is covered by an **Awaiting your approval / Open that version** overlay (the decision queue and designation card stay usable). On a pending version, the **Compare with published** button between the approval workflow and the change summary opens the compare screen for latest published ↔ this version (Getting Around manual, sections 3–4).
...
- The CSV uses **21 columns**: `name` (required), `description`, `assignee`, `role`, `department`, `system`, `duration`, `touch_time`, `cost_krw`, `cost_usd`, `headcount`, `annual_count`, `fte`, `input`, `input_flags` (per-item required/optional flags), `output`, `start_condition`, `end_condition`, `url`, `url_label`, `next` (the successor to connect to). A column outside this list, such as the `data_form` column in older files, is rejected with an "Unknown column" error, so remove it before uploading (data forms are per item and not part of the CSV). Older files with fewer columns still import (columns match by name, case-insensitively). Data rows are capped at **500**. Put multiple `input`/`output` items on separate lines inside the cell.
...
- **Layout**: the left side is the **working-map preview** that draws itself as the interview progresses; the right side is the chat panel (drag the divider to resize). Click a node in the preview to mention it in chat (**Ask about this**). Preview nodes show the **role, department, system, and per-run metric chips** collected in the interview plus I/O and start/end conditions, values only (option-card thumbnails show the same), and `Tab` / `Shift+Tab` moves focus to the next/previous node along the flow with the camera following (not while typing in the chat box).
- **Interview**: the AI asks stage by stage, and items with candidates (activities, branches) come as **option buttons**. The **Roles & Systems** stage fills each activity's **role** (candidates from the admin-managed role list plus free input), department (from the candidate list), and system, and **never asks for assignees' real names** (pick those in the editor's assignee picker). Items you can't answer can be skipped with **Mark unanswered items as TBD and move on**. The conversation is stored on the server, so you can come back and continue.
- **Attach documents (fast track)**: attach SOPs or process documents with the clip button (several files or a whole folder at once); the AI reads them and can draw the draft right away instead of interviewing (confirm with **Draw now**). Attached documents can be removed from the list in the chat panel.
...
- **Apply**: when the interview finishes, run **Review and apply collected parameters** and **merge into the draft**. The working map merges into the draft version by title, so **check out the draft in the editor first**. **Start over** discards the conversation, collected facts, attachments, and the working map (the draft version itself is untouched).
...
- **Checkout**: editing the draft follows the same single-writer checkout rule as regular maps. When new L6 maps join the category, entering edit **auto-reconciles** them in with an "N new L6 added (unplaced)" toast. If a slot change re-anchors the canvas on the server while you're still holding an earlier checkout, saving is blocked with **"Canvas changed on the server - reload the page"**. Reload and edit again.
...
- **No regular-map workflow**: submit, publish, approvers, collaborators, rename requests, and subprocess designation do not exist on a canvas (hidden in the UI and blocked on the server). Confirmed snapshots and the live draft cannot be deleted. In Map Settings the Collaborators tab is replaced by a read-only **Who can edit** tab that lists the linked category path, the managing department and the chain admins (change rights under Settings → Framework).
- **Category notes**: the **Notes** section at the bottom of the Map tab holds this category's notes. Notes left by the interview import with the **Entry / Flow / Open item / Task note** tags gather here, and category admins and sysadmins add, edit, and delete them.

### Confirm and versions

- **Confirm readiness (gates)**: all six checks on the Approval tab must pass before confirming. The six are **all linked L6 placed · no placeholders · no stale links · all linked L6 published · no exit-less loops · branches use decision nodes** (exempt when every outgoing edge is a parallel fan-out). **Locate** on a failing item jumps to the offending node, and the confirm button stays disabled while any item fails.
- **Who confirms**: the person who **holds the draft checkout** confirms directly when they are a **direct admin of that L5** (or a sysadmin). Admins of a higher category (L1–L4) can edit but send a **Request confirm** instead (optional note). It goes to the direct L5 admins and sysadmins, and **your checkout is released automatically** the moment you send it, so the decider can edit and confirm. You can **withdraw** the request until it is decided, and the outcome arrives as a **Confirm approved / Confirm rejected** notification. Direct admins and sysadmins are told to confirm directly instead of requesting.
...
- Open the **Framework L6 picker** (library button in the top bar, **Add L6 process** in the canvas right-click menu, or `S`), expand another category, and **drag an L6 card onto the canvas** to add an external L6 node (the **L5 explorer** tree at the top left is for travelling to other L5 canvases). It renders with a neutral body, a **colored left tab, and an origin badge** (its home L5 color). Clicking the origin badge opens that L5's **drill-in peek**.
...
- **Placeholders** - the slot for an L6 that is not registered in the system yet shows as a **dashed error-toned node**. The **Connect** banner opens a connect dialog: browse the framework tree (auto-expanded to the guided L5), click a map to open its preview flyout on the right, then press **Connect**. Picking a map outside the guided L5 asks for confirmation, and maps already on the canvas are shown as placed. Editors can also create a slot for an L6 that doesn't exist yet with **Create placeholder** at the bottom of the L6 picker.
- **Replacing stale links**: when a linked map is **unassigned** from the framework, the node switches to a dashed error look with a **"Removed from the framework - replace"** banner; when its slot is **handed over** to another map, the banner reads **"Handed over to (map name) - replace"**; when it's **deleted** with no successor, it reads **"Deleted - replace"**. The confirm checklist's **stale links** item catches this state. Clicking the banner opens the connect dialog with the **successor map pinned as a recommendation card**, so you can swap it in with one click.
- A node that just took over a slot carries a **"Recently handed over"** badge for **14 days**. Hovering any L6 node shows its slot history at the top right of the canvas (where the L5 map tag sits) - when it was handed over, when the map was last updated, and other slot events (assigned / moved / unassigned / deleted).
...
| `Esc` | Cancel action, close modal or popover |
