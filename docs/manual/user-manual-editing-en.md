...
- The CSV uses **20 columns**: `name` (required), `description`, `assignee`, `department`, `system`, `duration`, `touch_time`, `cost_krw`, `cost_usd`, `headcount`, `annual_count`, `fte`, `input`, `input_flags` (per-item required/optional flags), `output`, `start_condition`, `end_condition`, `url`, `url_label`, `next` (the successor to connect to). A `data_form` column in older files is ignored (data forms are per item and not part of the CSV). Older files with fewer columns still import (columns match by name). Put multiple `input`/`output` items on separate lines inside the cell.
...
- A node that just took over a slot carries a **"Recently handed over"** badge for **14 days**. Hovering any L6 node shows its slot history in the bottom-left - when it was handed over, when the map was last updated, and other slot events (assigned / moved / unassigned / deleted).
