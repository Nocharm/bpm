...
- At the bottom of the detail panel sit the **Interview notes** left by the import (the original interview wording for GMP, annual volume, duration, touch time, and system — collapsible, read-only) and the **Notes** section. Notes are added, edited, and deleted by the **map owner**: pick a tag from the preset chips (**Note / Exception / VOC / Rule basis / Open item**) or type `[` in the tag box for the list, add an optional title and the text, then save (`Ctrl`/`⌘+Enter` saves, `Esc` cancels). Notes that came from an import carry an **Imported** mark; editing one shows an **Edited** mark while keeping its source. Non-owners see only the hint "Only the map owner can add or change notes."
