...
- Clicking the **Framework pill** on a map card opens the assign dialog — choose **Add to framework** (assign, or move to another L5), **Unassign**, or **Transfer slot**. Admins of the affected L5 (or a sysadmin) apply the change immediately with a notice dialog; everyone else files a **request**, which L5 admins process from the Inbox's Approval tab, the map's own Approval tab, or Settings' Approval queue (sysadmin) — moving to a different L5 needs approval from **both L5s' admins** (one admin covering both counts as a single approval, and the requester's own side auto-approves). While a request is pending, the dialog shows a **progress banner (n/m)** and the requester can withdraw it; other slot changes are blocked until it's resolved.
