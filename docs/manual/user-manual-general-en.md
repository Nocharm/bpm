...
- Filter chips narrow the list by **Status** (Draft / Pending / Approved / Rejected / Published), by your **Role**, by **Issues** (maps missing an owning department · maps with stale references), and by **SP** (SP maps / non-SP maps). A map whose department, assignee, or user references went stale after a reorg or departure gets a **Stale refs badge** on its card (hover for details), and its owner receives a bundled fix-request notification — admins clean these up in bulk at Settings → Directory → **Orphaned refs**. **Clear filters** resets them. On narrow screens the filter pills shrink automatically (full label → short label → icon).
...
- **Apply To-Be** carries the target's changes forward; **Export** saves the comparison as a PNG — with an info card at the bottom carrying the map name, owning department, owner, versions (base → target), and published date.
...
- **Subprocess designation** — designate this map so it can be used as a subprocess of other maps, and set its representative attributes. The designated values show live on every node that links this map. **Designation requires a published version.** The designation dialog has three sections (each collapsible, with a header icon to fold or unfold them all):
  ...
  - **Detail tiles** — input/output items (the tile shows only the count; the popover edits one item per line with a data form per item — `+ Add new` adds a row).
...
*Business Process Map — Getting Around · Updated 2026-09-12*
