...
- The **AI summary tab** (when AI is enabled on the server) has the AI write up the difference between the two versions — generation **starts ahead of time** as soon as the graphs are ready after you enter the compare screen (a spinner shows on the tab label), and the result comes as an **overview**, **Highlights** (kind chips: added · removed · changed · flow · parameter; click an item to focus it on the canvas), **Points to check**, and activity/flow count chips. **Regenerate** at the top right of the overview card fetches a fresh one, and results are kept per base/target pair until you leave the screen. Very large diffs send at most 200 nodes and 200 edges to the AI.
