...
The right side of the home screen (with no map selected) is your **personal dashboard**: **My profile** at the top (department, role chip, last login) with quick actions (**My maps only** filter, notifications, framework settings, settings), the **activity tiles** below it (Approvals waiting · My requests · Checked-out versions · Unread notifications · My feedback. Clicking the Checked-out versions tile unfolds its list as an accordion), and then six sections in the order **Recently opened | Framework → My department | My documents → Approvals needed | Recent changes**. The profile and tiles stay pinned at the top while you scroll.

- Each section shows only a few items and sends the rest to the list or inbox through a **"N more"** link. With no data it shows a one-line empty state and a hint for the next action. When rows exceed the cap the header stays fixed and only the body scrolls inside; clicking the header expands the whole section. When the same map appears in several sections, hovering it in one highlights the matching row in the others.
- **My department** counts only maps owned by your immediate department; sub-department maps fold into a single count line that opens the **sub-department maps dialog** on click (clicking a department row switches the scope to that department and expands the org tree on the left). The **department scope dropdown** at the right of the section header (my department → parent departments; the choice is remembered) widens the scope.
- The **My department** and **My documents** cards show their distribution under **Status | Version** tabs. The Version tab has four buckets (unpublished · published latest · update in progress · needs re-check), the bar segments and the legend highlight together on hover, and clicking a legend entry filters the list to that bucket.
- Clicking an **Approvals needed** row jumps straight to that approval card in the inbox. **Recent changes** rows show the actor's person pill, the map name, a version chip, and an event icon.
- **One-click delayed navigation**: activity tiles, profile buttons, approval-card links, **Open** on map rows/cards, and clicks on approval, checkout, and recent-change rows do not navigate immediately: a **0.6-second countdown** runs first, then the action fires. Meanwhile the section (or, for a tile, that single cell) is covered by a translucent notice layer with the countdown ring, the destination ("Going to Inbox", "Selecting {map}") and a "click to cancel" hint. Clicking the layer cancels. Map cards outside the sections keep the ring in the icon slot and cancel on a second click, and new-tab modifiers such as `Ctrl`/`⌘`+click follow the browser's default behavior.

### Finding maps

- **Org tree (left)**: browse maps by department through an accordion tree. **My department** is pinned to the top as a favorite. Maps without an owning department gather in a **Dept unassigned** group at the bottom of the tree. The owner clicks the **Dept unassigned** pill on the card to open the org-tree picker, and the department is set only after **Confirm**. Under each department, the linkage canvases whose **managing department** it is are grouped separately as **L5 canvases** and shown as L5 cards (canvas state · ancestor path · Open canvas); canvases in search results use the same card, and clicking one opens that category's summary card on the right.
- The toggle above the list switches between the **Departments** view and the **Framework** view. The Framework view is a company-wide process-classification tree (L1–L5) for browsing maps, currently in a pilot stage. **Maps are assigned only to leaf categories (L5)**; higher levels group what's below, and assigning or transferring a map to a non-L5 category is blocked with a notice banner. The **Open canvas** button on an L5 card opens that category's work-linkage canvas (see "Framework Linkage Canvas" in the **Editing Maps** manual).
- The Framework view is a **drill-down, one level at a time**. Under the **breadcrumb** at the top (L1 → current category; click an entry to go back, or **Up one level**) the list splits into **siblings | children** (1:2): the left column lists the categories on the same level as the current one, the right column its direct children. Clicking a sibling swaps only the right column to that category's children; clicking a child row goes one level down. Each row shows a **level pill** (a color ladder: L1 darkest, fading toward L5), the name, the **direct admin** (a dotted dot when there is none; hovering the row reveals "Unassigned"), and on the right the direct-child count (**L{n} k**) and the map count. **L5 entries are compact cards** that always show the canvas state (fixed in English: **Confirmed / Draft / No canvas**), the direct admin, the map count, **Pending N** (slot changes awaiting approval), and an **Open canvas** button (**Create canvas** when there is none). Clicking an L5 card switches the summary panel on the right to that category. **Its maps are listed there** (the visibility, status, and role filters on the left apply to that list too). Your position is remembered, so a reload, returning from a search, or switching views brings you back to the same spot.
- The **tree icon** to the right of the breadcrumb opens the **Explore hierarchy** floating panel. This window does not dim the page (drag it by its header; it closes only with `×` or `Esc`), so it stays open and follows you as you drill down. The **Tree** tab is a tree with the current path pre-expanded (hovering a descendant highlights its ancestors); the **Diagram** tab is an ERD-style drawing running from the ancestor chain (up to L1) → the current category → its children → grandchildren: left-click = redraw centered on that category, **right-click menu** = Details · View this category (drill down there) · Center on this, drag = pan, wheel = zoom, and the **Whole tree (L1)** toggle shows the entire company tree. The **search** box at the top of the panel searches every category by substring and Korean initial consonants (including mixed, non-contiguous input; spaces ignored) and highlights the match. Result rows show the level pill (hover for the ancestor path), the name, and the L5 count, and **Go** drills down to that category and clears the search.
- **Searching** in the Framework view groups the results **by L5 category**. A group header shows the ancestor path › L5 name plus a canvas chip, and clicking the header drills down to that L5; maps registered nowhere fall into a dashed group, and L1 to L5 categories whose own name matches are listed under **Categories**. When you pick a card from the map list of an L5 summary panel and enter its detail, a **selected-map strip** ("Opened from L5 {name}") stays at the bottom left of the list so **Back to L5** or the browser back button returns you to that list.
- Clicking the **Framework pill** on a map card opens the assign dialog - choose **Add to framework** (assign, or move to another L5), **Unassign**, or **Transfer slot**. Admins of the affected L5 (or a sysadmin) press **Apply now** in the confirmation dialog and the change lands immediately (with a toast); everyone else files a **request**, which L5 admins process from the Inbox's Approval tab, the map's own Approval tab, or Settings' Approval queue (sysadmin). An L5 with no admin is decided by a sysadmin. Moving to a different L5 needs approval from **both L5s' admins** (one admin covering both counts as a single approval, and the requester's own side auto-approves). While a request is pending, the dialog shows a **progress banner (n/m)** and the requester (or the current owner / a sysadmin) can withdraw it; other slot changes are blocked until it's resolved.
- **Deleting** a slotted map, or **copying it with retirement**, follows the same approval flow - deleting opens a dialog to pick an optional successor map, and copying lets you choose **Copy inherits the slot** or **Free the slot**. The linked node on the L5 canvas stays a stale link until a successor is set or the map is restored.
- In the Framework view, drilling into a category or picking an L5 card opens that category's **level detail panel** on the right: a summary (direct children, subtree L5 and map counts, the admin list, the linkage-canvas state (an **Open canvas** button and the latest confirmed version), and the **Subtree confirmation** status (canvases that fail a gate show as "Not ready")) followed by the **list of direct child categories** (clicking one moves the drill-down on the left to that spot) and the **cards of the maps in it**. Map cards are title-first (status fixed in English, secondary info folds on hover, an unconfirmed owner gets a dashed border plus a banner); picking a card switches to the map detail, and clicking empty space brings the panel back. In the department tree, hovering a child row or a map card **highlights the names along the parent path and draws a guide line in the indent**, so you can see at once which category it sits under.
...
- Filters sit on two rows. The top row holds the **visibility** tabs (All · Public · Private), **Sort** (Latest updated · Name · Recently created, one always on) and the clear icon; the bottom row holds four dropdowns: **Status** (Draft / Pending / Approved / Rejected / Published), your **Role**, **Issues** (maps missing an owning department · maps with stale references) and **Type** (SP maps / Non-SP maps / L5 canvases / In the framework / Not in the framework. Values picked inside one dropdown combine with OR, and a value that cannot overlap the current selection is disabled). Every dropdown ends with **Clear this filter**, and the clear icon on the top row resets everything. The sort order also applies to the map lists in the org tree, in search results and in the L5 summary card. A map whose department, assignee or user references went stale after a reorg or departure gets a **Stale refs badge** on its card (hover for details), and its owner receives a bundled fix-request notification. Admins clean these up in bulk at Settings → Directory → **Orphaned refs**. On narrow screens the filter pills shrink automatically (full label → short label → icon).
- Click a card to open the **detail panel**: owner, **owning department**, node count, allowed members (Individuals / Teams / User groups), and the full version history (Created, Submitted, Approved, Rejected, Published, Withdrawn). Maps designated as subprocesses carry an **SP badge**. Each version entry supports a right-click menu (**Go to this version** · **View comments**) and a comment-bubble button (section 3), and with edit access you can add or remove collaborators right on the card. It uses the same staged Save / Undo flow as Settings, with the owner and owning-department rows protected (section 5).
...
- Below the map name the detail panel places **Description | Notes** side by side (3:2, notes as cards); both clip at a fixed height and the header's **Expand / Collapse** shows everything (wheel-scrollable, no scrollbar). **The chevron on the description header folds both sections**; the notes header has no fold control. Under them the **Subprocess info** section shows the designation as tiles: a department pill (click for the org info modal, the other-language name below) and the assignee tile (the designated **Role** chip first, then the person pill: hover or click for the person card), system (a value outside the managed list shows as **Other** with the wording behind the note icon), URL and GMP, metrics, input/output lists (each item tagged with its data form, scrolling inside the tile when there are many) and start/end conditions (bodies capped at three lines). It opens showing only BPM attributes and the Metrics header; hovering the section reveals **Expand all sections** left of the status pill, which unfolds everything. On a map not designated as a subprocess the section starts collapsed and toned down. The **source notes** left by an import (GMP, annual volume, duration, touch time, system) appear as a small dot on the tile icon: hover turns it into a note icon and a click opens the wording popover (with no representative value, the wording shows in the tile itself in small gray text). Notes are added, edited, and deleted by the **map owner**: **+** opens the note dialog; pick a tag from the preset chips (**Note / Exception / VOC / Rule basis / Open item**) or type `[` in the tag box for the list, add an optional title and the text, then **Confirm** (`Ctrl`/`⌘+Enter` confirms, `Esc` or clicking outside cancels). Notes that came from an import carry an **Imported** mark; editing one shows an **Edited** mark while keeping its source. Non-owners see only the hint "Only the map owner can add or change notes."
...
- **Transfer checkout** lets the holder (or the map owner / a sysadmin) pass editing rights to another editor directly.
- A checkout is **never released by time**. It changes only through a transfer, an approved request, or a sysadmin takeover.
...
2. **Submit for approval**: the confirmation dialog lists the approvers who will review, and you can leave an optional comment ("Add a comment (optional)"). On an AI-enabled server an **AI draft** button under the comment writes the change rationale (2 to 4 bullet lines) from the diff against the latest published version. Edit it before sending. This comment grounds the approver's AI summary on the compare screen (opening and the not-covered cross-check). If the version was previously rejected, a **Previous rejection** banner with the reason appears in the dialog. On submit the version becomes #Pending and locks, and all approvers are notified.
...
4. **Publish** the approved version (optional comment). Only the **submitter** can publish. The previously published version automatically becomes #Expired.
...
> **Self-publish (when you are the only approver):** if the approver list is **just you**, clicking Submit shows a **"You are the only approver. Approve and publish in one go?" Yes/No popover**. **Yes** runs submit → approve → publish in one click; **No** submits normally (pending review). Works the same in the editor and in Map Settings → Versions.

> **Approver landing and the "awaiting your approval" notice:** when you open a map that has a **pending** version waiting for your decision, the editor **lands on that version** (an explicit `?version=` in the URL still wins). If you are on another version, the status banner at the top shows an **Awaiting your approval: {version}** link, and the approval-workflow section of the Approval tab is covered by an **"Awaiting your approval · Open that version"** overlay. That section reflects the open version, so the overlay prevents deciding on the wrong one; the decision queue, the compare button, and the designation card stay usable. Anyone viewing a pending version gets a **Compare with published** button on the Approval tab that opens the compare screen for latest published ↔ pending (section 4).

The **approval dashboard** at the bottom of the right inspector shows the stepper (Submit → Review → Publish), each approver's check state, and the available actions. If this map is designated as a subprocess, the approval tab also shows its **designation card** (status and representative attributes). The approval tab and Map Settings → Pending Approvals also consolidate this map's **decision queue**: visibility changes, permission downgrades, map renames, and subprocess registrations, plus framework **Confirm request** and **Slot change** rows (decided by direct L5 admins / sysadmins), each row with **Approve / Reject**. The **Inbox** tab in the top bar carries a badge with the number of items waiting on you.
...
- Differences are highlighted: **Added** (green), **Removed** (red), **Changed** (yellow), with a "Changed: fields" detail and a summary line. **Edge label changes** show in yellow alongside node changes.
...
- Connectors render with the **line style saved on the map** (curved/stepped/straight), and long edge labels wrap to the compare screen's width (120px) so they never cover neighboring nodes.
- Canvas nodes show the **role, department, system, and per-run metric chips** and the **start/end conditions** that have values, and I/O collapses to a single **I/O +N −M** line shown only when items were added or removed. Hover it for a tooltip listing each input/output item with +/−. Chips and lines whose value changed carry a status-colored icon and background tint, so what changed is visible on the node itself. Node sizes are measured and the layout re-spaced so nodes never overlap.
...
- The **Changes** list filters by All / Nodes / Edges; the **Properties** pane shows the selected node's fields (per-run metrics, GMP, I/O & conditions included) side by side. The **All / Changes only** toggle on the tab row (default **Changes only**) picks whether to see only the changed fields or everything. For added, removed, and unchanged nodes the toggle is disabled and the body is dimmed under an "Added / Removed / No changes" watermark to signal it is not a value comparison (the read-only pill sits at the top of the body). Selecting an edge shows its label and line-style changes.
...
- The **AI summary tab** (when AI is enabled on the server) has the AI write the difference between the two versions as a **terse bullet-style report addressed to the approver** (noun phrases, one item per line). Generation starts when you open the tab (a spinner shows on the tab label). The change list is already in the left panel, so the report covers only what the AI can add: **Purpose of the revision** (sections grouped by business intent, not by change type; **Related** chips under each focus the canvas), **Flow and control impact** (based on the whole flow, parameter totals and input/output links: control gaps, lead-time shifts, broken outputs), **Changes not covered by the submission comment** (or a "nothing to cross-check" note when no comment was left), and **Questions before approving** (what to ask the submitter). The submission comment grounds both the opening and the cross-check. The server keeps a report for the same content, so other approvers or a reload get the **stored report** instantly (noted next to the time), and it is rewritten automatically once the draft is edited further. **Regenerate** at the right of the title discards the stored one and fetches a fresh report. Very large diffs send at most 200 nodes and 200 edges to the AI.
- The compare screen accepts `?base=<version id>&target=<version id>` in the URL to open a specific pair directly. The **Compare with published** button on the Approval tab (section 3) uses it.
- When the screen opens, the default pair is **Base = the published version** (the latest confirmed version on a linkage canvas) and **Target = the newest other version**, so the two dropdowns never start on the same version. The properties panel on the right can be **widened by dragging its left edge** between 240 and 520px, and the browser remembers the width. The **eye icon** next to the zoom pill in the bottom-right corner of the canvas opens the **node display card**, where you pick what the compare nodes show (attributes, per-run metrics, inputs and outputs, conditions). The choice is remembered for the compare screen only, and the card closes when you click outside it.
- **Apply To-Be** carries the target's changes forward; **Export** saves the comparison as a PNG. An info card at the bottom carries the map name, owning department, owner, versions (base → target), and published date.
...
- **Owning department** (inside the Details tab): assign or change the map's owning department (Owner / sysadmin only). Its members automatically get Editor access, and changing the department moves that derived Editor access with it. The picker browses the **org tree** with indentation, with your own departments pinned to the top.
- **Collaborators**: add individuals, teams, or user groups as Viewer / Editor. The picker's default (pre-search) list is sorted by **org proximity to you** (the editor's assignee picker sorts the same way, with search-result ranking unchanged). Changes are **staged** (To add / Change / Remove pills) and applied together with **Save changes**, or dropped entirely with **Discard**. Each pill carries a forecast icon: **instant on save** (⚡ Zap, "Applies immediately on save") or **needs approval** (Hourglass, "Needs approval after save"): adds and changes to Viewer grants apply instantly, while **removing an Editor or downgrading one to Viewer goes through approval** (the owner applies everything instantly). Changes awaiting approval stay on the row as an **Approval pending** tag (with the requester's name), visible to everyone; hover your own request to withdraw it in place with the **Withdraw** pill. Duplicate requests overlapping a pending one are blocked, and the owner applying a change directly closes the pending request automatically. Right after saving, the **Undo** button reverts that one save (a confirmation modal lists each item with its forecast icon; leaving the page discards the chance).
...
- **Subprocess designation**: designate this map so it can be used as a subprocess of other maps, and set its representative attributes. The designated values show live on every node that links this map. **Designation requires a published version.** The designation dialog has three sections (each collapsible, with a header icon to fold or unfold them all):
  ...
  - **Detail tiles**: input/output items (the tile shows only the count; the popover edits one item per line with a data form per item, and `+ Add new` adds a row).
...
- **Pending Approvals** / **Checkout Requests**: decide requests targeted at this map. The Pending Approvals tab gathers the four map kinds (visibility change, permission downgrade, map rename, subprocess registration) plus framework confirm requests and slot changes (decided by direct L5 admins / sysadmins) for per-row **Approve / Reject**; requests bundled with a version submission appear only as a read-only **"Decided with version approval"** row.
- **Map Settings of an L5 linkage canvas** are laid out differently. Only Details, **Who can edit**, Visibility, Pending Approvals, Checkout Requests and Danger Zone exist; the Collaborators, Approvers, Versions and Subprocess tabs are gone. **Who can edit** is read-only: it shows the linked category path, the managing department and the admins along the category chain, and marks direct L5 admins as "can confirm". To change editing rights, edit category admins and managing departments under Settings → Framework. The Pending Approvals tab collects this canvas's confirmation requests and slot changes.
...
The **Notices** tab lists currently active announcements. Filter by All / Important / General and search; opening a notice marks it **Read**, and bodies are rendered as Markdown. The read mark is stored **only in this browser (device)**. Opening on another device or browser shows it as unread again.

### Inbox

- **Approvals** tab: your personal review queue: version approvals, checkout transfers, permission / visibility requests, **map renames**, **subprocess registration requests**, and framework **confirm requests** and **slot change requests**. Approve or reject (with a reason) inline; **Open map** jumps to the source. The **Inbox** tab in the top bar carries a badge with your pending count (refreshed every 15 seconds). When a version is rejected, the rejection notification carries the reason at the end. Checkout transfers and permission/visibility changes also show their requests and outcomes on the Notifications tab. With nothing selected, the right pane shows an **activity digest**.
- **Handling a subprocess registration request (owner)**: when someone links your map as an unregistered placeholder and requests registration, a **"Subprocess registration" card** arrives (showing who asked and from which map). **Designate & approve** opens the designation form (department required). **Saving it approves the request** and notifies the requester. Use **Go to published version** to review the map first; a map with **no published version cannot be designated yet** and shows a publish-first notice. **Reject** declines without requiring a reason (optional comment) and notifies the requester.
...
- **Data clean-up notifications**: when your map still references a department, assignee, or user that has left the org chart, the admin's orphaned-reference audit reaches you as a **Stale refs fix request** notification (one per owner, with the map list attached), and an **Owner assigned** notification arrives when an admin changes the owner. When an external L6 placeholder from a consultant import gets connected to a real map, the L5 admins receive an **External L6 connected** notification.
...
- **There is no Add note button.** Notes are written by the **map owner** only (category admins on a linkage canvas). Source notes can be changed only by the editor who **holds the checkout**.
...
*Business Process Map · Getting Around · Updated 2026-09-24*
