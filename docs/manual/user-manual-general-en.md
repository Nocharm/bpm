...
- The **AI summary tab** (when AI is enabled on the server) has the AI write the difference between the two versions as a **terse bullet-style report addressed to the approver** (noun phrases, one item per line) — generation starts when you open the tab (a spinner shows on the tab label), and the result comes as a title, a meta block (map · compared versions · generated time), a one-line opening, numbered sections of bullet items (each with **Related** chips underneath; click one to focus it on the canvas), **Impact and points to check**, and a closing line. A comment left at submission becomes the basis of the opening. The server keeps a report for the same content, so other approvers or a reload get the **stored report** instantly (noted next to the time), and it is rewritten automatically once the draft is edited further. **Regenerate** at the right of the title discards the stored one and fetches a fresh report. Very large diffs send at most 200 nodes and 200 edges to the AI.
