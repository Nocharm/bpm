...
| Valid redirect URIs | `http://<서버호스트>:9787/*` (도메인 추가 시 `https://g-ai-agent.sbiologics.com/*` 도 등록) |
| Valid post logout redirect URIs | 위와 동일 |
| Web origins | `http://<서버호스트>:9787` (+ 도메인) |

> redirect_uri는 앱 origin이다. 포트 직접 접속 단계에서는 `:9787`, 엣지 nginx에 도메인 붙이면 그 도메인도 추가.
...
curl -s http://localhost:9787/api/health           # {"status":"ok"}
curl -s -o /dev/null -w "%{http_code}\n" http://localhost:9787/   # 200
```

브라우저에서 `http://<서버호스트>:9787` → Keycloak 로그인 화면으로 리디렉트되면 인증 연동 정상.

## 5. 트러블슈팅

| 증상 | 확인 |
|------|------|
| 로그인 후 redirect 오류 | Keycloak Valid redirect URIs에 `:9787/*` 등록됐는지 |
