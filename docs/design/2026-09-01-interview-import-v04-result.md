...
- **self edge(A→A)는 드랍하지 않는다(2026-09-02)** — 분기 판단 노드 `a{seq}r`("반복 여부(자동 생성됨)")를
  합성해 `A→◇→A` 루프백으로 그리고, A의 기존 진출은 ◇로 이설(택일은 분기에서 갈라진다).
  이설로 승격 근거를 잃은 A는 process 복원. quote 노트 보존, dry-run 노티에 자동 생성 노드명·코드 명시
...
- **self edge도 분기 노드로 푼다(2026-09-02)** — 어댑터가 `kind:"loop"`으로 강제 유지, 단독 진출이어도
  분기 노드를 세워 `A→◇→A` 루프백(back 등록으로 랭크 보호). 루프 유래 분기 노드 이름은 고정
  "반복 여부(자동 생성됨)"(팬아웃 유래는 "{이름} 결과" 유지)
...
| **분기 노드 제목** | 팬아웃 유래는 `"{선행 L6 이름} 결과"`, 루프(self) 유래는 고정 `"반복 여부(자동 생성됨)"`(2026-09-02, `LOOP_BRANCH_NODE_NAME` 단일 소스) 자동 생성. 전달물에 분기 명칭 키가 생기면 그걸 우선하도록 교체 |
