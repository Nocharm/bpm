...
**변경 (b) — `components/permissions/create-map-dialog.tsx`** *(2026-07-10 두 차례 개정)*

드롭다운 자리를 모달 기하로 확보하려는 시도를 두 번 했고, 둘 다 사용자가 반려했다. 기록:

1. 중앙 정렬 유지 + `max-h-[calc(100dvh-13rem)]` → 580px에서 모달이 372px까지 줄어 본문 스크롤만 늘었다.
2. 상단 정렬 + `max-h-[calc(100dvh-4rem)]` + 본문 `pb-40` → 뷰포트와 무관하게 아래로 열렸지만, 긴 화면(≥1080px)에서 160px가 액션행 위 **죽은 여백**으로 남고 모달이 993px까지 커졌다.

**최종:** 모달 기하는 드롭다운을 위해 희생하지 않는다.

- 백드롭 `items-center` → **`items-start pt-4`** (상단 정렬, 위로 최대한)
- 모달 **`max-h-[calc(100dvh-2rem)]`** (위아래 1rem)
- 본문 패딩 없음 — **내용이 다 들어가면 스크롤이 생기지 않는다**

결과(1280 폭 실측): 뷰포트 900px 이상에서 모달은 833px 고정·스크롤 0. 그 아래에선 `max-h`에 걸려 본문만 스크롤한다. 액션 버튼은 500px에서도 화면 안(스크롤 컨테이너 밖이라 밀리지 않는다).

드롭다운 방향은 배치 알고리즘에 전적으로 맡긴다 — 뷰포트 ≥1000px면 아래로 5줄, 그 미만이면 옆으로. **어느 경우에도 잘리지 않고 위로 뒤집히지 않는다.** 정확성은 실측 `spaceBelow`가 보장한다.
