...
- **3차 확정(2026-08-20)**: 필은 **노드 안쪽**(사각/SP=본문 첫 줄, 마름모=상단 중앙). **일반 노드는 분류가 노드 색을 자동 확정**(`GMP_NODE_COLORS` — 상태 토큰 hex 미러, 미분류 선택은 색 불변) — 변경 시 마우스 지점(X 버튼 기준) 안내 모달이 분류 배지·색 스와치 before→after를 보여주고 **"이전 분류로 되돌리기"(분류+색)·"색만 되돌리기"(분류 유지)** 2단 복원 제공. 그룹 일괄 편집은 원장 파생이라 touch_time 자동 포함(SP 멤버는 게이트가 제외) — bulk-params 테스트+실브라우저 5체크.
