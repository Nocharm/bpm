...
- **출구는 오른쪽 위 → 아래 → 옆 순으로 벌린다**(`plan_branch_fanout`, 사용자 결정 2026-09-01).
  ...
  4번째부터는 위/아래로 한 칸씩 더 벌린다. 행 배정은 **먼저 배정된 쪽이 이긴다**(두 갈래가 같은
  L6로 합류할 때 나중 분기가 위치를 흔들면 안 된다), 출구 변은 최종 행에서 되뽑아 기하와 일치시킨다
...
링크 그룹 불변식은 `2026-08-21-io-linking-design.md` §2 그대로.
