...
- [`version-lifecycle-summary.md`](version-lifecycle-summary.md) — 버전 수명주기(번호·만료·점유권·재게시) 요약.

## 배포 · DB
- [`deploy.md`](deploy.md) — 서버 docker-compose 배포 절차(포트 3333/nginx 토폴로지).
- [`db-seed.md`](db-seed.md) — DB 초기화·데모 시드(`python -m scripts.reset_db`).
- [`db-migration-9910.md`](db-migration-9910.md) — 2차 업데이트: 운영 DB 복사 → 검증 스택(9910) 마이그레이션.
- [`db-migration-9800.md`](db-migration-9800.md) — 1차 업데이트: 운영 DB 복사 → 검증 스택(9800) 마이그레이션.

## QA · 검증
- [`alarm-audit.md`](alarm-audit.md) — 알림 기능 전수 조사·감사.
- [`ai-connectivity-test.md`](ai-connectivity-test.md) — AI(LLM) 연결성 점검 절차.
- [`ai-real-model-smoke.md`](ai-real-model-smoke.md) — AI 실모델 스모크 테스트.
...
## 설계 기록 ([`superpowers/specs/`](superpowers/specs/README.md))
기능별 설계 스냅샷(날짜별). 분야별 목록은 specs 인덱스 참고. 코드 주석이 경로로 참조하니 **이동 금지**.
