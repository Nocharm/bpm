...
1. **점유권 요청 승인자**: (a) 현재 점유자만 (b) 점유자 **또는** 맵 오너/sysadmin. → **추천 (b)**.
2. **만료본 표시**: 버전 드롭다운/타임라인에서 만료본도 선택 가능(재게시 위해) — 노출 유지. → **추천: 노출**.
3. **버전 라벨(좌상단)** 텍스트: 고정 "Version"인지, 현재 선택 버전의 풀네임(`v3 · To-Be`)인지. → **추천: 현재 버전 풀네임**(pill은 전환 트리거).
4. **리네임/삭제 가능 범위**: 드래프트만(점유자) vs 게시본도? → **추천: 드래프트만 리네임·삭제**(게시본은 불변, 만료는 재게시).
