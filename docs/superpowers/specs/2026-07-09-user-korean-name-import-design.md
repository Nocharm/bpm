...
JSON 파싱 오류·형식 불일치는 모달 내 에러 메시지로 표시하고 요청을 보내지 않는다. 루트가 **배열**이면 조회 도구 응답 포맷(`[{userId, status, name, dept, ...}]`)으로 판별해 `status !== "found"` 항목은 무시하고, 루트가 **객체**면 수동 맵(`{"login_id": "이름"}` — 문자열 값)으로 판별한다 — 두 포맷을 자동 판별해 처리한다.
...
2026-07-09 후속: 배열 포맷·korean_dept 분리 저장·부서 매핑은 dept-korean-mapping design 참조.
