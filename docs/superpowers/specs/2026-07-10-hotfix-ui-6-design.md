...
**변경 (b) — `components/permissions/create-map-dialog.tsx`**

모달 `max-h-[calc(100dvh-2rem)]` → `max-h-[calc(100dvh-13rem)]`.

수직 중앙 정렬(`ModalBackdrop … items-center`)이므로, 뷰포트 높이 `H`·모달 높이 `M`에 대해

```
결재자 input 하단 여백 = (H - M)/2 + K
K = pb-6(24) + 액션행(≥29) + gap-5(20) ≥ 73     // 하한. 에러문구가 뜨면 더 커진다
```

알고리즘이 아래로 열려면 `여백 ≥ 160 + GAP(4) + MARGIN(8) = 172`.
`M ≤ H - 208`(13rem)이면 `여백 ≥ 104 + 73 = 177 ≥ 172` — **뷰포트 580px에서도 드롭다운이 아래로 온전히 열린다.**

대가: 짧은 화면에서 모달이 낮아져 본문 내부 스크롤이 늘어난다. 이것이 요청받은 "가장 아래지점을 위로 올리기"다.

`13rem`은 위 하한 계산에서 나온 **시작값**이며 검증 4번(Playwright 1280×580)에서 실측 확인한다. 계산이 어긋나도 피커는 실측 `spaceBelow`로 판단하므로 잘리지 않고 옆으로 열린다 — 정확성은 알고리즘이, 심미성("아래로 열림")은 이 값이 담당한다.
...
- **중첩 프로퍼티 이름을 모른다** → 각 노드의 *배열 값을 가진 모든 프로퍼티*를 재귀 순회한다. `children`/`subDept`/`childList` 무엇이든 동작. `enDeptNm` 없는 노드는 항목으로 취급하지 않되 자식 순회는 계속.
