...
### 10.1 플레이스홀더 계약 (2026-08-29 확정)

- **정의**: `linked_map_id`가 빈 subprocess 노드 = "존재해야 하지만 아직 시스템에 등록되지 않은 L6의 자리". 생성 경로(L5 임포트 / 라이브러리 드래그) 구분 없이 같은 상태.
- **임포트 시나리오**: 컨설턴트 L5 임포트의 L6 관계에 **다른 L5 소속 L6**가 등장하면(이름+소속 L5까지만 아는 상태) 플레이스홀더로 생성하고 **출처 L5 정보(카테고리 참조)를 노드에 저장**해 배지로 표시한다.
- **그릇 (구현됨 2026-08-29)**: `nodes.placeholder_category_id`(FK 없는 앱 경계 검증 — 미지 카테고리 422)·연계 캔버스 검증의 플레이스홀더 허용·응답 트랜지언트 `placeholder_category_path`(그래프 라우터 주입)·확정 스냅샷 clone 보존·확정 게이트 시그니처 포함(링크 정체성)·에디터 저장 왕복+출처 배지(에러 톤). 임포트가 이 필드를 채우면 즉시 표시된다.
- **후차 연결**: (a) **수동 지정 — 구현됨 2026-08-29**: 플레이스홀더 배너 CTA → 연결 다이얼로그(출처 L5 후보 우선 + 유사도 정렬·정확 일치 배지 + 트리 드릴로 타 L5 탐색 + 이미 캔버스에 있는 맵 비활성). 안내 밖 L5 선택 시 경로 비교 확인 모달 게이트. 연결 즉시 `placeholder_category_id` 소거·follow_latest 기본. (b) 재임포트 시 이름·카테고리 매칭 자동 연결 — 임포트 트랙에서 확정.
- **스테일 링크 교체 (구현됨 2026-08-30)**: 링크맵이 삭제/이양되면 노드 배너가 **Replace map CTA**로 전환(같은 다이얼로그 재사용, 출처=옛 맵 카테고리). 이양(retire_source)은 `process_maps.retired_to_map_id`로 계보를 남기고, refs가 살아있는 맵까지 체인 추적한 **후계자를 추천 카드로 고정 노출**(직결·게이트 없음). 지정해제/삭제 상태의 노드 바디는 플레이스홀더와 같은 점선 에러 룩으로 통일(코너 삼각 배지 제거).
- **시각 (구현됨 2026-08-29)**: 플레이스홀더 = 점선 보더+에러 레드 톤다운 채움+동톤 하단 배너(전 에디터 표면 공통, `linkedMapId === null` 명시 시에만). 링크가 지정된 외부 L6 = 뉴트럴 바디+좌측 5px 홈 L5 컬러 탭+틴트 배지·아이콘(C안).
