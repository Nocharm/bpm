...
- **임포트 시나리오**: 컨설턴트 L5 임포트의 L6 관계에 **다른 L5 소속 L6**가 등장하면(이름+소속 L5까지만 아는 상태) 플레이스홀더로 생성하고 **출처 L5 정보(카테고리 참조)를 노드에 저장**해 배지로 표시한다. → 출처 컬럼·연계 캔버스 검증의 플레이스홀더 허용은 임포트 트랙에서 구현(현재 검증은 linked 필수).
