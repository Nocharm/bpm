...
| 로그인 기록 수집 | (부가 기능) 사용자 현황조사용 `login_records` 테이블(`LoginRecord`: login_id·name·occurred_at). `/api/me`에서 **KST 일자별 1건 중복제거**(맵 새 탭/새로고침/토큰갱신마다 찍히던 것 수정) = "그날 접속" 단위. startup create_all로 테이블 자동 생성. 집계·리포트는 후속 |
