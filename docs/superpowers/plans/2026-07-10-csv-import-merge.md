...
### Task 2: Assignee(login_id) · Department 컬럼 + 비차단 경고 (TDD, 순수 모듈)
...
const H8 = "Name,Assignee,Department,System,Duration,URL,URL_Label,Next";
...
describe("buildGraphFromCsv — Assignee/Department 컬럼", () => {
  it("login_id를 이름으로 해석한다", () => {
    const o = outcomeOf(`${H8}\n요청 검토,hong.gd,Quality Part 1,,,,,\n`, DIR);
...
노드 생성 시 두 필드를 채운다 (`NODE_DEFAULTS` 뒤):
```ts
...
npm test -- csv-import
```
Expected: PASS — 기존 23개 + 신규 10개 = 33개.

```bash
npm run lint && npm test && npm run build
```
Expected: lint 0 errors · `Tests 172 passed` · build 성공.

- [ ] **Step 5: 커밋**

`PROGRESS.md`:
```markdown
- ①-b CSV 컬럼 확장 — Assignee(login_id→이름 해석, 이름 직접 표기도 통과)·Department(한글 부서명→정식명) + 비차단 경고(미해석 담당자·미지 부서·부서 불일치). 백엔드는 담당자를 검증하지 않아 프론트 드리프트 배지가 유일한 안전망. vitest 172·lint 0에러.
```

```bash
git add frontend/src/lib/csv-import.ts frontend/src/lib/csv-import.test.ts PROGRESS.md
git commit -m "$(cat <<'EOF'
feat(csv-import): add Assignee (login_id) and Department columns — CSV에 담당자·부서 열 추가
...
- Produces: 8열 헤더 `Name,Assignee,Department,System,Duration,URL,URL_Label,Next`

⚠️ 샘플 3종은 **이미 낡았다** — 헤더가 `Name,System,Duration,URL,Next`로 `URL_Label`이 빠져 있다. 파서가 열 부분집합을 허용해 조용히 통과 중이었다. 8열로 재작성한다. 앞의 BOM(`﻿`)은 유지한다.
...
export function buildTemplateCsv(): string {
  return [
    "Name,Assignee,Department,System,Duration,URL,URL_Label,Next",
    "Review request,hong.gd,Quality Part 1,SAP ERP,2 days,,,Approval decision",
...
⚠️ 3행은 셀 안에 콤마가 있어 **작은따옴표 문자열 + 큰따옴표 셀**로 적는다. `join("\r\n")` 뒤 파서를 다시 통과해야 한다.

- [ ] **Step 2: `buildAiPromptText`에 두 컬럼 규칙 추가**
...
`[작성 규칙]`의 마지막 줄을 두 열까지 포함하도록 고친다:

```ts
    "- 문서에 없는 단계를 지어내지 말고, 불명확한 속성(Assignee·Department·System·Duration·URL)은 비워두세요.",
    "- 빈 칸은 기존 값을 지웁니다가 아니라 '건드리지 않음'입니다 — 이미 있는 맵에 임포트해도 기존 값이 보존됩니다.",
```

- [ ] **Step 3: 샘플 CSV 3종 재작성**

각 파일의 헤더를 8열로 바꾸고 모든 행의 셀 수를 맞춘다. Assignee/Department는 실제 디렉터리를 모르므로 **비워 둔다**(경고 없이 임포트되게). `URL_Label` 열도 추가한다.

`docs/samples/csv-sample-01-procurement.csv`:
```
Name,Assignee,Department,System,Duration,URL,URL_Label,Next
...
```
(이하 기존 행들을 같은 방식으로 8열화 — 원본의 `Next` 값은 그대로 유지한다.)
...
python3 - <<'PY'
import pathlib
NEW = "Name,Assignee,Department,System,Duration,URL,URL_Label,Next"
for p in sorted(pathlib.Path("docs/samples").glob("*.csv")):
    ...
    for line in lines[1:]:
        if not line.strip():
            continue
        # 5열 → 8열. 따옴표 셀이 없는 샘플이라 단순 split 으로 충분(검증: 셀 5개)
        cells = line.split(",")
...
`PROGRESS.md`:
```markdown
- ①-b 템플릿·AI 프롬프트에 Assignee(계정 id)·Department 규칙 추가, "빈 칸=건드리지 않음" 명시. `docs/samples/*.csv` 3종은 헤더가 URL_Label 없이 낡아 있어 8열로 재작성. vitest 172·lint 0에러.
```
...
- 매칭 노드는 `id`·좌표·`description`·`color`·`group_ids`·서브프로세스 링크를 보존.
- **값이 있는 셀만 덮어쓴다** (`assignee`·`department`·`system`·`duration`·`url`·`url_label`). 빈 셀은 기존 값 유지.
...
describe("buildGraphFromCsv — 머지", () => {
  ...
  it("값이 있는 셀은 덮어쓴다", () => {
    const o = buildGraphFromCsv(`${H8}\nReview request,kim.cs,Finance Part,ERP,5 days,,,\n`, { base: baseGraph(), directory: DIR });
    const node = o.graph!.nodes.find((n) => n.id === "a1")!;
    expect(node.assignee).toBe("김철수");
    expect(node.department).toBe("Finance Part");
...
```ts
  ...
  // 매칭 노드: id·좌표·설명·색·그룹·서브프로세스 링크 보존.
...
npm test -- csv-import
```
Expected: PASS — 33개 + 신규 17개 = 50개 (머지 14 + `withKeptNodes` 3).

```bash
npm run lint && npm test && npm run build
```
Expected: lint 0 errors · `Tests 189 passed` · build 성공.

- [ ] **Step 5: 커밋**

`PROGRESS.md`:
```markdown
- ② `buildGraphFromCsv(text, context?)` 이름 기준 머지 — 제목 일치 노드 id 재사용(계보·코멘트·그룹 보존), 빈 셀=기존 값 유지, 서브프로세스 node_type 보존, 신규 노드만 부분 dagre. `withKeptNodes` 추가. vitest 189·lint 0에러.
```
...
`PROGRESS.md`:
```markdown
- ② 에디터 배선 — CsvImportSection `context`(base + eligible 디렉터리), 요약/확인 모달을 추가·갱신·삭제 실카운트로, 행 경고 노출, Import 버튼을 `eligible !== null`로 게이팅. vitest 189·lint 0에러.
```
...
```
Expected: `Tests 195 passed`

- [ ] **Step 3: 커밋**

`PROGRESS.md`:
```markdown
- ② 비교 회귀 테스트 `diff.test.ts` 신설 — 클론+머지 시나리오에서 미변경 엣지가 오탐되지 않고 실제 변경만 잡히는지 6케이스. vitest 195.
```
...
npm run lint && npm test && npm run build
```
Expected: lint 0 errors (특히 `react-hooks/preserve-manual-memoization` 무발생) · `Tests 195 passed` · build 성공.
...
`PROGRESS.md`:
```markdown
- ③ 프리뷰 상태 기계 일반화(`aiPreviewRef`→`previewRef` + `previewSource`) + CSV 머지 프리뷰 진입/확정/취소. 소멸 노드·엣지 `diffStatus`/빨간 점선. 확인 모달 폐지. vitest 195·lint 0에러. ⚠️ Apply/Cancel UI는 다음 커밋(Import 탭).
```
...
npm run lint && npm test && npm run build
```
Expected: lint 0 errors · `Tests 195 passed` · build 성공.

`PROGRESS.md`:
```markdown
- ③ 인스펙터 Import 탭(`forcedTab`/`lockTabs`, 프리뷰 중 다른 탭·접기 잠금) — MarkdownView 요약 + 행 경고 + 소멸 노드 React 리스트(클릭→캔버스 포커스) + 삭제/유지 세그먼트 + Apply/Cancel, 버튼별 리치 툴팁. vitest 195·lint 0에러.
```
...
4. 템플릿을 다운로드해 **8열 헤더**(`Name,Assignee,Department,System,Duration,URL,URL_Label,Next`)인지 확인.
...
`PROGRESS.md`:
```markdown
- ③ 브라우저 실검증 12케이스 — 새맵 준비 UI·에디터 자동 이동·8열 템플릿·담당자 해석/경고·빈 셀 보존·머지 하이라이트·탭 잠금·툴팁·포커스·Keep/Apply 보존·비교 정상화·Cancel 복귀.
```
...
- `npm run lint` 0 errors · `npm test` 195 passed (162 baseline + 10 컬럼 + 17 머지 + 6 diff) · `npm run build` 성공
