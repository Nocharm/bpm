...
| 파일 | 책임 | 변경 |
|------|------|------|
| `frontend/src/lib/csv-import.ts` | CSV 파싱 + **머지 그래프 생성**(순수) | 수정 (Task 2) |
| `frontend/src/lib/csv-import.test.ts` | 위의 단위 테스트 | 수정 (Task 2) |
| `frontend/src/lib/diff.test.ts` | 머지 후 비교가 실제 변경만 잡는지 (신규 파일) | 생성 (Task 4) |
| `frontend/src/components/csv-template-actions.tsx` | 템플릿 다운로드 + AI 프롬프트 복사 두 버튼 | 생성 (Task 1) |
| `frontend/src/components/csv-import-section.tsx` | 위 + 파일선택/붙여넣기/요약 | 수정 (Task 1, 3) |
| `frontend/src/components/permissions/create-map-dialog.tsx` | 맵 생성 — 임포트 제거, 노티스 | 수정 (Task 1) |
| `frontend/src/components/csv-import-tab.tsx` | 인스펙터 Import 탭 본문 | 생성 (Task 6) |
| `frontend/src/components/inspector-panel.tsx` | `import` 탭 + `forcedTab`/`lockTabs` | 수정 (Task 6) |
| `frontend/src/app/maps/[mapId]/page.tsx` | 프리뷰 상태 기계 · 배선 | 수정 (Task 3, 5, 6) |
| `frontend/src/lib/i18n-messages.ts` | 신규 키 (en+ko) | 수정 (Task 1, 3, 6) |
...
- Produces: `CsvTemplateActions({ disabled }: { disabled?: boolean })` — 이후 태스크는 쓰지 않는다.

- [ ] **Step 1: `CsvTemplateActions` 컴포넌트 생성**

`frontend/src/components/csv-import-section.tsx:39-54`의 `handleDownloadTemplate`·`handleCopyPrompt`와 `OUTLINE_BTN` 상수, 두 버튼 JSX(`:85-109`)를 그대로 옮긴다.
...
⚠️ 원본 41행의 BOM은 소스에 리터럴 U+FEFF로 박혀 있다. 위처럼 `"﻿"` 이스케이프로 옮겨 적어야 편집기가 문자를 먹지 않는다.

- [ ] **Step 2: `CsvImportSection`이 `CsvTemplateActions`를 품게 리팩터**

`csv-import-section.tsx`에서 `handleDownloadTemplate`·`handleCopyPrompt`·`promptCopied` state·`OUTLINE_BTN` 상수·두 버튼 JSX와 이제 안 쓰는 import(`Check`, `Download`, `Sparkles`, `buildAiPromptText`, `buildTemplateCsv`)를 제거하고, 버튼 행 맨 앞에 `<CsvTemplateActions disabled={disabled} />`를 둔다. 나머지 버튼은 `CSV_OUTLINE_BTN`을 import해 쓴다.
...
```tsx
      <div className="flex flex-wrap items-center gap-2">
        ...
        {/* csv-paste-toggle 버튼은 기존 그대로, className만 CSV_OUTLINE_BTN 사용 */}
```

- [ ] **Step 3: i18n 키 추가 · 수정 · 삭제**

`frontend/src/lib/i18n-messages.ts` — `en` 블록(≈96행 `csvImport.sectionTitle` 근처)과 `ko` 블록(≈1358행) **양쪽** 모두.
...
삭제 (이 태스크가 만든 고아 — 마지막 사용처가 Step 4에서 사라진다):
```ts
...
제거할 것:
- import: `acquireCheckout`, `saveGraph` (from `@/lib/api`), `CsvImportSection`, `type CsvImportOutcome`
- state: `csv`, `csvFileName` (`:137-138`), `createdRef` (`:140`)
- `handleCreate`(`:207-256`)의 `createdRef` 가드와 CSV 분기 전체
- `canCreate`(`:259-264`)의 `(csv === null || ...)` 절

추가할 것: `import { CsvTemplateActions } from "@/components/csv-template-actions";`

`handleCreate` 본문을 다음으로 교체한다 (deps 배열에서 `csv` 제거):
...
`lucide-react` import에 `Info`를 추가한다 (`:13`의 `{ X, Globe, Lock }` → `{ X, Globe, Lock, Info }`).
...
`csvImport.mapCreatedImportFailed`가 정말 미참조인지 확인 (ugrep 대괄호 함정 회피):
```bash
python3 -c "
import pathlib
...
`PROGRESS.md`의 `## 2026-07-10 — CSV 임포트 머지 전환 설계` 섹션 아래에 한 줄 추가:
```markdown
...
**Interfaces:**
- Consumes: `Graph`, `GraphNode`, `GraphEdge` (`@/lib/api`), `layoutWithDagre`, `layoutSubsetWithDagre`, `normalizeNodeType`, `AppNode` (`@/lib/canvas`), `genId` (`@/lib/id`)
- Produces:
  ```ts
  ...
  export interface CsvImportOutcome {
    graph: Graph | null;
    nodeCount: number;           // 결과 그래프 노드 총수
    edgeCount: number;           // 결과 그래프 엣지 총수
    errors: CsvImportError[];
    ignoredLabelCount: number;
    merge: CsvMergeInfo;         // base 미지정 시 addedNodeIds=전 노드, removed/lost=[]
  }
  export function buildGraphFromCsv(text: string, base?: Graph): CsvImportOutcome
  export function withKeptNodes(graph: Graph, kept: GraphNode[]): Graph
  ```

- [ ] **Step 1: 실패하는 테스트를 쓴다**

`frontend/src/lib/csv-import.test.ts` 하단에 추가. 상단 import에 `withKeptNodes`, `type GraphNode`를 더한다.
...
function baseGraph(): Graph {
  return {
    nodes: [
      { ...NODE_BASE, id: "s1", title: "시작", node_type: "start", sort_order: 0, pos_x: 0, pos_y: 0 },
      {
        ...NODE_BASE,
        id: "a1",
      ...
      { ...NODE_BASE, id: "e1", title: "종료", node_type: "end", sort_order: 2, pos_x: 600, pos_y: 0, is_primary_end: true },
    ],
    edges: [
...
describe("buildGraphFromCsv — 머지", () => {
  it("제목이 같은 노드는 id를 재사용한다 (계보·코멘트 보존의 근거)", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,SAP,2 days,,\n`, baseGraph());
    expect(outcome.errors).toEqual([]);
    const review = outcome.graph!.nodes.find((n) => n.title === "Review request");
    expect(review!.id).toBe("a1");
    expect(outcome.merge.matchedCount).toBe(3); // start + Review request + end
    expect(outcome.merge.addedNodeIds).toEqual([]);
    expect(outcome.merge.removedNodes).toEqual([]);
  });

  it("CSV가 싣지 않는 필드는 보존하고, 싣는 필드만 덮어쓴다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,SAP,2 days,,\n`, baseGraph());
    const review = outcome.graph!.nodes.find((n) => n.id === "a1")!;
    expect(review.system).toBe("SAP");
    expect(review.duration).toBe("2 days");
    expect(review.color).toBe("#334155");
    expect(review.assignee).toBe("kim");
    expect(review.department).toBe("QA");
    expect(review.description).toBe("기존 설명");
    expect(review.group_ids).toEqual(["g1"]);
    expect(review.pos_x).toBe(300);
  });

  it("기존 그룹을 그대로 통과시킨다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`, baseGraph());
    expect(outcome.graph!.groups).toEqual([{ id: "g1", parent_group_id: null, label: "검수", color: "" }]);
  });

  it("Start/End는 타입으로 매칭하고 기존 제목을 유지한다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`, baseGraph());
    const start = outcome.graph!.nodes.find((n) => n.node_type === "start")!;
    const end = outcome.graph!.nodes.find((n) => n.node_type === "end")!;
  ...
  it("서브프로세스 노드는 node_type을 보존한다 (Call Activity 링크 유지)", () => {
    ...
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`, base);
    const review = outcome.graph!.nodes.find((n) => n.id === "a1")!;
    expect(review.node_type).toBe("subprocess");
    expect(review.linked_map_id).toBe(7);
  });

  it("CSV에만 있는 행은 신규 노드가 되고 addedNodeIds에 담긴다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,Sign contract\nSign contract,,,,\n`, baseGraph());
    const sign = outcome.graph!.nodes.find((n) => n.title === "Sign contract")!;
    expect(outcome.merge.addedNodeIds).toEqual([sign.id]);
    expect(sign.id).not.toBe("a1");
  });

  it("base에만 있는 노드는 결과에서 빠지고 removedNodes로 보고된다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nSign contract,,,,\n`, baseGraph());
    expect(outcome.graph!.nodes.some((n) => n.id === "a1")).toBe(false);
    expect(outcome.merge.removedNodes.map((n) => n.id)).toEqual(["a1"]);
  });

  it("결과 그래프에 없는 base 엣지를 lostEdges로 보고한다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nSign contract,,,,\n`, baseGraph());
    expect(outcome.merge.lostEdges.map((e) => e.id).sort()).toEqual(["x1", "x2"]);
  });

  it("흐름이 그대로면 lostEdges가 비어 있다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`, baseGraph());
    expect(outcome.merge.lostEdges).toEqual([]);
  });

  it("신규 노드만 재배치하고 매칭 노드 좌표는 건드리지 않는다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,Sign contract\nSign contract,,,,\n`, baseGraph());
    expect(outcome.graph!.nodes.find((n) => n.id === "a1")!.pos_x).toBe(300);
    expect(outcome.graph!.nodes.find((n) => n.id === "s1")!.pos_x).toBe(0);
  });

  it("base 미지정이면 현행과 동일하게 전량 신규다 (회귀)", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`);
    expect(outcome.merge.removedNodes).toEqual([]);
    expect(outcome.merge.matchedCount).toBe(0);
    expect(outcome.merge.addedNodeIds).toHaveLength(3); // Start + 1행 + End
    expect(outcome.graph!.groups).toEqual([]);
  });
...
describe("withKeptNodes", () => {
  it("소멸 노드를 엣지 없이 되돌리고 sort_order를 뒤에 붙인다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nSign contract,,,,\n`, baseGraph());
    const maxOrder = outcome.graph!.nodes.reduce((max, n) => Math.max(max, n.sort_order), 0);
    const kept = withKeptNodes(outcome.graph!, outcome.merge.removedNodes);
  ...
  it("빈 배열이면 그래프를 그대로 반환한다", () => {
    const outcome = buildGraphFromCsv(`${HEADER}\nReview request,,,,\n`, baseGraph());
    expect(withKeptNodes(outcome.graph!, [])).toBe(outcome.graph!);
  });
});
```

⚠️ 기존 `HEADER` 상수는 `"Name,System,Duration,URL,Next"`(5열)다. `url_label`이 빠져 있으므로 위 CSV 행들은 5칸이다. 행 문자열의 콤마 개수를 `HEADER`와 맞춘다.
...
npm test -- csv-import
```
Expected: FAIL — `buildGraphFromCsv` 두 번째 인자 미지원 / `withKeptNodes` is not exported / `outcome.merge` undefined.

- [ ] **Step 3: `csv-import.ts` 구현**

import에 `layoutSubsetWithDagre`를 추가한다:
```ts
...
인터페이스 확장 (`CsvImportOutcome` 교체, `:20-26`):
```ts
export interface CsvMergeInfo {
  // CSV에만 있어 새로 만든 노드 id — 프리뷰에서 "added" 하이라이트 + 부분 자동정렬 대상
  addedNodeIds: string[];
...
`fail()` 헬퍼(`:124-130`)에 빈 merge를 더한다. **공유 상수가 아니라 매번 새 객체**로 만든다 — 배열을 공유하면 호출자의 실수 한 번이 전역을 오염시킨다.
```ts
...
const fail = (errors: CsvImportError[]): CsvImportOutcome => ({
  graph: null, nodeCount: 0, edgeCount: 0, errors, ignoredLabelCount: 0, merge: emptyMerge(),
});
```

시그니처와 노드 생성부(`:249-348`)를 교체한다. `nextsOf`/`errors` 계산까지는 현행 그대로 두고 그 아래를 다음으로 바꾼다:

```ts
export function buildGraphFromCsv(text: string, base?: Graph): CsvImportOutcome {
  // ... (기존 파싱·검증 — 변경 없음. `if (errors.length > 0) return fail(errors);` 까지)
  ...
  // 매칭 노드: id·좌표·CSV 미탑재 필드(설명/색/담당자/부서/그룹/서브프로세스 링크) 보존.
  ...
  const nodes: GraphNode[] = [
    ...
    mergeNode(baseStart, {
      ...NODE_DEFAULTS, id: startId, title: baseStart?.title ?? "Start", node_type: "start", sort_order: 0,
    }),
    ...rows.map((row, i) =>
    ...
    {
      ...mergeNode(baseEnd, {
        ...NODE_DEFAULTS, id: endId, title: baseEnd?.title ?? "End", node_type: "end", sort_order: rows.length + 1,
      }),
  ...
  // ── 엣지 (현행 그대로 — idOf/startId/endId만 위에서 해석됨) ──
  const edges: GraphEdge[] = [];
  const addEdge = (source: string, target: string, label: string) => {
    edges.push({
      id: genId(), source_node_id: source, target_node_id: target, label,
  ...
  const lostEdges = (base?.edges ?? []).filter(
    (e) => !keptEdgeKeys.has(`${e.source_node_id}→${e.target_node_id}`),
  );

  return {
    graph: { nodes: positioned, edges, groups: base?.groups ?? [] },
...
좌표 헬퍼 두 개를 `buildGraphFromCsv` 위에 둔다 (기존 dagre 블록 `:309-340`을 흡수):
...
`withKeptNodes`를 파일 끝(`buildTemplateCsv` 앞)에 추가한다:
...
npm test -- csv-import
```
Expected: PASS — 기존 23개 + 신규 13개.

기존 호출부가 `outcome.merge`를 몰라도 타입은 맞는다(추가 필드). 전체 게이트:
```bash
npm run lint && npm test && npm run build
```
Expected: lint 0 errors · `Tests 175 passed` · build 성공.

- [ ] **Step 5: 커밋**

`PROGRESS.md`에 추가:
```markdown
- ② `buildGraphFromCsv(text, base?)` 이름 기준 머지 — 제목 일치 노드 id 재사용(계보·코멘트·그룹 보존), 서브프로세스 node_type 보존, 신규 노드만 부분 dagre. `withKeptNodes` 추가. vitest 175·lint 0에러.
```
...
### Task 3: 에디터 배선 — base 전달 + 실제 카운트 확인 모달
...
**Interfaces:**
- Consumes: `buildGraphFromCsv(text, base?)`, `CsvImportOutcome.merge` (Task 2)
- Produces: `CsvImportSection`이 `base?: Graph` prop을 받는다. 이후 Task 6이 이 prop 계약을 그대로 쓴다.

- [ ] **Step 1: `CsvImportSection`에 `base` prop 추가**
...
interface CsvImportSectionProps {
  ...
  disabled?: boolean;
  // 머지 대상 기존 그래프 — 넘기면 제목 일치 노드의 id를 재사용한다. 없으면 전량 신규.
  base?: Graph;
}
```

`handleFile`·`handlePasteText`의 `buildGraphFromCsv(text)` 호출을 `buildGraphFromCsv(text, base)`로 바꾼다. `import type { Graph } from "@/lib/api";` 추가.

요약 블록(`:167-177`)을 머지 카운트로 교체한다:
...
- [ ] **Step 2: i18n 키 추가 (en + ko)**

```ts
// en
"csvImport.mergeSummary": "{added} added · {updated} updated · {removed} removed",
"csvImport.confirmUpdate": "Keep {n} matching nodes (comments, color, assignee, department, groups preserved)",
// ko
"csvImport.mergeSummary": "추가 {added} · 갱신 {updated} · 삭제 {removed}",
"csvImport.confirmUpdate": "일치하는 노드 {n}개 유지 (코멘트·색·담당자·부서·그룹 보존)",
```

`csvImport.confirmDelete`를 코멘트 삭제까지 알리도록 고친다:
```ts
// en
...
- [ ] **Step 3: 에디터가 `base`를 넘기고 확인 모달이 실제 카운트를 쓴다**

`page.tsx`의 `<CsvImportSection ... />`(≈8212)에 `base`를 넘긴다. 임포트는 루트 스코프에서만 열리므로(`onImportCsv` 게이트, ≈6452) 현재 캔버스가 곧 전체 그래프다:
...
확인 모달의 `sections`(≈8253-8275)를 교체한다:
...
`csvImport.confirmCreate`의 EN/KO를 `"Create {x} nodes · {y} connections"` / `"새 노드 {x}개 · 연결 {y}개"`로 다듬고, `lucide-react` import에 `Check`를 더한다(없으면).

⚠️ `nodes.length`/`edges.length`/`groups.length`를 쓰던 자리다. 그 변수들이 이 모달에서만 쓰였다면 미사용 경고가 난다 — lint가 잡아준다.

⚠️ `buildGraph`(`page.tsx:581`)는 **아무 노드도 태그하지 않은 빈 그룹을 버리고**(`:591`) `parent_group_id`를 `null`로 평탄화한다(`:632`). 자동저장이 매번 하는 정규화라 새 손실은 아니지만, base로 넘어가는 `groups`가 DB 원본과 정확히 같지는 않다는 걸 알고 있어야 한다.
...
`PROGRESS.md`:
```markdown
- ② 에디터 배선 — CsvImportSection `base` prop, 요약/확인 모달을 추가·갱신·삭제 실카운트로. 코멘트 삭제 경고 문구 추가. vitest 175·lint 0에러.
```
...
git commit -m "$(cat <<'EOF'
feat(csv-import): editor passes current graph as merge base — 에디터가 현재 그래프를 머지 base로 전달

The confirm dialog claimed every node and group would be deleted because import
really did delete them. It now reports the true added/updated/removed counts and
warns that comments on removed nodes go with them.
...
### Task 4: 비교 회귀 테스트 — 머지 후 실제 변경만 잡히는가
...
describe("computeVersionDiff — CSV 머지 임포트 후", () => {
  it("바뀌지 않은 Start→Review 엣지를 added/removed로 잡지 않는다", () => {
    ...
    expect(diff.rightEdgeStatus.get("y1")).toBeUndefined();
  });

  it("실제로 사라진 엣지만 removed로 잡는다", () => {
    const diff = computeVersionDiff(v1, v2);
    expect([...diff.leftEdgeStatus.keys()]).toEqual(["x2"]); // Review→End 는 B 삽입으로 끊겼다
  });

  it("실제로 생긴 엣지만 added로 잡는다", () => {
    const diff = computeVersionDiff(v1, v2);
    expect([...diff.rightEdgeStatus.keys()].sort()).toEqual(["y2", "y3"]);
  });

  it("system이 바뀐 노드만 changed로 잡는다", () => {
    const diff = computeVersionDiff(v1, v2);
    const changed = diff.entries.filter((entry) => entry.status === "changed");
  ...
  it("신규 노드만 added로 잡는다", () => {
    const diff = computeVersionDiff(v1, v2);
    const added = diff.entries.filter((entry) => entry.status === "added");
    expect(added.map((entry) => entry.title)).toEqual(["Sign contract"]);
  });

  it("삭제 노드는 없다", () => {
    const diff = computeVersionDiff(v1, v2);
    expect(diff.entries.filter((entry) => entry.status === "removed")).toEqual([]);
  });
});
```
...
이 테스트는 **머지 구현(Task 2)이 이미 옳다는 것을 문서화**하는 회귀 방어망이다. 실패한다면 Task 2가 틀렸거나 `diff.ts` 가정이 어긋난 것이니, 원인을 찾기 전에 넘어가지 않는다.
...
```
Expected: `Tests 181 passed`

- [ ] **Step 3: 커밋**

`PROGRESS.md`:
```markdown
- ② 비교 회귀 테스트 `diff.test.ts` 신설 — 클론+머지 시나리오에서 미변경 엣지가 오탐되지 않고 실제 변경만 잡히는지 6케이스. vitest 181.
```
...
### Task 5: 프리뷰 상태 기계 — 캔버스에 머지 결과를 미저장으로 띄운다

**Files:**
- Modify: `frontend/src/app/maps/[mapId]/page.tsx`

**Interfaces:**
- Consumes: `withKeptNodes`, `CsvImportOutcome.merge` (Task 2)
- Produces: page.tsx 내부 상태 — `previewSource: "ai" | "csv" | null`, `csvKeepRemoved: boolean`, `enterCsvPreview()`, `applyCsvImport()`, `cancelCsvPreview()`. Task 6이 이것들을 Import 탭에 연결한다.

- [ ] **Step 1: `aiPreviewRef`/`aiPreviewActive`를 소스 태그로 일반화**

`page.tsx:820-821`을 교체한다:
...
`aiPreviewRef.current`를 쓰는 두 곳(`:1332` `saveCurrentScope`, `:1363` `scheduleAutoSave`)을 `previewRef.current`로 바꾼다. 주석의 "AI 미리보기"를 "미리보기"로 고친다.

`applyAiProposal`(`:1596-1601`)·`applyAiOps`(`:1725-1729`)의 두 줄:
```tsx
...
`AiChatPanel`(`:7283`)의 prop은 이름을 유지한 채 소스로 좁힌다 — CSV 프리뷰 중에 AI 패널이 Apply 바를 띄우면 안 된다:
```tsx
...
`csvConfirmOpen` state와 `ConfirmDialog`(`:8245-8279`)를 제거하고, 대신 프리뷰 상태를 둔다. `page.tsx:785` 근처:
...
`applyCsvImport`(`:1480`) 위에 프리뷰 진입 함수를 추가한다:
...
```tsx
  ...
  const enterCsvPreview = useCallback(() => {
    ...
    // 캔버스 그래프 = 머지 결과 + 소멸 노드 + 소멸 엣지 (하이라이트용)
    const canvasGraph = withKeptNodes(graph, outcome.merge.removedNodes);
    ...
    const previewNodes = toAppNodes(canvasGraph, null).map((node) => ({
      ...
      data: {
        ...node.data,
        diffStatus: removedIds.has(node.id) ? ("removed" as const) : added.has(node.id) ? ("added" as const) : undefined,
      },
    }));
    const previewEdges = [
...
⚠️ **신규 노드 부분 자동정렬을 여기서 다시 하지 않는다.** `buildGraphFromCsv`의 `layoutAddedOnly`(Task 2)가 이미 씨앗 배치 + `layoutSubsetWithDagre`를 마쳤고, `outcome.graph`의 좌표가 최종값이다. 여기서 또 돌리면 앵커가 어긋난다. (그리고 `applyAutoLayout`은 내부 `pushHistory()` 때문에 undo가 2단이 되어 어느 경우에도 쓰면 안 된다.)

⚠️ `toAppNodes(graph, null)`의 두 번째 인자는 scope id다 — `applyCsvImport`(`:1485`)와 같은 호출 형태를 따른다.

- [ ] **Step 3: Apply / Cancel**

`applyCsvImport`(`:1480-1503`)를 프리뷰 커밋으로 교체한다. 서버 PUT 성공 후에만 캔버스를 확정한다(423/409 시 프리뷰 유지).
...
```tsx
  ...
  const applyCsvImport = useCallback(async () => {
    ...
    try {
      ...
      // 프리뷰를 유지한 채 실패만 알린다 — 다시 Apply 하거나 Cancel 할 수 있다
...
`csvConfirmOpen` state·`setCsvConfirmOpen` 호출·`ConfirmDialog` 블록·이제 미사용인 `FileUp`/`Trash2`/`FilePlus2` import를 제거한다(`FileUp`은 툴바에서도 쓰이면 남긴다 — lint가 알려준다).
...
npm run lint && npm test && npm run build
```
Expected: lint 0 errors (특히 `react-hooks/preserve-manual-memoization` 무발생) · `Tests 181 passed` · build 성공.

⚠️ 이 시점에서 Apply/Cancel 버튼은 아직 화면에 없다. Task 6까지는 프리뷰에 들어가면 나올 방법이 없으므로, **Task 5와 6은 연속으로 수행하고 그 사이에 앱을 시연하지 않는다.**

- [ ] **Step 5: 커밋**

`PROGRESS.md`:
```markdown
- ③ 프리뷰 상태 기계 일반화(`aiPreviewRef`→`previewRef` + `previewSource`) + CSV 머지 프리뷰 진입/확정/취소. 소멸 노드·엣지 `diffStatus`/빨간 점선. 확인 모달 폐지. vitest 181·lint 0에러. ⚠️ Apply/Cancel UI는 다음 커밋(Import 탭).
```
...
### Task 6: 인스펙터 Import 탭 — 삭제/유지 선택 + 호버 안내
...
**Interfaces:**
- Consumes: `previewSource`, `csvKeepRemoved`, `setCsvKeepRemoved`, `applyCsvImport`, `cancelCsvPreview`, `csvOutcome`, `highlightNode` (Task 5 · page.tsx)
- Produces: `CsvImportTab` 컴포넌트 · `InspectorPanel`의 `importSlot`/`forcedTab`/`lockTabs` prop
...
"csvImport.tabIntro": "**{updated} nodes matched** and keep their comments, color, assignee, department and groups.\n\n**{added} nodes added** from the CSV.",
...
"csvImport.tabIntro": "**노드 {updated}개가 일치**해 코멘트·색·담당자·부서·그룹을 유지합니다.\n\n**노드 {added}개를 CSV에서 추가**합니다.",
...
⚠️ `tabIntro`는 `MarkdownView`가 파싱한다. 노드 **제목은 절대 여기 넣지 않는다** — `**`·`#`·`[](...)`가 서식으로 해석되고 클릭 핸들러도 못 단다. 제목은 Step 2의 React 리스트가 렌더한다.
...
// 인스펙터 Import 탭 — CSV 머지 프리뷰의 요약·소멸 노드 목록·삭제/유지 선택·Apply/Cancel.
...
import type { CsvMergeInfo } from "@/lib/csv-import";
import { useI18n } from "@/lib/i18n";
...
export function CsvImportTab({
  merge, keepRemoved, onKeepRemovedChange, onFocusNode, onApply, onCancel,
}: CsvImportTabProps) {
  const { t } = useI18n();
  const removedCount = merge.removedNodes.length;
  const lostCount = merge.lostEdges.length;

  const modeButton = (mode: "delete" | "keep", label: string, tip: React.ReactNode) => {
      ...
      {removedCount === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          <ul className="scroll-soft flex max-h-40 flex-col gap-1">
            {merge.removedNodes.map((node: GraphNode) => (
              <li key={node.id}>
                <button
                  type="button"
          ...
          <div className="flex gap-1.5">
            {modeButton("delete", t("csvImport.modeDelete"), (
              <TipCard
                head={t("csvImport.modeDeleteTipHead", { n: removedCount, m: lostCount })}
                body={t("csvImport.modeDeleteTipBody")}
              />
            ))}
            {modeButton("keep", t("csvImport.modeKeep"), (
              <TipCard
                head={t("csvImport.modeKeepTipHead", { n: removedCount, m: lostCount })}
                body={t("csvImport.modeKeepTipBody")}
              />
            ))}
          </div>
        </div>
...
사용한 토큰·유틸은 전부 실재함을 확인했다 — `--color-removed`(`globals.css:51`), `--color-on-accent`(`:16`), `--color-accent-focus`(`:14`), `.scroll-soft`(`:117`), `.md`(`:366`), `text-caption-strong`(48곳 사용). `border-removed`는 `compare/page.tsx:331`이 이미 쓴다. **raw hex 금지** — 새 색이 필요하면 토큰을 먼저 정의한다.
...
const IMPORT_TAB = { key: "import" as const, labelKey: "csvImport.tabTitle" as MessageKey, icon: FileUp };
```
...
접기 버튼(`:94-102`)에 `disabled={lockTabs}` 추가. 탭 버튼 map을 `tabs.map(...)`으로 바꾸고 `onClick={() => setInternalTab(key)}`, `disabled={lockTabs && key !== tab}` 추가. `disabled:opacity-40 disabled:hover:bg-transparent`를 className에 더한다.
...
프리뷰 중 인스펙터가 접혀 있으면 강제로 펼친다 — `enterCsvPreview` 안에 `setRightCollapsed(false)`(실제 state 이름은 `leftCollapsed`의 우측 대응을 확인해 쓴다)를 추가한다. **이펙트 안 setState 금지**이므로 반드시 `enterCsvPreview` 핸들러 안에서 부른다.
...
npm run lint && npm test && npm run build
```
Expected: lint 0 errors · `Tests 181 passed` · build 성공.

- [ ] **Step 6: 커밋**

`PROGRESS.md`:
```markdown
- ③ 인스펙터 Import 탭(`forcedTab`/`lockTabs`, 프리뷰 중 다른 탭·접기 잠금) — MarkdownView 요약 + 소멸 노드 React 리스트(클릭→캔버스 포커스) + 삭제/유지 세그먼트 + Apply/Cancel, 버튼별 리치 툴팁. vitest 181·lint 0에러.
```
...
백엔드·프론트는 **사용자 터미널에서** 띄운다 (백그라운드 서버는 턴 경계에서 SIGTERM으로 회수된다). 사용자에게 다음을 요청한다:
...
4. 툴바 **Import CSV** → 3의 노드 중 하나와 **제목이 같은 행**을 포함한 CSV를 붙여넣는다. 요약이 `추가 N · 갱신 M · 삭제 K`로 뜨는지 확인.
5. **Continue** → 캔버스가 병합 결과로 바뀌고, 신규 노드는 초록 테두리, CSV에 없는 노드와 사라질 엣지는 **빨간 점선**인지 확인. 신규 노드가 겹치지 않게 정렬돼 있는지 확인.
6. 인스펙터가 **Import 탭으로 고정**되고 다른 탭·접기가 **눌리지 않는지** 확인. 각 버튼에 호버해 툴팁 2줄이 **잘리지 않고** 뜨는지 확인.
7. 소멸 노드 이름 클릭 → 캔버스가 그 노드로 이동하는지 확인.
8. **Keep** 선택 → **Apply**. 유지한 노드가 엣지 없이 남았는지, 갱신된 노드의 **색·담당자·코멘트가 살아 있는지** 확인.
9. 비교 화면 진입 → **바뀐 노드와 실제로 재배선된 엣지만** 잡히는지 확인. 이전에는 전 엣지가 잡혔다.
10. 다시 임포트 → **Cancel** → 캔버스가 원래대로 복귀하고 자동저장이 다시 도는지 확인.
...
`PROGRESS.md`:
```markdown
- ③ 브라우저 실검증 10케이스 — 새맵 준비 UI·에디터 자동 이동·머지 하이라이트·탭 잠금·툴팁·포커스·Keep/Apply 보존·비교 정상화·Cancel 복귀.
```
...
- `npm run lint` 0 errors · `npm test` 181 passed · `npm run build` 성공
- 백엔드 회귀 1회: `cd backend && .venv/bin/python -m pytest tests/ -q` (변경 없으므로 기존과 동일해야 한다)
- Task 7의 10개 시나리오를 실제로 실행하고 관찰 결과를 기록
