...
- 후속 2(사용자 제안): **도트 그리드 제거 + 모드 링** — 차콜 L5는 무늬 없는 민 무대(Background 미렌더, `--color-canvas-l5-dot` 토큰 폐기)로 하고, 프레임 테두리를 `border-2 border-accent/70` 액센트 링으로 — 흰 크롬↔차콜 경계를 색이 중재해 어색함 완화(화면공유 모드 링 관례). 라이트 토글은 도트·풀블리드 그대로. 검증 22/22(민 무대·링 2px solid·라이트 무링 — Tailwind preflight가 border-style을 solid로 깔아 폭 0으로 판정).
