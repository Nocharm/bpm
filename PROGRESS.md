...
- **자연어 피드백 엔드포인트(dev, 09-23, B4/B5/B9):** 캔버스·행을 고치려면 손으로 그리거나 설문을 다시 받아야 했던 것을 `POST /framework-interviews/{id}/feedback`(`{scope: relations|task, task_pk?, message}`) 한 곳으로 — relations는 현재 캔버스+카드 목록을 실어 같은 형태를 받아 `normalize_canvas`(id·task_id 불변, 새 노드는 `__branch__` 분기만, 빠뜨린 subprocess/start/end는 직전 캔버스에서 보충, 미지 끝점 엣지 폐기) → `validate_canvas` → `relayout_canvas`(모델은 좌표를 0으로 보내는 계약이라 LR 배치만 다시)로 저장하고 relations는 건드리지 않는다(확정 시 역산). task는 현재 행을 실어 `RowOut`을 다시 받아 `validate_row` 오류면 422, 통과면 행·이슈·drawn_at 갱신 + 세션이 `ready`였으면 `linking`으로 되돌려 조립본 폐기. 두 경로 모두 `feedback_log`에 최근 10건만 남긴다. 계약 2종은 레지스트리 키(`l5_canvas_feedback_contract`·`l6_row_feedback_contract`)로 오버라이드 가능(PROMPT_KEYS 15개).
