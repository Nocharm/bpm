...
## 2026-09-19 — 업무 체계 뷰 2열(형제|하위) + 탐색 모달(계단식·ERD식 다이어그램) (feat/fw-drill-columns)

- 드릴다운 좌측을 **형제(1) : 하위(2)** 두 열로 — 왼쪽은 현재 카테고리의 형제(직전 레벨 목록, 현재 강조·최근 연 항목 점, 클릭=오른쪽만 교체), 오른쪽은 하위(행=드릴인, L5=폭 275px 컴팩트 카드: 상태 점 title 영어 라벨·관리자·맵 수·열기 아이콘). 형제 칩 스트립(MRU)은 제거(왼쪽 열이 대체), 좌:우 컬럼 비율은 기존 1:2 유지. 루트=왼쪽 L1 목록+오른쪽 안내. 목업 v3→v4 두 라운드로 확정(사용자 피드백: 비율 원복·방사형 대신 ERD식 직각 엣지·상위는 L1까지 위 가운데 다른 색·우클릭 메뉴).
- **탐색 모달**(`components/maps/framework-explorer-modal.tsx`, 브레드크럼 우측 트리 아이콘): [계단식|다이어그램] 세그먼트·검색(`/categories/search`)·모드 영속. 계단식=lazy 트리 엔진 재사용, 현재 경로 미리 펼침·강조, 행 클릭=이동+닫힘, 자손 호버 시 조상 강조(globals.css `data-tree-*` 규칙 공용). 다이어그램=`lib/framework-diagram.ts` 순수 레이아웃(상위 체인 L1까지 위 가운데 차콜 스택 → 현재 액센트 → 자식 좌/우 번갈아 → 손자 계단 스택, 수평→수직→수평 직각 버스라 엣지 교차 없음, 단위 테스트 3종). 좌클릭=재중심(박스 transform 전환 350ms·새 박스는 중심에서 등장), 상위 박스=한 단계 위, **우클릭=GoToMenu 3항목(정보 보기·이 업무체계 보기·이 업무 중심으로 그리기)**, 정보 카드는 요약 API(관리자·서브트리 수). "전체(L1 중심)"=L2 좌우·L3 스택. 폭 560↔1000 스프링 전환. L5 좌클릭=정보 카드. **플로팅 패널**(사용자 지시): 배경 딤 없이 페이지 위에 뜨고(컨테이너 pointer-events-none) 헤더 드래그로 이동, 바깥 클릭엔 안 닫히고 ×/Esc만, 이동해도 열린 채 현재 위치를 따라감(체인은 `{forId, nodes}`로 묶어 centerId 변경 직후 옛 체인으로 중심을 맞추는 경합 차단).
- 검색·레벨 색(사용자 피드백 3차): 계단식 폭 440. 검색은 `GET /categories/all`(경량 전 행 2,300개, `CategoryLiteOut`)을 한 번 받아 클라이언트 `lib/search` `filterByQuery`로 — 부분일치·초성·**초성 섞인 비연속 시퀀스**(`subsequenceMatch`가 글자별 원문/초성 비교, 공백 무시: "ㅂㅈㅈㅈ"→"배지 조제", "배조계"→"배지 조제 계획") + `Highlight` 하이라이트, 상한 60. 결과 행은 레벨 필 + 이름 + L5 수만 — 필을 호버하면 조상 경로(L1 › … › 부모) 툴팁, 경로 문자열은 제거(조상마다 숫자 칩을 늘어놓은 1차안은 "밀도만 높다"는 피드백으로 폐기). **레벨 필 색 사다리** 공용 `components/level-pill.tsx`(L1 100%→L2 75→L3 55→L4 30→L5 12% 액센트, `LEVEL_FILL_OPACITY`로 SVG 태그도 동일) — 드릴 헤더·요약 카드·대시보드·탐색 모달 전부 교체. 헤더는 두 줄(제목·모드·닫기 / 검색) — 한 줄이면 440에서 검색창이 밀려 사라진다.
- 형제 열 항목 2행 = **직속 관리자**(없으면 점선 "Unassigned" 플레이스홀더), L5 수는 제거(사용자 지시 2026-09-19). 하위 행(L2~L4)도 같은 문법 — 앞 아이콘→레벨 필, 2줄 관리자, 우측은 서브트리 L5 수 대신 **직계 하위 수 "L{n+1} k"** · 맵 수. 미지정 라벨 "Unassigned"는 두 열 모두 행 호버 시에만(점선 점은 상시, `group-hover` opacity) — 같은 문구 반복이 밀도를 높였다. BE `/categories/nodes`의 `admin`을 전 레벨로 확장(`_card_meta`, `canvas_state`·`slot_pending_count`는 L5만) — FE/BE 동시 배포.
- 함정: 모달 백드롭을 z1300으로 두면 GoToMenu(z1200 포털)가 아래 깔려 클릭이 svg에 막힌다 → 모달은 z1200(사다리). 메뉴 열린 채 Escape는 GoToMenu·백드롭이 같은 window keydown을 받아 둘 다 닫히므로 백드롭 onClose에서 메뉴 열림 중엔 무시. Playwright는 CSS transform이 걸린 svg `<g>`를 locator.click으로 못 맞춘다 → boundingBox 중심 `page.mouse.click`. 체인 응답 노드는 카운트 0 고정이라 자식 캐시에서 같은 id로 되살리고, 못 살리면 건수를 숨긴다.
- 검증: `pw-smoke-framework.mjs` 드릴다운 구간을 2열+탐색 모달 검사로 갱신(계단식 펼침·폭 전환·상위 체인 3개·우클릭 3항목·닫힘), vitest 1011, tsc(기존 `.next/dev/types` 오탐 제외)·eslint 클린, 규모 시드(L5 2,000)에서 L3 중심 손자 56개 배치·전체 보기 확인.
