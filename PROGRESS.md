...
- 검색·레벨 색(사용자 피드백 3차): 계단식 폭 440. 검색은 `GET /categories/all`(경량 전 행 2,300개, `CategoryLiteOut`)을 한 번 받아 클라이언트 `lib/search` `filterByQuery`로 — 부분일치·초성·**초성 섞인 비연속 시퀀스**(`subsequenceMatch`가 글자별 원문/초성 비교, 공백 무시: "ㅂㅈㅈㅈ"→"배지 조제", "배조계"→"배지 조제 계획") + `Highlight` 하이라이트, 상한 60. 결과 행 왼쪽에 조상 레벨 마커(색 사다리 칩, 호버 시 그 조상 이름·L5 수 툴팁), 경로 문자열은 제거. **레벨 필 색 사다리** 공용 `components/level-pill.tsx`(L1 100%→L2 75→L3 55→L4 30→L5 12% 액센트, `LEVEL_FILL_OPACITY`로 SVG 태그도 동일) — 드릴 헤더·요약 카드·대시보드·탐색 모달 전부 교체. 헤더는 두 줄(제목·모드·닫기 / 검색) — 한 줄이면 440에서 검색창이 밀려 사라진다.
