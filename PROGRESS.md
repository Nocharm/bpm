...
- 설계 `docs/superpowers/specs/2026-09-03-import-fallback-followups-design.md`(사용자 확정 답변 반영). ④ SP 지정 참고치 `sp_annual_count/sp_fte`(모델·지정 PUT·SubprocessRef·라이브러리·임포트 착지, 노드 인스펙터 Info 호버) ② 인터뷰 원문 메모 5종 — `PATCH /maps/{id}/fallback-notes`(editor), `MapFallbackNotes`(에디터 맵 탭=점유권자 편집·홈 카드·비교 요약 기본 접힘), FallbackHint 빈 값 "추가" 어포던스 ③ 노트 CRUD API(맵=오너, L5=체인 관리자/sysadmin, `edited_at`) ⑥ SP 카드 액션 행 wrap.
