...
- **기존 L5 학습·정정(dev, 09-22):** 캠페인이 L5 아래 이미 있는 L6를 무시하던 문제 — `app/framework_interview/existing.py`(맵 그래프 → 인터뷰 행 역변환·스냅샷 로더·계획 카드 병합). 엣지 kind는 저장되지 않아 구조(되돌아감=loop, 판단 노드 출발=branch)로 복원하고, 임포터가 맵 컬럼에 적는 ownerRole·시작/완료 조건은 스냅샷 단계에서 되살린다. system은 카탈로그 정규화(Other) 때문에 왕복하지 않는다. 세션 생성 시 `FrameworkInterviewSession.existing`에 스냅샷 저장(`FrameworkInterviewOut.existing`로 노출, `row` 미노출) + 계획 카드/태스크 `mode`(new|keep|revise) 컬럼 배선. 계획 UI에서의 실제 병합·잠금 배선은 후속 태스크.
