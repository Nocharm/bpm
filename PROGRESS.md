...
- Task 4(스모크+게이트): `pw-smoke-member-card.mjs` 신규(admin.sys 소유 테스트맵 자동 생성+협업자·그룹 부여+한글이름 임포트 → Me 뱃지·en/ko 이름줄·펼침 alt 필·그룹명 해석) 10/10. `pw-smoke-korean-names.mjs` 17/17, `pw-smoke-korean-dept.mjs` 5/5 회귀 통과. 최종 게이트: pytest 488·ruff 0 / vitest 137·lint 0(무관 파일 warning 1)·build 성공. 서버 기동 중 발견: 메인 dev.db엔 admin.sys가 소유·멤버인 맵이 없어(Me 뱃지 전제 불충족) 스모크가 테스트맵을 직접 생성하도록 설계 — 제품 결함 아님, 데모 시드 특성.
...
## 2026-07-06 — 검색 우선 노출 로직 개선 (사용자: 피커 이름 유사 시 랭킹, A·B·C+캡삭제+부서핀 선택)
- `lib/search.ts` 랭킹 v2 — 랭크 세분화(정확>접두>**단어시작**>중간>초성 접두>초성·로마자 중간>시퀀스) + 동순위 타이브레이크(필드 순서=이름 우선→매치 위치→짧은 필드→입력 순서) + **공백도 AND**(콤마와 동일). 전 소비처(홈·공지·인박스·피커 2종) 공통 적용.
- principal-picker: 검색 8개 캡 삭제 → 전량을 25청크 증분 렌더로. 부서/그룹 매치 시 최고 랭크 1개를 스코어 무시하고 최상단 핀(유저 무더기에 묻힘 방지).
...
- home: 검색어·필터 sessionStorage 유지(뒤로가기/목록복귀 시 복원)
- home 개선 배치: 새로고침·브랜드로고 클릭 시 검색·필터 초기화(맵→복귀만 유지, reload 감지+로고 stash clear+full reload) · 빈 목록영역 클릭=선택해제·자동 첫선택 제거·상세 플레이스홀더 · "/" 검색 포커스+kbd 힌트+플레이스홀더 축약 · 최근 배지 플로팅(상단경계 안쪽·축소) · 최근밴드 초기 2/+3/접기=섹션라벨 우측 쉐브론(전부 닫힘, 라벨 행 클릭=2개부터 재펼침, 라벨 행 hover 효과)+펼친 뒤 하단에도 동일 쉐브론 접기 + "전체 맵" 섹션 라벨 · 캐시 cap 11.
- home 후속: 선택해제 영역을 페이지 전체 여백으로 확대(루트 컨테이너 onClick, 상세패널 stopPropagation) · 최근 배지 폰트 10→11px · 배지 배경/테두리 제거(텍스트만)·좌상단 플로팅(top-1 left-2) 위치 튜닝.
...
- **핸들 톤 후속** — violet→**무채색**(ink-tertiary 20% 채움·50% 테두리), 13px→**11px**(조금 작게). 호버 효과에서 `scale(1.2)` 제거 → **색상 진하게(ink-tertiary 42%)+테두리 강조(ink-secondary)만**(크기 불변).
- **노드 박스 호버 = 리프트→색상 링 강조**(`process-node.tsx`·`page.tsx`) — 리프트(translate+scale+shadow) 전부 제거하고, 노드 **자기 색상**(`--nc` CSS 변수)으로 3px 반투명 링(42%)을 hover 시 표시(`.bpm-node-emph`). 노드마다 색이 달라도 **기존 톤 유지·이동/크기 변화 없이** 강조. process/subprocess/decision 3형태 공용. `--nc`는 @types/react가 커스텀 속성 미지원이라 `nodeStyle` 헬퍼로 캐스팅.
...
- home: MapCard recentOpenedAt 배지(accent pill, 상대시각)
...
- **d5 최소화 스파클** (`page.tsx`) — 최소화 시 고정 dock → **정사각 스파클 아이콘**(44px accent), 화면 어디든 **드래그**(bounds 클램프), 제자리 클릭 시 창 복원. AI 창을 WindowDock에서 분리.
- 검증: 브라우저 — 카드 렌더·인채팅 제안 카드·최소화→스파클→클릭 복원 확인. lint 0 errors·build OK.
