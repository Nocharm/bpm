...
- 후속 3(사용자 지시): **프레임 제거 + 흰 모드 링** — 거터/라운드 프레임을 걷고 차콜을 풀블리드로 복귀, 신호는 캔버스 안쪽 6px에 뷰포트 고정 `border-surface/60` 2px 흰 링 오버레이(`l5-mode-ring`, 화면공유 관례)가 담당. 칩 20px 인셋은 링 안쪽 배치로 유지, ScopePreview는 민 차콜로 단순화. 검증 22/22.
