...
- Phase B+C(FE) 완료 — PARAM_FIELDS 7필드(원장 단일 소스: 일괄편집·캔버스 칩·Σ(duration 미러)·Excel 1안/2안 컬럼·CSV 20컬럼 왕복·AI 변환 3경로) · NodeData/toAppNodes/buildGraph 스레딩(소거 방지) · SP 상속 5필드+IO/조건/data_form read-only(3표면 강제, 폴백은 CSV/AI 제외·병합 무조건 보존) · 인스펙터 Details 카드(IO 리스트 add/remove·형식 배지·조건) · 설정 ProcessFieldsCard(gmp 셀렉트+PATCH process-fields) · FallbackHint 팝오버(원문·수정·적용, 6표면). 게이트 vitest 659·tsc 0·build OK.
- GMP 배지 보더 추가(사용자 요청) — `getGmpBadgeStyle` 단일 소스에 45% 틴트 1px 보더 → 캔버스 필·홈 카드·설정·안내 모달 배지 일괄 적용.
- GMP 3차 다듬기(사용자 지시) — 필을 노드 안쪽으로(마름모=상단 중앙)·**일반 노드는 분류가 노드 색 자동 확정**(GMP_NODE_COLORS, 미분류는 색 불변)·안내 모달 가시성 재설계(분류 배지·색 스와치 before→after, X=마우스 지점) + 2단 되돌리기("이전 분류로"=분류+색, "색만 되돌리기"=분류 유지). 그룹 일괄 편집 검수(사용자 지목) — MODE_META·canBulkEditField·buildBulkAttrPatch 전부 PARAM_FIELDS 원장 파생이라 touch_time 자동 적용 확인(bulk-params 단위 3케이스+실브라우저 5체크: 필 내부 판정·색 자동/2단 복원·일괄 적용 API 대조).
- 활동별 GMP 캔버스 확장(사용자 확정 — 처음엔 맵 전용이었으나 노드 확장 유지 결정) — `nodes.gmp` 신설(3값, 무효 "" 소거)·맵 탭 표시 토글 "GMP"·노드 라벨 왼쪽 위 필 태그(미분류=아이콘만)·편집 모드 필 클릭=클릭 지점 분류 피커·SP 노드는 링크 맵 sp_gmp read-only 상속 필. **재임포트 승계**(엔진이 계보로 이어받기·시그니처 제외 — 검토값 보존, 테스트 포함). CSV/AI 표면 제외(병합 보존)·diff/비교 포함(gmp 라벨 표시). 랜드마인: NodeFields 토글 필터에 비텍스트 토글(gmp) 누락 시 아이콘 미정의 크래시(실측)·setState만 부르는 useCallback은 컴파일러 preserve-manual-memoization 에러 → 평 함수. 실브라우저 6체크 통과.
- 디자인 리뷰 2차(사용자 지시 4건) — ① 조건 입력 말줄임 유지 ② Data form을 IO 그룹 종속 행으로 재배치(공용 `NodeDetailsFields` 추출, 헤더 배지·동등 행 제거) ③ GMP 분류별 배지 색 자동 지정(신호등: removed/changed/added 토큰 12% 틴트) + 변경 시 마우스 지점(X 버튼 기준) 안내 팝오버·이전 값 되돌리기 ④ 편집 모달에 Details 섹션 추가(승격 필드 전부 버퍼 편집, 파라미터 7종은 기존 노출 확인). 항목별 dataForm은 전달물이 액션당 1값이라 임포트 무변경 — 항목별 분리는 PwC 협의 확장으로 설계 문서화.
- 디자인 리뷰 패스(원격 스크린샷 검수)에서 실버그 4건 픽스 — ① ParamInput 표시/정규화가 duration 하드코딩이라 touch_time이 45m 대신 0.45로 노출 ② 캔버스 파라미터 칩 values 열거에 touch_time 누락(일반·SP 양쪽) ③ SP 노드 Details가 미주입 data.sp*를 읽어 상속값이 — 로 표시 → selectedSpRef 소스로 교정 ④ 버전 비교 diff/compare 필드 목록에 승격 필드 6종 누락. touch_time 칩 아이콘은 duration(시계)과 구분되게 스톱워치(Timer)로 분리.
- Phase V(QA 주도 실검증) 완료 — `docs/qa/2026-08-20-field-promotion-qa.md` 29항목 전부 ✅: 신규 스모크 `pw-smoke-field-promotion.mjs` 25/25(임포트 착지·Details 편집·touch_time 정규화·FallbackHint 수정/적용·gmp 선정 보존(Unchanged)·폴백 재전달 우선(Updated)) + 회귀 3종(interview-import 15/15 단언 갱신·framework-admin 7/7·framework 25/25). 문서 스윕: 9910 §8 백필 절차(FE/BE 동시 배포 필수)·편집 매뉴얼 EN/KO(7파라미터·CSV 20열)·관리자 매뉴얼 EN/KO(승격·GMP 검토 카드).
