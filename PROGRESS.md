...
- 사이드바 단축키 카드 다듬기 + 아웃라인 선택 자동 스크롤 — 안내에 `Tab`/`⇧Tab` 별칭 병기(`↓`/`↑` 행), 카드 접기/펼치기(노드인포와 동일), 두 카드 접힘 상태 `sessionStorage` 영속(새로고침 유지, i18n 복원 패턴으로 hydration 회피), 둘 다 접히면 `contents`↔`flex`로 좌우 분할 표시. 선택 노드 변경 시 아웃라인 행 `scrollIntoView({block:"nearest"})`로 스크롤 영역 밖 이탈 방지(키보드 이동·캔버스 선택 모두). 프론트 tsc/eslint green(런타임 미실행).
