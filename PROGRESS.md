...
- **다크 셸 확장**: 차콜 캔버스만 어둡고 크롬이 흰색이라 "깨진 다크모드"처럼 어색하다는 피드백 → 에디터 루트에 `.bpm-l5-dark`(globals.css) 토큰 15종 플립으로 **크롬(헤더·툴바·아웃라인·인스펙터)까지 동조**. 글로벌 네비·body 포털(모달·드롭다운)은 라이트 유지(다크 무대 위 스포트라이트), 라이트 전용 룰(§7)에 L5 한정 예외 명시. 캔버스 콘텐츠(노드·엣지 라벨 = `.react-flow__viewport` 서브트리)는 라이트 토큰 재정의로 제외 — 노드는 파스텔 fill 위 잉크 텍스트의 "데이터" 렌더라 흰 글자가 되면 워시드. ⚠️ 이 재정의는 **런타임 클래스 대상이라 Turbopack이 globals.css에서 purge** → page.tsx raw `<style>` 주입에 배치(bpm-expand-anim과 동일 함정 재확인). 검증 18/18(크롬 다크/복귀·노드 잉크 라이트 유지 추가) + 스크린샷 픽셀 실측(캔버스 #363843·크롬 #2B2C35·노드 fill 파스텔). 이 작업 일체는 dev가 아닌 **feature/l5-dark-shell** 브랜치(cc89929f 이관, dev는 ea4ee464로 복귀).
