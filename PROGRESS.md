...
- **후속(같은 날, 2차 재구성):** 1줄 = 공개 범위 탭(세그먼트 복귀) · 정렬 · 해제 FilterX 아이콘(우측 끝, 활성 시 error 톤), 2줄 = Status · Role · (부서 뷰) Issues · **Type**(SP/Non-SP·L5 canvases·In/Not in the framework를 한 그룹 OR로 통합, 세션 키 `kind`). 필은 내용 폭 비례 차등(`flex-auto`)으로 행을 채우고 내용 좌측·쉐브론 우측. Status/Role/Type/Issues 필 라벨은 UI 언어 무관 영어 고정(Issues 옵션은 UI 언어, 옵션 아이콘 유지), 공개 범위 탭 앞 Layers/Globe/Lock 아이콘. 반응형 단계는 2줄 필 기준. 스모크는 클램프 검사를 미지정 목록에 고정(선택 맵의 부서 박스가 앞에 오면 "첫 클램프" 가정이 깨지던 데이터 의존 — 종전 실행은 중간에 멈춰 41개 중 26개만 돌았음). 목록 `load_my_roles`가 캔버스에도 체인 관리자 파생(editor/viewer)을 적용해 목록=상세 역할 불일치 해소(캔버스 있을 때만 `get_admin_scope`+역조회 각 1회).
