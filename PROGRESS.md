...
- **Merge review fix wave(3건)**: `import_consultant.py` `upsert_categories`가 전달분 code만 갱신해 UI 생성 카테고리(`ui-*`)의 level이 조상 depth 변경에 낡은 채 남던 버그 — 업서트 후 테이블 전체를 루트부터 BFS 재계산(사이클 가드 포함)하도록 수정. `framework-panel.tsx` `refreshTree`가 접힌 가지의 stale `childrenByParent` 캐시를 영구 보존하던 버그 — 재조회한 루트+펼친 id만으로 map 재구성. `categories.py`의 BFS 서브트리 순회 2곳(update_category)+`build_category_paths` 재귀에 부모 사이클 가드 추가(무한루프/RecursionError 방지). 게이트 BE pytest 997(+2)·ruff 0 / FE tsc 0·lint 0(무관 사전 경고 1건)·vitest 599/599·build OK.
- dev↔main 체크리스트 쉬운 판 재작성(n8n 연동 완료 전제·화면 경로→행동→통과 기준 형식·웹 임포트 경로 반영) + 9910 가이드 §8 웹 임포트 우선 전환 + 설계 §6 관리자 탭 개정 각주.
- **Framework Admin UI Task 4 리뷰 픽스**: ① 파일 재선택 레이스(dry-run 진행 중 파일 재선택 → 무효화된 옛 응답이 늦게 도착해 importResult를 되살려 Apply가 검증 안 된 선택에 대해 활성화되던 문제) — importBusy 중 파일 입력·트리거 버튼 둘 다 disabled로 선택 자체를 불변으로 고정(요청 토큰 없이 레이스 차단) ② warnings 칩이 rows(500행 캡)에서 세어 대형 전달분에서 undercount — `categories.py` import 엔드포인트가 `report.counts()`(CLI 전용, warning 집계 제외) 뒤에 `summary["warning"]`을 caps 이전 report.rows 전체 기준으로 별도 채우도록(엔드포인트 로컬, `ImportReport.counts()` 자체는 무변경), FE는 `summary.warning ?? 0`으로 단순화. 기존 dry-run/apply 정확 dict 단언 2건에 `"warning": 0` 반영, 신규 warning-존재 케이스 테스트 1건 추가. 부수: 파일 input onChange 후 `value=""`로 동일 파일 재선택도 재발화. 게이트 BE pytest 995(+1)·ruff 0 / FE tsc 0·lint 0(무관 경고 1건)·vitest 599/599·build OK.
- **Framework Admin UI Task 4**: Framework 탭 트리 하단에 웹 대량 임포트 섹션 — 파일 2개(categories.json/maps.jsonl) 클라이언트 파싱(신규 순수 유틸 `framework-import-parse.ts`, JSONL·JSON 배열·깨진 줄 수집·빈 파일 케이스 vitest 10건 TDD) → Dry-run(`importFramework({apply:false})`, 요약 칩 5종+행 테이블) → confirm-dialog → Apply(성공 시 `refreshTree()`로 트리 갱신). `api.ts`의 `FrameworkImportResult.summary` 타입이 실제 백엔드 계약(`dict[str,int]` sparse, 키는 단수 `error`뿐이고 `warnings`는 아예 없음 — `ImportReport.counts()`가 warning을 집계 제외)과 어긋나 있던 걸 발견해 `Record<string, number>`로 정정(FE 소비처가 이번 태스크가 처음이라 파급 없음), warnings는 rows에서 action==="warning" 직접 카운트. 게이트 FE tsc 0·lint 0(무관 사전 경고 1건)·vitest 599/599(589+10)·build OK.
...
- **Framework Admin UI Task 3 리뷰 픽스**: 캐스케이드 셀렉트 재선택 버그(`framework-panel.tsx` MoveCategoryModal + 원본 `framework-assign-modal.tsx`) — `fetchedParentIds`가 `optionsByDepth` 잘림과 동기화 안 돼 A→D→A 재선택 시 A의 하위 레벨이 영영 안 돌아오던 문제, `pickAt`에서 truncate와 함께 리셋하도록 수정. CountTag는 접힌 행에만(펼치면 롤업 숫자 중복). 미사용 i18n 키(`framework.adminImportPlaceholder`) 제거. 게이트 FE tsc 0·lint 0(무관 경고 1건)·vitest 589/589·build OK.
