...
## 2026-09-07 — 인터뷰 JSON 0.5 외부 L6 참조(`externalTasks`) 설계 확정 (feat/interview-external-refs, dev 분기·머지 보류)
- **배경**: 컨설턴트가 L5 단위 전달 시 타 L5의 L6를 "소속 L5 코드 + 불확실한 이름"으로만 표기할 예정(taskId 없음). dev의 플레이스홀더 임포트(spec 2026-09-06 §8)는 자리만 있고 L5·이름을 실을 계약이 없어 백로그였음 → 스펙 `docs/superpowers/specs/2026-09-07-interview-external-refs-design.md`.
...
- **후차 해소 알림 + 임포트 관리자 파라미터**(2026-09-08 사용자 요청): ① `fw_external_linked` — 자리표가 자동 연결되면 캔버스 단위 1건, 수신자=캔버스 L5 직속·조상 관리자 ∪ 실제 L5 직속·조상 관리자(실행자 제외), 제목=캔버스·본문 인물 필+전달 L5/연결 L6 칩+개수(KNOWN_TYPES·i18n·Link2). ② `framework.categories[].admins`(어느 레벨이나) — 홈 체인 행만 `CategoryPermission(user)` **add-only**(사용자 결정: 제거 없음), 외부 행 admins는 경고 후 무시, 리포트 `category admin … added @ 코드`+미등재 경고 → 패널 "카테고리 관리자(파일에서)" 목록. 샘플 3종에 seed 로그인 admins 부여(calibration L5/L3·change-control·qa-deviation).
- **승인·알림 배선 재검토**(사용자 요청 2026-09-08): 빈틈 2개 보정 — ① `fw_external_linked`가 인박스 카테고리 매핑에 없어 "전체"에서만 보이고 통계에서 빠짐 → `subprocess` 계열로 매핑. ② 후차 연결이 캔버스 draft를 조용히 바꿈 → `VersionEvent(external_linked, note=연결 제목)` 기록 + 타임라인 라벨/아이콘(Link2)/칩. 확인 결과 이상 없음: 확정 게이트는 확정 시점 재검증(대기 요청 경합 없음), 임포트 관리자 추가는 sysadmin 전용 경로(설정 PUT과 동급)·결정권은 판정 시점 라이브 계산이라 즉시 유효, dry-run은 알림·이벤트 모두 롤백.
- **드라이런 리포트 외부 참조 섹션**(사용자 요청 "드라이런에도 내용 알 수 있게"): `interview-report.ts`가 캔버스 문구 5종을 `externalRefs`(외부 L6·출처 L5·캔버스·상태 linked/placeholder/ambiguous/unknown-origin, 조치 필요 먼저)+`externalSummary`로 재구성 — `ReportMessage.captures` 추가, 모호 경고가 같은 참조의 자리표 행을 이김, `linkage` 고정 코드의 후차 해소 행은 맵 항목 대신 resolved 카운트. 패널은 다이제스트 아래 전용 표+상태 칩+확정 불가 안내, 캔버스 행은 "외부 L6 · 연결 n · 자리표 m" 카운트만. 엔진 linked 문구에 `@ L5코드` 추가(FE PATTERNS 계약 동기).
...
- **드라이런 리포트 2열 재구성(A안, 2026-09-08)** — 나열식으로 좌우로 퍼지던 리포트를 목업(HTML 4차, 사용자 승인) 그대로 구현. 좌: 요약 카드(강조 톤, 6셀) · 확인 필요(종류별 접기 + 펼치면 맵 칩) · 외부 L6(헤더 상태 필 = 필터, 해당 없는 행은 제거 대신 흐림) · 카테고리 관리자 · 거버넌스(유지/교체 드롭다운 + 아이콘, 굵은 값 = 적용 뒤 값·버려지는 값 취소선). 우: L5 파일 카드(`Lv5` 필 색 = 검증 결과, L1~L4 계보 필 브레드크럼 + 업무체계 탐색 아이콘(계보 코드를 `/nodes`로 따라가 id 해석, 미적용 L5는 안내 토스트), 외부 L6 필·파일명 우측, 맵·노트 수) + 항상 펼친 L6 목록(4행 노출·숨은 스크롤·5개 이상은 모두 보기) + 행 미리보기(업로드 JSON의 actions/relations를 `lib/interview-preview.ts`가 백엔드 규칙과 동형으로 그래프화 → 에디터 자동정렬 → ScopePreview, 1.5노드 높이·Start 기준 시야·드래그/휠 줌). 우측 맵/L5 클릭 = 좌측 관련 항목 우선 재정렬(FLIP `lib/use-flip-order.ts`)·강조·흐림, 좌측 호버 = 우측 해당 행 밝힘, 하단 바에 필터 상태 + Clear. 사용자는 필(UI 언어 이름 → 영어 → 아이디, 카드 재활용), 섹션 헤더 끝 i 툴팁(긴 대시 없음). 컴포넌트 `components/admin/import-report/`(9파일), `import-governance-review.tsx`는 `governance-section.tsx`로 대체. 스모크 `pw-smoke-interview-import` 26/26(OK 배지→`Lv5` 필 data-state, 체크박스→select), field-promotion `[1]` 통과, vitest 909(+미리보기 그래프·관계 색인 테스트).
- **파일 이슈를 좌측에도**(사용자 질문 "자기순환·승격 경고도 뜨지?" → 우측 카드 이슈 표에만 있었음): `files[].issues`(어댑터 AdapterIssue)를 `classifyFileIssue`가 분류(자기 반복 ◇ 합성·판단 승격·외부 L5 계보 없음·seq 폴백·기타 file-issue)해 다이제스트에 합침 — `rows[i]…` 경로는 그 행의 맵에 귀속(맵 행 ⚠ 수에도 반영, 제외된 파일이면 맵 이름만), 파일 수준 경로는 홈 L5(캔버스)에. 문구의 `a0N`/`seq N`은 `IndexedMap.steps`(actions seq→label)로 단계 이름으로 바꿔 보인다. 요약 경고·오류 셀은 리포트 행 + 파일 이슈 합(부제 "오류 n · 파일 이슈 m"). 우측 카드의 이슈 표(경로 포함)는 그대로 유지.
