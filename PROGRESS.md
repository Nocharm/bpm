...
- **구현(Task 12, 연결·등록):** `relations-step.tsx`(`RelationsStep` — entry/edges 표 편집 + `ImportMapPreview scope=canvas` 미리보기)·`register-step.tsx`(`RegisterStep` — 문서 조립→기존 인터뷰 임포트 dry-run/apply 리포트 UI 재사용, 완료 화면은 `openLinkageMap`+`router.push`로 L5 캔버스 이동, `RegisterStep.TaskPreviewModal`) 신규. `ImportMapPreview`에 `hideClose` prop 추가(연결 단계 임베드에서 죽은 닫기 버튼 은닉). `governanceKey`/`parseGovernanceKey`는 이미 `lib/interview-report.ts`에 공개돼 있어 그대로 재사용(framework-panel.tsx 변경 불필요). 실 AI 없이는 드로잉→연결→등록 전체 흐름을 끝까지 못 밟아 Task 12 Step 4(가짜 AI 서버) 실행은 보류(스모크는 Task 14, 브라우저 확장 미연결이라 시각 검증도 미수행) — 게이트(tsc·lint·vitest 1033·catalog)만 확인.
- **구현(Task 13, 관리자 패널 진입):** `framework-panel.tsx` 인터뷰 임포트 헤더 위에 sysadmin 전용 진입 카드(`fw-consult-entry`) — `/categories/all`(전 카테고리 경량 목록, 트리 state는 펼친 가지만 지연 로드라 전체 L5 목록엔 못 씀)에서 level===5만 걸러 `SearchSelect` 옵션 구성, 시작 시 `createFrameworkInterview`→`/framework/consult/{id}` 이동(409는 토스트+`listFrameworkInterviews` 재조회), 진행 중 세션 이어하기 목록, 외부 AI 프롬프트 버튼(선택된 L5의 `code`/경로는 `getCategoryChain` 지연 조회). 설정 페이지 `?tab=framework` 딥링크(`framework`만 화이트리스트, 그 외는 기존 첫 가용 탭 폴백).
- **구현(Task 14, 스모크·매뉴얼·계약 노트):** 가짜 OpenAI 호환 AI 서버(`fake-ai.mjs`, scratchpad, 미커밋)를 4개 계약(계획·설문·행·연결)의 시스템 프롬프트 고유 구절로 판별해 스키마를 만족하는 JSON을 돌려주도록 작성, `frontend/scripts/pw-fw-consult.mjs`(관리자 진입→계획 잠금→설문 전량 제출→카드 드로잉→연결 확정→dry-run 리포트, 10/10 PASS 1회 시도, 버그 없음) 신규 — 브리프의 DOM 가정(입력창·`role=option`)이 실제 `SearchSelect`(버튼 트리거+포털 목록)와 달라 실측으로 교체, 매 실행 고유 이름의 L1~L5 체인을 `POST /api/categories`로 자급해 기존 시드와 충돌 없이 반복 가능. 관리자 매뉴얼(ko/en) 13장에 "AI로 L5 채우기" 절 추가(4단계·제안 적용 배지·일시정지/재개/이어하기·등록은 인터뷰 임포트와 동일·외부 AI 프롬프트 왕복, 긴 대시 미사용). CLAUDE.md Lessons에 인터뷰 JSON 0.5 계약 3표면 결합 규칙 추가. 게이트 backend 1527 passed·ruff clean, frontend tsc·lint·vitest 1033·catalog 재확인.
- **리뷰 수정(Task 10~12):** 2초 폴링이 매번 새 `session`/`task` 객체를 만드는데 `useEffect` deps에 그 객체를 그대로 걸어둔 곳들이 폴링마다 사용자의 미저장 입력을 지우던 문제 — 설문 답(`AnswerStep`, deps를 `task?.id`/`task?.status` 스칼라로), 계획 카드(`PlanEditor`)·연결 엣지(`RelationsStep`)는 resync effect를 없애고 부모가 `key={JSON.stringify(session.plan/relations)}`로 리마운트하는 쪽으로 전환. 그 밖에: 마지막 카드 제출 후 `task=null`인 "준비 중" 단계가 빈 화면이던 것 고침(스피너+`waitingDrawing`), 실패 카드가 있으면 `deriveStep`이 "relations"로 새지 않고 "waiting"에 머물게, `submittedAtRef`가 실패/재시도 카드에서 새지 않게, 보드에 지금 그리는 카드 이름 표시, relations 편집 행에 `data-id`+i18n(`fwConsult.save`/`back`/`cancel`/`resizeBoard`/`entry`/`edge*`/`workingOn`), 라디오 키보드 포커스 링(`has-[:focus-visible]`), 초기 세션 조회를 `run()` 우회 직접 호출로 바꿔 불필요했던 `eslint-disable`(set-state-in-effect) 제거 + 폴링 응답에 `alive` 가드·성공 시 에러 배너 클리어. 게이트 tsc·lint(0/0)·vitest 1033·catalog 재확인.
- **구현(전 브랜치 리뷰 반영):** 설문 생성 실패를 `pending`이 아닌 `failed`로 멈춰 러너 무한 루프를 끊고(보드 재시도 버튼으로 넘김), brief를 실제로 입력·저장하게 만들고(`FrameworkInterviewPlanIn.brief`, 제안 생성은 저장→생성 체인), 러너가 멎은 듯하면 폴링 5틱 뒤 보드에 재시작 안내를 띄운다. 세션 접근을 생성자 전용에서 sysadmin 전체로 풀어(담당자 교대 시 캠페인이 묻히던 문제) `_get_owned`→`_get_session_row`, 채번은 휴지통 맵 코드까지 예약(임포터가 거절하므로). 그 밖에 relations taskId 검증 공용화(생성 502 전 계량 커밋·확정 422)·AI 행에서 실명 `owner` 제거·`_category_path`/`_catalogs` 정리·단일 워커 전제 주석. 게이트 backend 1530 passed·ruff clean, frontend tsc·lint·vitest 1033·catalog.
