...
- 후속 2(사용자 요청): **브랜드 워터마크** — "SAMSUNG BIOLOGICS" 사선(-24°) 리버스 텍스트를 캔버스 중앙에 저불투명(6%)으로, 뷰포트 고정·ReactFlow 앞 DOM이라 노드 아래(`l5-brand-watermark`). 브랜드 가이드의 모노크롬/리버스 적용 관례 준수, 실 로고 SVG 제공 시 교체 여지. 검증 23/23(알파 0.94·워터마크 유/무).
