...
- **연결 화면 채팅 플로팅 + 노드 룩 단일 소스(dev, 09-24):** 우측 고정 패널을 걷고 피드백 채팅을 에디터 AI 채팅과 같은 `ScopeWindow`(끌기·크기 조절·최소화→우하단 말풍선 칩, 기하는 `bpm.fwRelationsChatGeom`)로 캔버스 위에 띄워 맵이 전폭을 쓴다. 로그는 `MarkdownView`. 캔버스 L6 노드는 이미 같은 `ProcessNode`였고(파라미터는 링크 맵이 생기는 등록 전이라 빈 것), 카드의 요약·역할·부서를 sp* 자리에 넣어 L5 맵 룩과 맞췄다. 핸들·호버 raw style은 `lib/flow-handle-style.ts` 한 곳으로 합쳐 에디터와 연결 캔버스가 같은 문자열을 쓴다(별도 트랙 금지, 사용자 우려).
- **설문의 초점 = 절차의 애매한 지점(dev, 09-24):** `L6_QUESTIONNAIRE_CONTRACT`를 필드 채우기(basic 2~3·io 1~2)에서 판단 주체·분기 조건·반려 복귀·병렬/순차·예외 경로·경계·인계 시점을 묻는 구성(exceptions 3~5·activities 1·basic/io 0~2)으로 뒤집고, 브리프·첨부·기존 맵에서 읽히는 값은 묻지 않고 suggested로만. 문항마다 `why`(근거 한 줄, `Question.why`·normalize 동의어 reason/rationale)를 받아 폼이 문항 아래 작게 보여준다. 섹션 순서 exceptions→activities→basic→io, 라벨 "판단·예외·분기". 첨부는 이미 `build_context_text`로 설문 프롬프트에 들어가고 있었다.
- **계획 화면 3열(dev, 09-24):** 우측 L6 카드가 인풋 3줄 전폭 카드로 너무 넓게 쓰던 것을 `brief | 읽기전용 카드 목록 | 선택 카드 상세(380)`로 재구성(사용자 제안). 목록 행은 클릭 선택·핸들 드래그(포인터, offsetTop 기준으로 FLIP transform 중간값에 안 흔들림)·Alt+↑/↓ 순서, 상세 열이 유일한 편집면(이름·요약·역할·부서·유지/정정·삭제), 새 카드는 자동 선택. 인라인 hover 편집 카드와 ↑↓ 버튼은 폐기. `moveCard` 헬퍼+테스트, 스모크 2종 이식(드래그 재현 포함).
- **데모 리뷰 피드백 5종(dev, 09-23):** L5 하늘 캔버스 드롭존 라벨을 잉크 반투명 알약 위 밝은 글자로(보라 글자가 하늘에, 흰 글자가 노드에 묻히던 문제, 노드 유무 모두 읽힘) · AI 버튼 크기를 primary 한 벌로 통일(`inline`=primary, 전폭 "다시 제안"은 우측 정렬)하고 그라데이션을 elevated→accent→sky 3단으로 · 주관식 [AI 제안]은 제목 줄 우측 글자형 버튼(`AiButton variant="text"`), 편집 박스는 전폭 · 연결 캔버스 핸들/호버 강조를 에디터와 같은 raw style 토큰으로(RF 기본 파란 원형 핸들 제거) · 연결 단계 우측 "L6 카드" 목록 삭제(좌측 보드와 중복, 미리보기·수정은 보드 행 버튼) — UX 스모크는 보드의 눈 버튼으로 미리보기.
- **최종 리뷰 픽스 2차(dev, 09-23):** 보드 카드를 고르면 연결·등록 단계를 **언마운트하지 않고 숨긴다**(`selectedTask ? "hidden" : …` 호스트 div) — 언마운트는 편집 중 캔버스의 300ms 디바운스 저장을 취소한 뒤 낡은 `session.canvas`로 리마운트해 드래그·분기·라벨을 되감고, 등록 단계에선 드라이런이 다시 돌아 거버넌스 선택을 조용히 버렸다. 숨은 ReactFlow는 뷰포트를 그대로 유지한다(스모크에서 transform 동일 확인). 곁들여: 캔버스 노드 라벨에 `taskNames` 적용(L6 카드 이름이 캔버스 title보다 우선, 컨텍스트 메뉴만 쓰던 prop) · FE 분기 노드 제목을 서버와 같은 `{앞 단계} 결과`로(기존 "분기") · 등록 끝난 세션은 카드 피드백 작성창을 걷고 이유를 적는다(서버가 409로 막는 자리, `FeedbackChat locked`) · 매뉴얼 en `[AI suggest]`→`[AI suggestion]`.
- **보드 전 행 클릭 = 상태별 카드 패널(dev, 09-23, Task 4):** ready·drawn 행만 눌리던 보드를 전 행 `role=button`으로 열고, `AnswerStep`을 흡수한 `task-panel.tsx`가 상태별 뷰를 낸다(준비 중 링 / 설문 폼 / 제출 답+드로잉 링 / 완성 행=흐름 미리보기+답+`FeedbackChat scope="task"` / 실패 재시도·건너뛰기 + 상단 [닫기]). 카드 피드백은 제자리 수정(상태는 drawn 유지)이라 `submitted` 배지 전환이 없다 — 패널이 `session.feedback_log`의 그 카드 건수를 트리거로 `GET /tasks/{pk}`를 다시 받아 답·미리보기를 갱신한다. `selectedTaskId`는 상태와 무관하게 우측을 차지하고(연결·등록 단계에서도), 단계가 relations/register로 **전환되는 순간**에만 `applySession`이 선택을 푼다 — 이펙트에서 풀면 set-state-in-effect 린트이고, 매 갱신마다 풀면 그 단계에서 연 카드가 피드백 한 번에 닫힌다. 설문·확인 화면은 `groupQuestionsBySection`(basic→activities→exceptions→io, 빈 섹션 생략)으로 섹션 헤더를 달고 활동 섹션만 1열 전폭(2열 섹션에서만 `xl:col-span-2` — 1열에 주면 암시적 열이 생긴다). 새 스모크 `pw-fw-consult-task-panel.mjs` 11/11(러너를 미리 pause해 pending·submitted 상태를 붙잡는다), `pw-fw-consult-pick-card.mjs`는 제출 뒤 선택 유지 + [닫기] 복귀로 4/4 갱신. 리뷰 픽스: **미리보기 source는 `useMemo`로 참조 고정**(`ImportMapPreview`가 source로 메모하고 그래프가 바뀌면 Start로 스크롤을 되돌려, 새 객체를 매 렌더 넘기면 2초 폴링마다 사용자가 잡은 화면이 튄다) · `onClose` 옵셔널로 자동 흐름 패널엔 [닫기] 미노출(선택이 없어 눌러도 무반응이었다) · 제출 답은 `detail.answers` 도착 후에만 렌더(제출 직후 "제안 적용" 배지 번쩍임) · 행 keydown은 `e.target !== e.currentTarget`이면 무시(행 안 버튼의 Enter가 선택까지 일으키고 Space가 삼켜졌다) · 미지·누락 section은 버리지 않고 basic으로(서버 기본값과 동치 — 버리면 문항이 화면에서 사라져 답 없이 제출된다).
- **연결 단계 = 편집 캔버스(dev, 09-23, Task 3):** 엣지 표로 흐름을 고치던 연결 단계를 폐기하고 `relations-canvas.tsx`(ReactFlow, 핸들 드래그 연결·엣지 클릭 라벨·우클릭 분기 추가/제거·언급/taskId 복사·자동 정렬/되돌리기 1단계/맞춤)로 교체 — 진입 즉시 `onPropose("")` 1회(오버레이 링 최소 1.5초)·편집은 300ms 디바운스 `PUT /canvas`·우측은 L6 카드 목록 + `FeedbackChat scope="relations"`. 캔버스 엣지엔 핸들 개념이 없어 표시 단계에서 4변 핸들+우→좌로 못 박고(LR 고정), RF 상태를 편집 진실로 두고 구조 변경만 `flowToCanvas`로 캔버스를 만들어 되돌려 받는다. **디바운스 저장 응답은 `applySession`하지 않는다** — 하면 `key={JSON.stringify(session.canvas…)}`가 매 편집마다 바뀌어 300ms마다 캔버스가 리마운트(뷰포트·입력 초기화)된다. 오버레이는 캔버스 도착 또는 호출 종료(busy 해제) 중 먼저 오는 쪽에 걷힌다 — 실패한 제안이 링으로 화면을 잠그지 않게. 리뷰 픽스: **제어형 edges엔 `onEdgesChange`가 필수**(없으면 select 변경이 버려져 어떤 엣지도 selected가 안 되고 Delete→`onEdgesDelete` 경로가 영구히 죽는다) · 노드는 전부 `deletable: false`(Delete가 노드까지 지우면 `deleteElements`가 연결 엣지를 먼저 떼어내 서버엔 고아 노드, 화면엔 없는 노드가 남는다 — 마름모 제거는 컨텍스트 메뉴만) · `NodeData.hideLinkBanner` 옵트인 플래그로 아직 맵이 없는 L6 노드의 "링크 미지정" 배너 억제(기본 off라 에디터·비교·미리보기 불변) · `proOptions.hideAttribution` · 전역 `<style>`을 캔버스 래퍼 클래스로 한정(같은 단계의 카드 미리보기 모달이 또 다른 RF 인스턴스다) · `fitViewOptions maxZoom 1.2`. 새 스모크 `pw-fw-consult-canvas.mjs` 13/13(오버레이·노드 4·피드백 엣지 뒤집기·분기 +1·라벨 저장·엣지 Delete 삭제+저장·노드 Delete 무반응·PUT 200·등록 도달), 기존 3종은 자동 제안 대기로 이식 12/12·10/10·15/15.
- **FeedbackChat 공용 컴포넌트(dev, 09-23, Task 2):** `frontend/src/components/framework-interview/feedback-chat.tsx` — `session.feedback_log`를 scope(relations/task, task는 `task_pk` 일치까지)로 필터해 목록+textarea(Enter 전송·Shift+Enter 줄바꿈, IME 조합 중 Enter는 무시 — `isComposing` 가드, `ai-chat-panel.tsx`/`interview-panel.tsx`와 동일 컨벤션)+`AiButton inline` 전송을 렌더. `draft`/`onDraftChange` 제어형이라 우클릭 "피드백에 언급"이 밖에서 텍스트를 끼워 넣을 수 있다. Task 3(관계 캔버스)·Task 4(행 패널)가 마운트.
- **캔버스↔ReactFlow 순수 변환(dev, 09-23, Task 1):** `frontend/src/lib/relations-canvas.ts` — `canvasToFlow`/`flowToCanvas`(왕복 시 task_id·node_type은 `prev`에서 되찾고 위치만 갱신) + `addBranchAfter`/`removeBranch`(분기 마름모 삽입·제거, 엣지 원위치 치환으로 순서 보존)·`connectNodes`/`setEdgeLabel`/`removeEdge`/`moveNode`. `gateway`는 RF 엣지 `data.gateway`로 왕복하고 없으면 키 자체를 생략(전부 parallel 팬아웃이 확정 게이트 6에서 소실되지 않도록).
- **자연어 피드백 엔드포인트(dev, 09-23, B4/B5/B9):** 캔버스·행을 고치려면 손으로 그리거나 설문을 다시 받아야 했던 것을 `POST /framework-interviews/{id}/feedback`(`{scope: relations|task, task_pk?, message}`) 한 곳으로 — relations는 현재 캔버스+카드 목록을 실어 같은 형태를 받아 `normalize_canvas`(id·task_id 불변, 새 노드는 `__branch__` 분기만, 빠뜨린 subprocess/start/end는 직전 캔버스에서 보충, 미지 끝점 엣지 폐기, **모델이 빠뜨린 gateway는 같은 쌍의 base 값을 이어받는다** — 전부 parallel인 팬아웃이 라벨 수정 한 번에 seq로 떨어지면 확정 게이트 6이 무너진다) → `validate_canvas` → 노드 구성이 바뀐 경우에만 `relayout_canvas`(그대로면 잡아 둔 배치 유지)로 저장하고 relations는 건드리지 않는다(확정 시 역산). task는 현재 행을 실어 `RowOut`을 다시 받아 `validate_row` 오류면 422, 통과면 행·이슈·drawn_at 갱신 + 세션이 `ready`였으면 `linking`으로 되돌려 조립본 폐기. 두 경로 모두 `feedback_log`에 최근 10건만 남기고, 422는 `{"errors":[…]}`(PUT /canvas와 같은 모양)로 끝내되 청구된 AI 호출의 계량 이벤트는 커밋한다. RowOut 마감(`owner` 제거 + 빈 부서는 카드 값으로)은 `assemble.finalize_row_output` 한 곳으로 모아 러너 드로잉과 피드백이 같은 규칙을 쓴다 — 한쪽만 부서를 메우면 피드백 한 번에 부서가 조용히 지워졌다. 계약 2종은 레지스트리 키(`l5_canvas_feedback_contract`·`l6_row_feedback_contract`)로 오버라이드 가능(PROMPT_KEYS 15개).
- **세션 연결 캔버스(dev, 09-23, B4/B5):** relations(엣지 목록)만 편집 진실이라 흐름을 눈으로 고칠 수 없던 것을 `canvas`(노드+엣지) 컬럼으로 승격 — `app/framework_interview/canvas.py`가 `expand_relations_to_canvas`(팬아웃≥2·전부 parallel 아님 또는 자기 반복 → `__branch__{task_id}` 마름모, `consultant_layout.layout_flow` LR 배치, `__start__`/`__end__` 보강; 분기 규칙의 진실은 `scripts/import_consultant.expand_linkage_branches`이고 FE 미리보기 `buildL5PreviewGraph`와는 start/end 보강 규칙만 동형)·`collapse_canvas_to_relations`(마름모 접기, 흐름 역행 직결은 loop, 미지 task_id 폐기)·`validate_canvas`의 단일 진실이고 라우터는 호출만 한다. `POST /relations`는 `{comment}`를 받아 `[직전 제안]`+`[사용자 피드백]`을 프롬프트에 실어 재제안하고 canvas를 채우며, `PUT /canvas`(검증·저장, relations 불변 — 단 `ready`였으면 `linking`으로 되돌리고 조립본 폐기)·`PUT /relations`(`{canvas}` 접기 또는 `{relations}`, 어느 쪽이든 캔버스를 relations와 맞춰 저장)가 확정 경로. 캔버스 엣지는 `gateway`를 들고 다닌다 — 전부 parallel인 팬아웃은 마름모 없이 직결로 펴지므로 이 표시 없이는 확정 게이트 6(plain_fanout 예외)이 왕복에서 소실된다(직결=parallel 추론은 쓰지 않는다). 라벨 자리는 seq면 `label`, 갈라짐·되돌아감이면 `condition` 한쪽으로만 되돌린다. `feedback_log` 컬럼은 노출만(쓰기는 후속).
- **러너 병렬(dev, 09-23, B13):** 캠페인 러너가 한 바퀴에 AI 1콜만 돌리고 설문을 `PREFETCH_READY=2`장으로 묶던 것을 폐기 — `run_one_step` → `collect_jobs`(submitted 드로잉 먼저, pending 설문 다음·seq 순) + `run_jobs`(전부 `gather`)로, 동시 실행은 새 Settings `fw_consult_concurrency`(기본 3, `.env.example`·compose 동행)가 제한한다. 잡마다 자기 `SessionLocal()`을 열어 동시 커밋을 분리하고 시작 직전 일시정지·비활성·AI 차단을 재확인한다(전역 상한은 기존 `ai_max_concurrency`). 세마포어는 `run_jobs` 호출마다 새로 만든다 — 모듈 전역 캐시는 이벤트 루프 바인딩·미회수 항목만 남기고 세션당 루프가 하나라 동치. 리뷰 픽스: `gather(return_exceptions=True)` 후 첫 예외를 되던진다(즉시 전파하면 형제 잡이 도는 채로 `recover_stale_tasks`가 그 행을 되돌려 다음 kick이 중복 실행) + 잡이 세마포어를 기다리는 동안 API가 상태를 바꾼 태스크는 건너뛴다. FE `hasBackgroundWork`도 ready 장수 조건을 버리고 pending/generating/submitted/drawing 존재로 단순화. 병렬 테스트는 위치 큐 대신 시스템 프롬프트 마커 라우팅(`_fake_ai_router`)이 필요하다.
- **IO 배열 계약(dev, 09-23, B8a):** 인터뷰 JSON 0.5 `actions[].input`/`output`을 문자열에서 `list[str]`로 승격 — `normalize_row`(`_io_list`, `,`/`\n`/`/`/`·` 분해·trim·중복 제거)·`RowAction.input/output`(`list[str] | str | None`)·`L6_ROW_DRAFTER_CONTRACT` 예시+규칙·역변환 `map_to_row`(개행 문자열→list) 4표면 동기화. 어댑터 `_join_multi`는 이미 list→개행이라 무변경. FE 외부 프롬프트(`interview-json-prompt.ts`)·샘플 md에도 배열 표기 반영. 리뷰 픽스: `render_existing_row`(정정 프롬프트 렌더러)가 list를 `str()`로 굳혀 `['요청서', '첨부']` 파이썬 리스트 표기를 AI 프롬프트에 그대로 흘리던 것을 `", ".join`으로 고침.
...
- **가짜 AI 피드백 라우팅(dev, 09-23, Task 6):** `frontend/scripts/fake-ai-server.mjs`에 캔버스·행 피드백 마커 2종 추가(`"연계 캔버스(노드·엣지)를 사용자 피드백대로"` → 첫 subprocess→subprocess 엣지의 src/dst를 뒤집어 반환, `"rows[] 원소를 사용자 피드백대로"` → `l6`에 `(수정)` 접미) — 기존 `"연계 캔버스"`/`"rows[] 원소"` 마커의 부분 문자열이라 route()에서 먼저 검사한다. 설문 고정문에 `section` 필드(q1 activities·q2/q3 basic·q4 exceptions·q5/q6 io), 행 고정문의 input/output을 배열로. 스모크 회귀: `pw-fw-consult.mjs` 12/12·`pw-fw-consult-existing.mjs` 10/10·`pw-fw-consult-ux.mjs` 15/15·`pw-fw-consult-pick-card.mjs` 3/3 전부 green(수정 불필요). 백엔드 pytest 1590 passed·ruff clean, 프론트 tsc/lint/vitest 1073 passed/catalog check 전부 green.
...
- **스모크 이식·신규 + 매뉴얼(dev, 09-23, Task 5):** `pw-fw-consult-canvas.mjs`(13/13→18/18)에 대기 카드 클릭 패널·설문 섹션 헤더·등록 단계에서도 살아있는 보드 클릭(완료 카드=미리보기+피드백 채팅)·카드 피드백 (수정) 반영을 얹었다 — 러너를 API로 미리 pause해 pending을 붙잡는 패턴은 `pw-fw-consult-task-panel.mjs`와 동일. 붙이자마자 "제안 오버레이" 체크가 안정적으로 깨져(가짜 AI가 즉답이라 등장→소멸이 while 루프 종료보다 빠르다) `waitForFunction` 등록을 while 루프 진입 **전**(주석이 원래 말하던 지점)으로 옮겨 재현성 있게 고쳤다. 3종 이식 스모크(`pw-fw-consult`·`-existing`·`-ux`)는 이미 자동 제안 대기로 정리돼 있어 수정 없이 그대로 green. 관리자 매뉴얼 ko/en "AI로 L5 채우기" 절을 라운드2 UX(관리 패널 타일 드릴다운·캔버스 기반 연결 편집·전 행 클릭 패널·주관식 빈칸+[AI 제안] 타이핑)로 갱신.
- **최종 리뷰 픽스(dev, 09-23):** `validate_canvas`에 node_type 화이트리스트 검증 추가·`feedback`/`PUT canvas`/`POST·PUT relations` 4곳에 `applied` 세션 409 가드(`reopen_task`/`revise_task`와 같은 메시지)·canvas.py 거짓 동형 주석 정정(분기 규칙 진실은 `expand_linkage_branches`, FE 미리보기는 start/end 보강만 동형)·spec 3곳 정정·`_io_list` 조건식 분해(동작 불변)·FE `generateFrameworkRelations`에 comment 인자, `confirmFrameworkRelations` 시그니처를 `{canvas}|{relations}` 판별 유니온으로, `saveFrameworkCanvas` 신설.
...
- **스모크(dev, 09-23):** 신규 `pw-fw-level-actions.mjs` 10/10(빈 안내·L1→L2 타일·서브트리 배지 1·타일 클릭=트리 싱크·L4 모달 중복 차단+확인 비활성·정상 이름=세션 오픈·L5 이어서 타일·임포트 섹션·복사 토스트). 구 id(`fw-consult-create/start/new-name/sessions-toggle`)를 쓰던 4종 이식 — `pw-fw-consult-new-l5.mjs` 6/6·`pw-fw-admin-layout-shot.mjs` 8/8(시드 L5는 세션이 붙어 있을 수 있어 start/resume 타일 중 하나)·`pw-fw-consult.mjs`·`pw-fw-consult-existing.mjs`는 진입 단계까지 PASS(이후 계획 제안은 저장소 밖 가짜 AI :9999 전제라 이번 실행에선 미검증). 함정: Playwright `isVisible({timeout})`은 기다리지 않는다 → 토스트 같은 다음 틱 마운트는 `waitFor`. 헤드리스 클립보드는 `grantPermissions` 필요. 자식 타일 클릭 시 부모도 펼쳐야 트리 행이 렌더된다.
...
- **L5 드롭존 가시성(dev, 09-23):** 드래그 드롭존 부채꼴이 `--color-ink-tertiary` 4~36% 알파라 남색 하늘 프레임 위에서 안 보였다 → `isFrameworkMap`이면 기반색을 `--color-canvas`(밝음)·활성 존은 `--color-accent-sky`로 바꾸고 알파를 올린다(10/45·14/35·24/55). 라이트 캔버스는 값 그대로.
...
- **기존 L5 학습·정정(dev, 09-22):** 캠페인이 L5 아래 이미 있는 L6를 무시하던 문제 — `app/framework_interview/existing.py`(맵 그래프 → 인터뷰 행 역변환·스냅샷 로더·계획 카드 병합). 엣지 kind는 저장되지 않아 구조(되돌아감=loop, 판단 노드 출발=branch)로 복원하고, 임포터가 맵 컬럼에 적는 ownerRole·시작/완료 조건은 스냅샷 단계에서 되살린다. system은 카탈로그 정규화(Other) 때문에 왕복하지 않는다. 세션 생성 시 `FrameworkInterviewSession.existing`에 스냅샷 저장(`FrameworkInterviewOut.existing`로 노출, `row` 미노출) + 계획 카드/태스크 `mode`(new|keep|revise) 컬럼 배선. 계획 제안·저장은 `merge_existing_cards`를 거쳐 기존 맵을 지워도 유지 카드로 되살아나고, 잠금 시 keep 카드는 기존 코드·행 그대로 drawn(러너 제외)·revise 카드는 pending·새 카드만 채번, `POST .../tasks/{pk}/revise`로 유지→정정 전환(설문·답 비우고 row는 미리보기로 유지, 유지 카드의 `reopen`도 같은 전환으로 위임), 정정 태스크의 설문·드로잉 프롬프트에는 `[현재 등록된 내용]` 블록이 붙는다. 세션 화면은 계획 카드에 기존 칩·유지/정정 세그먼트(삭제는 병합이 되살리므로 비활성)와 안내 줄, 보드에 기존/정정 칩·[정정] 버튼을 얹고 외부 AI 프롬프트에 기존 L6 목록을 넣는다. 연결 화면의 reopen 버튼은 유지 태스크면 라벨을 [정정]으로 바꾼다(서버가 그렇게 위임). 스모크 `pw-fw-consult-existing.mjs` 10/10(임포트로 L6 2개 → 기존 칩 2장 → 2번 카드 정정 → 잠금 즉시 drawn 1 → 설문 1장 → 등록 dry run에서 Unchanged 1·Updated 1), 캡처 `docs/qa/screens/fw-consult-existing.png`. **리뷰 픽스(09-22 2차):** 유지 행이 사실상 데이터 유실 경로였다 — 행 스코프 복원이 ownerRole·시작/완료 조건만 되살려 재임포트가 나머지 `sp_*`(IO·시간·건수·시스템·폴백)를 빈 값으로 덮었고, 별칭으로 전달된 시스템은 정식 표기로 되돌아가 폴백을 덮어 무변경이 "변경"으로 보였다. `fields` 전량 복원 + 카탈로그 인지 해석기(`make_system_resolver`)로 왕복이 `unchanged`가 되게 맞췄다. 더해 유지 행은 잠금 시점 스냅샷이라 조립 직전에 현재 맵에서 다시 읽고(맵이 사라졌으면 문서에서 빼고 경고), 하위 맵 링크가 있는 L6는 `frozen`으로 동결해 카드·재게시 대상에서 제외한다(역변환이 링크 노드를 떨어뜨리므로). FE 잠금 버튼의 중복 판정도 서버 규칙(`hasBlockingDuplicate`)으로 통일.
- **관리 뷰 상세 패널(dev, 09-22):** Categories & import Manage 뷰를 좌 트리 : 우 선택 행 상세 패널(둘 다 340px)로 재구성 — 트리 행은 선택 전용(액션 아이콘·인라인 관리자 제거), 상세 패널이 머리·정보 줄·액션 6종(불가는 숨기지 않고 비활성+title 이유)·AI L5 블록(L4=새 L5 만들기, L5=채우기·진행 세션이면 이어하기, 세션 목록은 fixed 포털 드롭다운)·[인터뷰 임포트] 버튼을 담는다. 임포트는 버튼이 바로 파일 탐색기를 열고 고른 파일은 그리드 아래 전폭 스트립에 필로 나열(개수·모두 지우기·Dry run). 아코디언 3종은 폐기해 `admin-section.tsx` 삭제. 스모크 진입을 검색→행 선택→상세 패널 버튼으로 갈아끼웠다 — `pw-fw-admin-layout-shot.mjs` 8/8(휴지 상태·L5 선택·임포트 스트립 3장 캡처, `pw-fw-import-section-shot.mjs`는 중복이라 삭제)·`pw-fw-consult.mjs` 12/12·`pw-fw-consult-new-l5.mjs` 6/6·`pw-smoke-framework-admin.mjs` 7/7, 폐기된 `interview-import-file-list`를 쓰던 스모크 6종도 필 id로 교체. 캡처 갱신: `framework-admin-rest`·`framework-admin-selected`·`framework-import-section`(구 `framework-admin-consult-open`·`framework-admin-import-open` 삭제).
- **진입 피커 드롭다운화 + 섹션 높이 재조정(dev, 09-22):** [AI로 L5 채우기]가 트리 높이만큼 길어진다는 지적 → `FrameworkCascadePicker variant="dropdown"`(검색 상자만 자리, 트리는 상자 클릭/포커스 시 body 포털 fixed 패널 z 1350, 고르면 닫힘·Esc·바깥 클릭 닫힘, 아래 공간 부족 시 패널 축소). consult maxHeight 640→480·import 900→720·좌측 트리 70vh→고정 340px(검색 상자+10행, 내부 스크롤). L5 개수 표기는 관리 트리·피커 모두 L4 행에만. [인터뷰 임포트]는 접힘/펼침 모두 그리드 아래 전폭(이전 공유 캡처가 옛 배치였음). `pw-fw-admin-layout-shot.mjs`(휴지 상태·드롭다운·임포트 펼침 3장).
...
- **5 관리 부서(B안, 사용자 결정):** `ProcessCategory.admin_department`(조직 경로, `_ADDED_COLUMNS`) — 권한(user|group)과 별개 속성, 비면 상위 상속(`resolve_admin_departments` 순수 함수, nodes/chain/summary에 유효값+출처). PATCH는 개명과 같은 서브트리 게이트 + 유효 조직 경로만(422). **캔버스 owning_department는 결착 L5의 유효 관리 부서에서 파생**(`set_committed_value`로 응답에만, 저장 안 함) → 부서 뷰 조직도에 자리가 생긴다. 지정 UI = 설정>Categories 행의 Building2 버튼(`category-dept-modal.tsx`, 오우닝 피커와 같은 PrincipalPicker, 모달 z 1200 — 피커 포털 드롭다운 1250보다 아래여야 목록이 블러 뒤에 안 묻힘), 요약 카드 "Managing department" 섹션(상속 출처 표기). A안(부서 principal)은 직속 L5 관리자=확정권이 부서 전원에게 새어 폐기.
...
- **4 FW 뷰 검색:** `framework-search-groups.tsx` — 히트를 L5 경로별 그룹(헤더 1단=L5 이름 먼저+캔버스 확정/초안 칩+수, 2단=상위 경로 브레드크럼 최대 2줄 `line-clamp-2`, 클릭=검색 해제+드릴다운 이동 `revealCategory`), 미등록은 점선 그룹 맨 뒤(`lib/framework-search-groups.ts`). 한 줄 말줄임에 L5 이름이 잘리던 문제로 2단 분리(사용자 지시). 카테고리 이름 히트(L1~L5, `/categories/all` 첫 검색 시 1회 로드, 상한 40)를 "Categories" 섹션으로 그룹 위에 — 종전엔 맵·캔버스만 검색돼 L1~L4가 안 나왔다. 클릭=`revealCategory`. 스트립 쉬머는 2.6s·액센트 70%·넓은 띠로 은은하게(1.2s는 너무 빠르다는 피드백).
- **후속(같은 날, 2차 재구성):** 1줄 = 공개 범위 탭(세그먼트 복귀) · 정렬 · 해제 FilterX 아이콘(우측 끝, 활성 시 error 톤), 2줄 = Status · Role · (부서 뷰) Issues · **Type**(SP/Non-SP·L5 canvases·In/Not in the framework를 한 그룹 OR로 통합, 세션 키 `kind`). 필은 내용 폭 비례 차등(`flex-auto`)으로 행을 채우고 내용 좌측·쉐브론 우측. Status/Role/Type/Issues 필 라벨은 UI 언어 무관 영어 고정(Issues 옵션은 UI 언어, 옵션 아이콘 유지), 공개 범위 탭 앞 Layers/Globe/Lock 아이콘. 해제 버튼은 항상 마운트된 슬롯(폭 0→28px·아이콘 슬라이드, 350ms)로 등장해 기존 필을 왼쪽으로 밀고, `data-id`는 활성일 때만(검증 스크립트 "존재=활성" 계약). Type 옵션은 배타 표(`lib/map-kind.ts`: sp⟂non_sp·sp⟂canvas·registered⟂unregistered·canvas⟂unregistered)로 교집합 없는 선택지를 비활성화. 각 드롭다운은 선택이 있으면 목록 맨 아래에 "이 필터 지우기"(accordion-open 등장, 정렬 제외). 탐색 모달의 "이 업무 체계 보기"(정보 카드·우클릭·상단 버튼·행 Go·검색 결과)는 모달을 닫고 그 항목이 선택된 메인 화면으로(종전 "열린 채 따라감" 철회). 반응형 단계는 2줄 필 기준. 스모크는 클램프 검사를 미지정 목록에 고정(선택 맵의 부서 박스가 앞에 오면 "첫 클램프" 가정이 깨지던 데이터 의존 — 종전 실행은 중간에 멈춰 41개 중 26개만 돌았음). 목록 `load_my_roles`가 캔버스에도 체인 관리자 파생(editor/viewer)을 적용해 목록=상세 역할 불일치 해소(캔버스 있을 때만 `get_admin_scope`+역조회 각 1회).
- **6 L6 스트립:** 요약 카드에서 맵을 열면 좌측 맨 아래 `selected-map-strip.tsx`(이름·상태·"L5 …에서 열었음", 떠 있는 카드 톤), L5 카드는 선택 유지. "L5로 돌아가기"·브라우저 뒤로가기(popstate)가 직전 L5 선택으로 복귀(`mapOrigin` ref 미러). ×는 같은 동작이라 제거, 등장 시 테두리 액센트 쉬머 1회(`.strip-shimmer::after` mask-composite 오버레이, 1.2s). 부수 픽스: `FrameworkDrill` revealRequest가 마운트와 동시에 오면 StrictMode 이중 실행의 첫 실행에서 "처리됨"으로 남아 건너뛰던 문제 — 처리 표시를 완료 시점으로.

## 2026-09-21 — 업무 체계 탐색 패널·드릴다운 UX 6종 (dev)

- **탐색 패널 헤더(다이어그램):** 검색을 w-56으로 좁히고 우측에 중심 브레드크럼(상위 체인→현재 중심). 단계 클릭=그 단계의 형제 목록 GoToMenu(현재 강조, L1은 루트 목록, 부모 자식 미캐시면 열 때 fetch) → 재중심(L5는 정보 카드). 패널은 열린 채. 브레드크럼은 우측 정렬(깊어질수록 앞 단계가 왼쪽으로, 넘치면 왼쪽 페이드 마스크), 단계 id 키 + `fw-crumb-in`으로 바뀐 단계만 등장. 재중심 시 `chainFor`(현재 상위·중심·자식·손자 관계, 크럼 메뉴는 앞 단계)로 새 중심의 체인을 미리 chainCache에 넣어 [중심]만 그렸다 앞 단계가 끼어드는 통째 갱신 제거 (같은 날 후속).
...
- **드릴다운:** 브레드크럼은 현재 이름을 생략(바로 아래 레벨 헤더가 담당)하고 끝 체브론만 남기며, 줄바꿈 없이 말줄임(체브론은 말줄임 컨테이너 밖). 우측 `CategorySummaryCard`는 `key` 리마운트를 없애고 이전 요약을 흐리게(0.35, 350ms) 남긴 채 새 요약이 오면 되살리는 크로스페이드 — 스피너 통째 교체 깜빡임 제거. 에러는 `{forId}`로 카테고리에 묶는다.
...
- **결정(재제안 금지):** 방사형 대신 ERD식 직각 엣지 · 브라우저 전체 모달 대신 플로팅 패널 · 결과 행의 조상별 숫자 칩("1 2 3 4")과 상시 "Unassigned" 반복은 밀도 문제로 폐기 · 상위 행 카운트는 서브트리 L5 수가 아닌 직계 하위 수.
- **함정:** 패널 z는 1200(1300이면 GoToMenu 포털이 깔림)·메뉴 열린 Escape는 패널에서 무시·Playwright는 CSS transform `<g>`를 boundingBox+`mouse.click`으로·`/chain` 응답은 카운트 0이라 자식 캐시로 되살림·계단식 헤더는 두 줄(한 줄이면 440에서 검색창 소실). 검증: `pw-smoke-framework.mjs` 36/37(잔여 1건은 미지정 부서 시드 의존, 변경 전부터), vitest 1012, backend 1491, 규모 시드(L5 2,000) 확인.
...
- 형제 칩은 폭을 실측해 넘치면 쉐브론 버튼 → 같은 레벨 전체 목록 드롭다운(현재 항목 체크, 사용자 지시). 넘칠 때 보이는 칩은 **현재 칩 + 최근에 연 칩**(MRU, "매번 같은 칩이면 의미 없다" 2026-09-19) — 방문 기록을 위치와 함께 영속(`{currentId, recent}`), `orderChipsByRecency`+`layoutChips`(전부 들어가면 원래 순서). 칩 전환은 FLIP 이동+등장 애니메이션(WAAPI 350ms, reduced-motion 존중). 가용 폭은 스트립이 아니라 래퍼 기준 — 버튼 폭만큼 좁아진 스트립으로 재면 넓혀도 못 돌아오는 순환이 생긴다. `lib/framework-drill.ts` 단위 테스트 8종. 위치는 현재 카테고리 id 하나로 영속(`bpm.home.frameworkDrill`, 구 openIds 영속 대체) → 새로고침·검색 복귀·리마운트 시 체인으로 복원. 우측 요약 "직계 하위" 클릭(revealRequest)은 부모 레벨로 이동.
...
- 규모 시드(2026-09-19, dev): `backend/scripts/seed_framework_scale.py` — L1 1개 아래 L5 2,000개(L2 6·L3 37·L4 231, 맵 없음, 고정 시드 난수로 들쭉날쭉한 분포, 멱등 재실행)를 넣어 드릴다운을 실측. `/categories/nodes` 전 레벨 12~30ms, 페이지 에러 0, L3 형제 5개는 칩 3개+드롭다운·L4 형제 8개는 칩 2개+드롭다운으로 접힘. 실행: `DATABASE_URL=… .venv/bin/python -m scripts.seed_framework_scale [--l5 N]`.
- 후속(2026-09-19, dev): 이 라운드의 함정 3종(Tailwind display 유틸 이중 지정, 이펙트 내 setState는 rAF/then/ResizeObserver 콜백으로, 워크트리 세션의 복합 Bash 거부)을 `docs/lessons/react-ts-patterns.md` §3·§9, `browser-verification.md` 함정 5, `frontend/AGENTS.md` gotchas, CLAUDE.md Lessons 줄에 기록.
