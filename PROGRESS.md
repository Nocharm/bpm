...
- **세션 연결 캔버스(dev, 09-23, B4/B5):** relations(엣지 목록)만 편집 진실이라 흐름을 눈으로 고칠 수 없던 것을 `canvas`(노드+엣지) 컬럼으로 승격 — `app/framework_interview/canvas.py`가 `expand_relations_to_canvas`(팬아웃≥2·전부 parallel 아님 또는 자기 반복 → `__branch__{task_id}` 마름모, `consultant_layout.layout_flow` LR 배치, `__start__`/`__end__` 보강; 분기·start/end 규칙은 FE 미리보기 `buildL5PreviewGraph`와 동형)·`collapse_canvas_to_relations`(마름모 접기, 흐름 역행 직결은 loop, 미지 task_id 폐기)·`validate_canvas`의 단일 진실이고 라우터는 호출만 한다. `POST /relations`는 `{comment}`를 받아 `[직전 제안]`+`[사용자 피드백]`을 프롬프트에 실어 재제안하고 canvas를 채우며, `PUT /canvas`(검증·저장, relations 불변)·`PUT /relations`(`{canvas}` 접기 또는 `{relations}`)가 확정 경로. `feedback_log` 컬럼은 노출만(쓰기는 후속).
