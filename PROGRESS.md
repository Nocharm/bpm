...
- W3 재검증(19921742): 0/8 재현 시드로 신선 로드 **8/8회 재핏 발동 + 전 9노드 뷰 내부**(zoom 0.56354→0.523~0.525, 최하단 여유 12.4~15.7px), W4 카메라 불가침도 재확인 — W라운드 최종 6/6 PASS. 트윈 중간 좌표로 fit이 잡혀 회차별 zoom이 0.4% 흔들리나 회귀 아님(관찰 로그 #5).
- W3 픽스: 재핏 소모 플래그를 발사 시점으로 이동 — 분할 도착 오프셋 커밋에 cleanup이 타이머를 취소하고 재예약 없이 끝나던 버그를 마지막 커밋 기준 80ms 디바운스로 교정.
- W라운드 브라우저 검수(docs/qa/node-spacing-qa.md) 5/6 — 라벨 회피·폴백·카메라 불가침·제목 핸들 ✅. **재핏은 FAIL**: yOffsets가 2커밋에 나눠 오면(예: 여러 줄 터미널 + IO 성장) 이펙트 cleanup이 80ms 타이머를 취소하고 one-shot 플래그는 이미 소모돼 재예약되지 않는다(0/8회, 하단 노드 54px 잘림).
...
- 워스트 케이스 브라우저 QA(T1~T8, `docs/qa/node-spacing-qa.md`): 캡/펼침 겹침 0·행 보존·트윈 단조·그룹/PNG 표시 기준·드래그+스왑 저장좌표 무오염·인라인 펼침 중 밀림 해제까지 29/29 통과, 콘솔 0·잔류 0. 하네스 교훈: 드래그 임계용 선이동 3px이 flow 델타를 줌만큼 깎으므로 절대 이동량 대신 "저장 = 표시 − 오프셋" 불변식으로 판정할 것.
- 엣지 우회 브라우저 QA(U1~U6, `docs/qa/node-spacing-qa.md`): 회랑 이탈·무관 엣지 불변·성장 추종·드래그 라이브 재계산·직선 폴백 무회귀까지 13/13 통과, 콘솔 0·잔류 0. 판정 교훈: 타깃이 소스 왼쪽인 right→left 핸들의 RF 기본 smoothstep은 midpoint 단일 회랑이 아니라 `sourceX+20 / targetX−20` 2회랑 Z 형상이라, "우회 해제" 판정은 이 실제 서명과 대조해야 한다.
