...
- home 개선 배치: 새로고침·브랜드로고 클릭 시 검색·필터 초기화(맵→복귀만 유지, reload 감지+로고 stash clear+full reload) · 빈 목록영역 클릭=선택해제·자동 첫선택 제거·상세 플레이스홀더 · "/" 검색 포커스+kbd 힌트+플레이스홀더 축약 · 최근 배지 플로팅(상단경계 안쪽·축소) · 최근밴드 초기 2/+3/접기(2로 리셋, 라벨우측+하단) + "전체 맵" 섹션 라벨 · 캐시 cap 11.
- home 후속: 선택해제 영역을 페이지 전체 여백으로 확대(루트 컨테이너 onClick, 상세패널 stopPropagation) · 최근 배지 폰트 10→11px · 배지 배경/테두리 제거(텍스트만)·상단좌측(top-0 left-1)으로 이동해 타이틀 비겹침.
