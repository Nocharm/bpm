...
- **4 FW 뷰 검색:** `framework-search-groups.tsx` — 히트를 L5 경로별 그룹(헤더 1단=L5 이름 먼저+캔버스 확정/초안 칩+수, 2단=상위 경로 브레드크럼 최대 2줄 `line-clamp-2`, 클릭=검색 해제+드릴다운 이동 `revealCategory`), 미등록은 점선 그룹 맨 뒤(`lib/framework-search-groups.ts`). 한 줄 말줄임에 L5 이름이 잘리던 문제로 2단 분리(사용자 지시).
