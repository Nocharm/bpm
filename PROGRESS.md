...
- **트랙 C 구현 완료(요청·승인·FE 표면·스모크·QA)**: `POST /maps/{id}/slot-changes`가 비관리자 owner에겐 `ApprovalRequest(kind="fw_slot")`을 만들고(중복 대기 409 — 관리자 즉시 적용 경로도 남의 대기 요청 앞에선 409), 다측 승인(`payload.sides/approvals`, move는 양 L5 각 1명·동일인 1회)·거절 알림·철회(`GET/DELETE …/slot-changes/pending`)·인박스/설정 큐/맵 승인 탭/벨(`fw_slot_requested·applied·rejected`)에 노출. 결정권 `can_decide_slot`은 잔여 side 기준(sysadmin 포함, `can_decide_slot_for_map` 공용 헬퍼 — 맵 상세와 승인 목록 열람 공유). FE는 공용 `SlotChangeDialog`(관리자 안내/비관리자 요청, busy 중 닫기 차단, 해제·삭제는 위험 톤) + 배정 모달 대기 배너·철회(조회 실패는 fail-closed), 슬롯 맵 삭제 `SlotDeleteDialog`(후계자 선택), 복사+은퇴 해제/승계 라디오(슬롯 원본은 이름 편집 가능 — 원본이 승인 전까지 이름을 쥐고 있어 409 회피), 옛 체크아웃 보유자의 저장 422는 "canvas changed - reload" 안내, 삭제된 링크 노드도 "Deleted - replace" 슬롯 배너.
- **검증**(QA `docs/qa/2026-09-fw-slot-governance-qa.md` — 기능 26·시나리오 10, 컨트롤러가 실브라우저로 직접 수행): 리셋 DB에서 `pw-smoke-framework-slot.mjs` 20/20 + 임시 Playwright 대본으로 S1~S7·S9·S10 전부 ✅(스크린샷 세션 공유). 검증 중 결함 3건 발견·수정(L5 관리자의 맵 승인 목록 403 → 에디터 패널 공백 / 삭제 노드 범용 CTA / 복사+은퇴 이름 잠금 409). 게이트: pytest 1375·ruff·tsc·lint·vitest 864·build 그린.
- **회귀 스모크 정비·잔여**: 임포트 패널 리팩터(09-03)로 사라진 확인 다이얼로그를 기다리던 framework 스모크 4종의 시드 단계를 `waitForResponse`로 보정. framework-admin 7/7·framework-delegation 18/18 통과. **잔여(별도 정비 필요, dev에서도 실패)**: `pw-smoke-framework`(홈 트리 행 셀렉터), `pw-smoke-framework-canvas`(익명 fetch가 draft 열람 룰에 걸림·워터마크 Title case), `pw-smoke-copy-purge`(시드에 없는 하드코딩 부서).
- **최종 수정 라운드(컨트롤러 리뷰 F1–F6, 12e29862+e5289152)**: 레거시 어댑터(`PUT /category`·`POST /framework-transfer`)가 대기 중 fw_slot 요청 가드 없이 적용되던 결함 → `assert_no_pending_slot_change`를 코어로 승격해 두 어댑터 + `create_slot_change` 공용화. `_apply_request`가 승인~적용 사이 드리프트(카테고리 이동·target 오너십 상실)를 재검증 없이 통과시키던 결함 → sides·target owner 재검증 후 409 "request preconditions changed"(요청은 pending 유지). 이동 요청자가 이미 한쪽 side의 직속 관리자인데도 크레딧을 못 받던 결함 → `build_request_payload`가 생성 시점에 프리뷰의 `satisfied_by_caller`로 approvals 시딩. FE는 배정 모달의 중복 Esc 리스너(자식 `SlotChangeDialog`의 busy 가드 무력화) 제거, 후계자/이양 피커를 `my_role==="owner"` 맵만으로 필터, 복사+은퇴 재시도의 404(self-apply로 이미 소프트삭제된 소스) 처리 추가. 신규 pytest 4종은 가드를 임시로 무력화해 실패하는지 확인한 뒤 복구 — pytest 1383·ruff·tsc·lint·vitest 864 전부 그린. 상세: `.superpowers/sdd/2026-09-06-fw-slot-track-c-approval/final-fix-wave-report.md`.
- ⚠️ **배포 단위**: 트랙 A+B는 C 없이 배포 불가(비관리자 409·삭제 409에 다이얼로그 없음·알림 라벨 미정의) — A+B+C를 한 번에 dev→서버로 낸다. 트랙 D(임포트 플레이스홀더)·E(문서)는 후속.
- **트랙 D+E**: 어댑터 외부 taskId 엣지 보존(external) → 임포터 플레이스홀더 배치(source_node_id = make_node_id("__ext__", code))·재전달 해소(resolve_external_placeholders, draft만) → spec.md §7.x·매뉴얼 6종 갱신.
