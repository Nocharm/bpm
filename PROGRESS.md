...
## 2026-09-03 — 임포트 후속 2~6 진행 (feat/consultant-import-fallbacks)
- 설계 `docs/superpowers/specs/2026-09-03-import-fallback-followups-design.md`(사용자 확정 답변 반영). ④ SP 지정 참고치 `sp_annual_count/sp_fte`(모델·지정 PUT·SubprocessRef·라이브러리·임포트 착지, 노드 인스펙터 Info 호버) ② 인터뷰 원문 메모 5종 — `PATCH /maps/{id}/fallback-notes`(editor), `MapFallbackNotes`(에디터 맵 탭=점유권자 편집·홈 카드·비교 요약 기본 접힘), FallbackHint 빈 값 "추가" 어포던스 ③ 노트 CRUD(맵=오너, L5=체인 관리자/sysadmin, `edited_at`) + `MapNotesSection` 추가/수정/삭제·프리셋 칩·'[' 자동완성·비권한자 안내, 에디터 맵 탭에서 노트를 SP 카드 아래로 이동(L5 캔버스는 카테고리 스코프), 재임포트는 기존 임포트 노트를 `notes` 거버넌스 행으로(수정 없으면 기본 체크·있으면 해제, 사용자 노트 불변) ⑤ SP 지정 모달 재디자인 — `max-w-lg`(672는 시선 이동이 길어 폐기), 필드 타일 2열(시스템·URL·파라미터 7·Input/Output 개수; 라벨 작게·톤다운 + 값 우측 강조, 자리 부족 시만 라벨 생략) → 클릭 위치 팝오버(안내·Σ·원문 메모, Enter/바깥=확정·Esc=취소(모달까지 안 닫힘), IO는 항목 편집기 플라이아웃) ⑥ SP 카드 액션 행 wrap. 원문 메모 행은 행머리 아이콘이 호버 시 노트 아이콘으로 스왑(값 톤다운, 빈 행 비활성, 팝오버 360px). 스모크 `pw-smoke-import-followups.mjs` 23/23.
