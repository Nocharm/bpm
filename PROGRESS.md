...
- **구조**: 상단 3:2 **설명 | 노트(카드형)** — 둘 다 접힘 높이 272px 클립(스크롤바 숨김·하단 페이드·헤더 펼치기/접기, `components/clip-body.tsx`), 헤더는 공용 `SectionHeader`(체브론+아이콘+제목+건수). 하단 전폭 **서브프로세스 정보**(`maps/map-detail-sp-section.tsx`, 순수 로직 `lib/sp-detail.ts`): BPM 속성=부서 트리(1.5/6, 5단↑는 중간 …으로 접음)·담당자 세로 목록(1/6)·3행 높이 클립(셀을 absolute로 채워 fr 행 성장 차단) + 시스템·URL·GMP 스택, 수행 지표 1행 타일 3열, 입출력=결합 타일(5/12, 글머리 목록)+시작/종료 조건(7/12) — 본문은 내용 높이에 맞추고 상한 3줄 클램프(사용자 지시). 폭 <40rem(컨테이너 쿼리 `@[40rem]`)이면 세로 쌓임. 비SP 맵(값·메모 없음)은 섹션 미노출.
