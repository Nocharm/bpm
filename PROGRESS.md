...
- 후속(사용자 요청): **하늘 90% 알파(스모크 글라스)** — 그라데이션 스톱을 `color-mix(… 90%, transparent)`로 바꿔 뒤 흰 거터가 배어들게(상단 53,56,66·하단 73,75,85 실측). 함정 2건: ① background-color 언더레이가 같이 깔리면 알파가 이중 적층돼 상쇄 — 언더레이 제거. ② **Turbopack이 재기동 후에도 이전 CSS를 영속 캐시로 서빙** — `.next` 삭제 후 재기동으로 해소(globals.css 리컴파일 누락 함정의 상위 버전). 검증 21/21(그라데이션 알파 포함 단언).
