...
- 기본 상태에서 시계 칩 전체가 accent-tint 배경이라 그 줄이 "최근 수정"이 아닌 다른 값처럼 읽혔다 — 칩 스타일을 빼고 **시계 아이콘 색으로만** 최근 열람을 표시(텍스트는 다른 카드와 동일하게 `updated_at`).
- 호버 전환을 비대칭으로: 들어올 땐 **1초 지연 후 페이드**(스쳐 지나는 커서에 반응하지 않게), 나갈 땐 `duration/delay 0`으로 **즉시 복귀**.
- 스모크 갱신: 칩 배경 검사 → 아이콘 색 검사(액센트 rgb(106,65,255) + 칩 배경 투명), 지연 구간 중간(400ms) 미전환 검사와 언호버 100ms 내 복귀 검사 추가.
