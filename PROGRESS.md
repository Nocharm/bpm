...
- 워스트케이스 개선 브라우저 검수 M1~M10(`fe7b86bd`, 8✅/2❌·콘솔 에러 0): 체크리스트 3단계·체크 동기 애니(120ms 지연 실측)·디시전 클램프/배지 코너·엣지 라벨 줄바꿈·힌트 4표면은 전부 통과. 미해결 2건 — #3 캡만으론 워스트 노드 높이가 710→628px(-12%)뿐이라 아래 노드와 여전히 겹침(#1 이월 트랙과 동일 원인), #4 인쇄 보정의 `max-width:200px`가 96px flex 부모에 눌려 무효(전문 노출은 클램프 해제로 성립).
