...
- **최종 리뷰 후속 5건 완료**: 필터 결합 경고 주석 양쪽(`api-errors.ts` DETAIL_PREFIX_MAP·settings page 토스트 억제 정규식) + 에디터 `PendingApprovalsPanel` onToast 톤 전달(`item.tone`) + `_normalize_comment` 전방 참조 해소(정의를 `DecisionIn` 위로 이동) + 재승인 no-op 코멘트 폐기 의도 주석 + `was_rejected` 회수 코멘트 테스트(반려 사유 보존 확인) 추가. 게이트: BE 1050/1050·ruff 0, FE lint 0 error·tsc 0·vitest 620/620·build OK.
