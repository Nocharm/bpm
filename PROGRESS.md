...
- **연결 단계 = 편집 캔버스(dev, 09-23, Task 3):** 엣지 표로 흐름을 고치던 연결 단계를 폐기하고 `relations-canvas.tsx`(ReactFlow, 핸들 드래그 연결·엣지 클릭 라벨·우클릭 분기 추가/제거·언급/taskId 복사·자동 정렬/되돌리기 1단계/맞춤)로 교체 — 진입 즉시 `onPropose("")` 1회(오버레이 링 최소 1.5초)·편집은 300ms 디바운스 `PUT /canvas`·우측은 L6 카드 목록 + `FeedbackChat scope="relations"`. 캔버스 엣지엔 핸들 개념이 없어 표시 단계에서 4변 핸들+우→좌로 못 박고(LR 고정), RF 상태를 편집 진실로 두고 구조 변경만 `flowToCanvas`로 캔버스를 만들어 되돌려 받는다. **디바운스 저장 응답은 `applySession`하지 않는다** — 하면 `key={JSON.stringify(session.canvas…)}`가 매 편집마다 바뀌어 300ms마다 캔버스가 리마운트(뷰포트·입력 초기화)된다. 오버레이는 캔버스 도착 또는 호출 종료(busy 해제) 중 먼저 오는 쪽에 걷힌다 — 실패한 제안이 링으로 화면을 잠그지 않게. 새 스모크 `pw-fw-consult-canvas.mjs` 10/10(오버레이·노드 4·피드백 엣지 뒤집기·분기 +1·라벨 저장·PUT 200·등록 도달), 기존 3종은 자동 제안 대기로 이식 12/12·10/10·15/15.
