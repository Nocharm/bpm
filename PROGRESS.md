...
- **최종 리뷰 픽스웨이브 (FE)**: 동봉 체크박스를 3표면 모두 오너에게만 노출(에디터는 기존 `myRole` 재사용 — 로드 전 null이라 pre-load race도 함께 해소, 설정 패널은 `canBundle` prop을 panel→row로 스레딩) + 셀프 게시 팝오버 클램프 높이를 체크박스 유무로 분기(148/112). 게이트: FE vitest 599·lint 0 error·tsc 0·build OK.
- **QA 체크리스트**: `docs/qa/governance-ux-checklist.md` 신설 — 4페이즈 사용자 실검증 항목 + docs/README 인덱스 등록. 픽스웨이브 반영 갱신(A-1 오너 전용·A-1b 비오너 미노출·A-5 차단 표면 확대·A-10 approved 버전 삭제 엣지).
