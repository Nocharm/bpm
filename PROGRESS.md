...
- 형제 열 항목 2행 = **직속 관리자**(없으면 점선 "Unassigned" 플레이스홀더), L5 수는 제거(사용자 지시 2026-09-19). 하위 행(L2~L4)도 같은 문법 — 앞 아이콘→레벨 필, 2줄 관리자, 우측은 서브트리 L5 수 대신 **직계 하위 수 "L{n+1} k"** · 맵 수. BE `/categories/nodes`의 `admin`을 전 레벨로 확장(`_card_meta`, `canvas_state`·`slot_pending_count`는 L5만) — FE/BE 동시 배포.
