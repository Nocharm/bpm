...
- **IO 배열 계약(dev, 09-23, B8a):** 인터뷰 JSON 0.5 `actions[].input`/`output`을 문자열에서 `list[str]`로 승격 — `normalize_row`(`_io_list`, `,`/`\n`/`/`/`·` 분해·trim·중복 제거)·`RowAction.input/output`(`list[str] | str | None`)·`L6_ROW_DRAFTER_CONTRACT` 예시+규칙·역변환 `map_to_row`(개행 문자열→list) 4표면 동기화. 어댑터 `_join_multi`는 이미 list→개행이라 무변경. FE 외부 프롬프트(`interview-json-prompt.ts`)·샘플 md에도 배열 표기 반영.
