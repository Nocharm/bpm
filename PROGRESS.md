...
- **커서 스포트라이트**(`l5-cursor-glow`) — 커서 주변 하늘만 은은하게 밝힌다. 좌표를 state로 올리면 에디터 전체가 매 프레임 리렌더되므로 **ref로 transform/opacity만 직접 쓴다**(리렌더 0). 원은 정적 radial-gradient라 이동이 GPU 합성만 하고 리페인트가 없다 — 커서 위치를 `background-position`으로 옮기면 매 프레임 그라데이션 재래스터라 이 방식이 아니다. 워터마크보다 아래에 깔아 "배경만" 밝아진다. 드릴인 창(index>0) 위 이동엔 반응하지 않음.
- **반경 900→300px·잔상 3겹**(2차 요청) — 좁힌 만큼 코어를 올려 밀도를 유지한다. 잔상은 파티클이 아니라 **transition 속도가 다른 3겹**(80/240/440ms)이 같은 좌표를 각자 속도로 쫓아가는 방식 — 정지 시 겹쳐 코어가 진해지고, 움직이면 뒤로 벌어져 꼬리가 된다(실측: 빠른 이동 중 중심 942/837/770 = 172px 꼬리). 첫 등장 때만 transition을 껐다 되돌려 좌상단(0,0)에서 줄이 그어지는 것을 막는다.
- 실측(마우스 스윕 60틱, 구성별 대조): median 16.7ms / p95 16.7~16.8ms — 구성 무관 동일(면적은 오히려 3×300px < 1×900px). 스윕 중 잡히던 long task 2건은 **글로우와 무관** — 잔상 제거·글로우 제거 구성에서도 로드 후 t≈5.5s에 고정 발생(`unknown:window`, 앱 폴링 추정)해 귀속이 뒤바뀐다.
