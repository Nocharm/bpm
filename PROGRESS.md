...
- Task 2 구현: `app/tokens.py`(자체 HS256 세션 토큰 발급·검증) 신설, `get_current_user`를 `resolved_auth_mode()` 기준 3분기로 재구성(Keycloak RS256 검증은 `_decode_keycloak_token`으로 그대로 이관, 동작 무변경). `AUTH_MODE=ldap`인데 `AUTH_JWT_SECRET`이 비면 위조가 자유로워지므로 `lifespan` 진입 시 기동을 막는다. 리뷰 픽스 1R: HS256 토큰이 keycloak 모드에서 거부되는지·`X-Dev-User`가 ldap·keycloak에서 무시되는지 회귀 테스트 고정 + TTL 단언 강화 + auth.py 모듈 docstring을 3모드 기준으로 갱신(테스트/문서만, 프로덕션 동작 무변경). 리뷰 픽스 2R: keycloak 거부 테스트가 실은 빈 `keycloak_issuer`발 JWKS URL 에러로 우연히 통과하던 가짜-그린이었음을 확인 — `_jwk_client`를 monkeypatch 스텁으로 교체해 실제 `algorithms=["RS256"]` 검사까지 도달시키고, 서명·issuer는 실값과 맞춰 alg만 다른 위조 토큰으로 재작성(fail-then-pass로 알고리즘을 넓히면 실패·원복하면 통과함을 실측 확인).
- Task 4 구현: `is_sysadmin()`이 env(`BPM_SYSADMINS`) 외에 설정 화면 부여분도 보게 — `logic.py`에 모듈 캐시(`_granted_sysadmins`, 단일 uvicorn 프로세스 전제)와 `grant/add/remove_granted_sysadmin`+`load_granted_sysadmins`(기동 시 `local_credentials.is_sysadmin` 로드) 추가, `lifespan`에서 `init_models()` 다음 호출. 게이트 조건을 `not settings.auth_enabled` → `resolved_auth_mode() == "dev"`로 교체 — 안 그러면 `AUTH_MODE=ldap` + `AUTH_ENABLED` 기본값(미설정)에서 로그인한 사람 전원이 sysadmin이 되는 전면 권한 상승이 있었다(아직 토큰 발급 전이라 미노출, 회귀 테스트로 고정). 같은 이유로 `main.py`의 데모 유저 시딩·로그인 시 HR 동기화 게이트도 `resolved_auth_mode()` 기준으로 통일(전자는 dev 전용, 후자는 keycloak+ldap 둘 다). 기존 `auth_enabled` 소비 테스트는 전부 `auth_mode`가 기본값(빈 문자열)이라 구 유도 규칙과 동치 — 회귀 0건(1098→1103, 신규 테스트 5개만 추가).
- Task 6 구현: `POST /api/auth/login` — 로컬 계정(`local_credentials`)을 AD보다 먼저 확인해 컨설턴트 비밀번호가 AD로 새지 않게 하고, 없으면 `ad.client.authenticate_user` 폴백. `app/login_throttle.py`(프로세스 메모리, `login_id|client_ip` 키, 5회/5분)로 앱이 AD bind를 무제한 중계해 실계정을 잠그는 경로를 막음 — 스로틀 소진 시 자격증명 검사 전에 429. 실패는 사유 불문 401 하나로 통일하고 `login_records`에는 남기지 않음(사용 현황 집계 오염 방지). 전수검토에서 지적된 미사용 `AuthModeOut` 모델 제거. 신규 테스트 8개, 전체 1118 그린.
- Task 6 리뷰 픽스 1R: `verify_password`(scrypt)·`ad_client.authenticate_user`(블로킹 LDAP)를 `asyncio.to_thread`로 감싸 이벤트 루프 정지 방지(AD 무응답 시 전체 프로세스가 멎던 문제). 스로틀 dict가 실패 0건 키도 영구 보관하던 무한 증식 버그 수정(`recent` 비면 `pop`). 테스트 파일의 `Employee` 시드가 session-scope DB에 영구 잔류하던 것도 정리(생성한 id만 추적 삭제). 경계 단언(5회째=401, 6회째=429) + 성공 시 리셋 테스트 추가. 신규 테스트 9개, 전체 1119 그린.
- Task 7 구현: `POST/GET/PATCH/DELETE /api/admin/local-accounts` — sysadmin 전용·ldap 모드 한정(`_ensure_ldap_mode` 404 게이트) 로컬 계정 CRUD. loginId가 AD 소유(`source != 'local'`) 또는 기존 credential과 충돌하면 409. sysadmin 부여·회수·삭제 모두 `logic.add/remove_granted_sysadmin` 캐시에 반영. 브리핑 픽스처가 ldap 모드에서 `X-Dev-User`로 액터를 잡으려 했으나 `get_current_user`는 ldap 모드에선 Bearer만 신뢰해 401이 나는 구조라, 테스트는 `tokens.create_access_token`으로 실제 토큰을 발급해 프로덕션 인증 경로를 그대로 태움. `logic.is_sysadmin()` end-to-end 단언과 `load_granted_sysadmins`(기동 시 DB 로드) 실행 경로도 신규 테스트로 커버. 신규 테스트 7개, 전체 1126 그린.
- Task 8+9 구현(프론트): `NEXT_PUBLIC_AUTH_ENABLED` 빌드타임 상수를 걷어내고 `GET /api/auth/mode`를 부팅 시 묻는 런타임 분기로 교체 — `lib/auth-mode.ts`(fail-closed to keycloak) + 모듈 캐시 `getCachedAuthMode()`(Providers·로그인 페이지·top-nav가 공유, 중복 fetch 없음). 로그인 화면은 모드당 CTA 1개(keycloak 버튼/`LdapLoginForm`/dev 피커)만 그리고 죽은 보조 Keycloak 버튼은 삭제, silent 자동 로그인은 keycloak 모드로만 게이팅. `LdapGate`(providers.tsx)를 DevGate와 같은 골격으로 추가하고 `lib/ldap-session.ts`가 토큰을 `{token, expiresAt}`로 저장 — 만료는 클라에서 조용히 소거, 서버가 401로 거부하면(`publishMe`에 `onUnauthorized` 콜백 추가) 저장분을 지우고 `/login`으로 되돌려 루프를 막는다. top-nav 로그아웃도 모드별 3분기(keycloak SSO/ldap 토큰 삭제/dev)로 재구성.
- Task 10 구현(프론트): 설정 콘솔 "Local Accounts" 탭(`components/admin/local-account-table.tsx`) — Directory 카테고리에 얹고 `getCachedAuthMode()`로 ldap 모드에서만 노출(sysadmin 게이트는 카테고리 access가 기존에 담당). 생성 폼+sysadmin 토글(env 지정분은 체크·비활성)+인라인 비밀번호 재설정+`ConfirmDialog` 기반 삭제 확인. `deptCode`는 EDW `departments.dept_code`를 참조하는데, 기존 부서 피커(`DeptTreePicker`)는 org-path 문자열이 키라 재사용 불가(dept_code 조회 API 자체가 없음) — 자유 텍스트 입력으로 낮춤, 후속 필요 시 조회 엔드포인트 추가 검토.
