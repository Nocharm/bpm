...
- Task 2 구현: `app/tokens.py`(자체 HS256 세션 토큰 발급·검증) 신설, `get_current_user`를 `resolved_auth_mode()` 기준 3분기로 재구성(Keycloak RS256 검증은 `_decode_keycloak_token`으로 그대로 이관, 동작 무변경). `AUTH_MODE=ldap`인데 `AUTH_JWT_SECRET`이 비면 위조가 자유로워지므로 `lifespan` 진입 시 기동을 막는다.
