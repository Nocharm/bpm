...
- **최종 픽스웨이브**: 스택 Save 부분 실패를 fatal error 채널 대신 dismissible 인라인 배너(`stagedSaveError`)로 분리 — 카드 전체를 죽이지 않고 X로 닫고 계속 편집 가능 · `humanizeApiError` 미적용 catch 3곳(체크아웃 폴·`runTransition`·패널 `runAction`) 인간화 문구로 교체 · 고아 i18n 키 `perm.toastGatedBy`/`perm.approversNone` 제거(사용처 0 확인) · QA 체크리스트 R2-2 문구를 실제 pill 동작(선택 전 무선택·현재쪽 "· current" 라벨)에 맞춤. 게이트: FE vitest 617·lint 0 error·tsc 0·build OK(백엔드 무변경).
