...
## 2026-08-31 — L5 캔버스 차콜 배경 + 태그 토글 (dev)
- L5 연계 캔버스 기본 배경을 **차콜(`--color-canvas-l5` #363843)**로 — 회색 4안+파격 4안 목업 비교 끝에 사용자 선정. hue/명도 파격으로 "지금 L5를 보고 있다"가 단독 화면에서도 즉시 인지되고, 파스텔 노드가 무대 위처럼 도드라진다. 도트(#5f6170)만 함께 교체, 엣지·노드·미니맵 무변경(라이트 칩·라벨은 자체 배경이라 대비 유지).
...
- 후속 2(사용자 제안): **도트 그리드 제거 + 모드 링** — 차콜 L5는 무늬 없는 민 무대(Background 미렌더, `--color-canvas-l5-dot` 토큰 폐기). 라이트 토글은 도트·풀블리드 그대로. (Tailwind preflight가 border-style을 solid로 깔아 보더 검증은 폭 기준.)
- 후속 3(사용자 지시): 프레임 제거 + 흰 모드 링(풀블리드) — 이후 리셋 라운드에서 폐기.

## 2026-09-01 — L5 최종 디자인 "새벽 조감도" (feature/l5-dark-shell)
- 여덟 라운드의 시행착오 끝에 **브레인스토밍으로 리셋**: 목표 이미지를 처음으로 직접 물어 "전략 조감도 × 새벽 그라데이션 × 배경+마감 디테일"로 확정. 표면 재도색·질감 실험이 계속 어긋난 이유는 "어떤 인상이어야 하는가"를 정하지 않아서였다.
- 구현: 흰 거터 위 라운드 뷰포트(프레임 복귀·모드 링 제거)에 **남색(#1E222D)→차콜(#363843) 세로 그라데이션 하늘**(`.bpm-l5-sky` — 소스 클래스명이라 Turbopack purge 안전, 토큰 `--color-canvas-l5-sky` 추가). 별빛·비네트는 "임원 보고용 플랫" 피드백으로 제외 — 그라데이션만으로 깊이를 만든다. 도트 없음·칩 20px 인셋·Moon/Sun 토글·영속·일반 맵 무영향 유지, ScopePreview도 sky 동조.
- 검증 `pw-verify-l5-canvas-bg.mjs` 22/22(그라데이션 present·라이트는 none·프레임 11px·풀블리드 복귀) + 픽셀 실측(상단 30,34,45 → 하단 53,55,66·거터/크롬 화이트). 게이트 tsc 0·lint 0·vitest 807·build OK.
- 후속(사용자 요청): **하늘 알파 스모크** — 그라데이션 스톱을 `color-mix(… X%, transparent)`로 바꿔 뒤 흰 거터가 배어들게(90%로 시작 → "톤 더 낮추자"로 94% 확정). 함정 2건: ① background-color 언더레이가 같이 깔리면 알파가 이중 적층돼 상쇄 — 언더레이 제거. ② **Turbopack이 재기동 후에도 이전 CSS를 영속 캐시로 서빙** — `.next` 삭제 후 재기동으로 해소(globals.css 리컴파일 누락 함정의 상위 버전).
- 후속 2(사용자 요청): **브랜드 워터마크** — 처음 대형 도장 1개로 넣었다가 "작은 항목 반복+교차"로 재설계: "SAMSUNG BIOLOGICS"·"Business Process Map"·Workflow 플랫 아이콘을 사선(-24°) 타일링(14행×5열, 홀수 행 오프셋, 화이트 6%, 노드 아래·뷰포트 고정, `l5-brand-watermark`). 브랜드 가이드의 모노크롬/리버스 적용 관례 준수, 실 로고 SVG 제공 시 교체 여지.
- 후속 3: **알파는 "더 투명=더 밝게"가 의도**(94%로 어둡게 간 것은 오독) → 80% 확정(상단 75,79,87 실측). 그라데이션 체감을 위해 상단 토큰을 진짜 네이비 `#1B2743`으로(상단 73,82,104 — 블루 캐스트 뚜렷). React Flow 우하단 어트리뷰션은 공식 `proOptions.hideAttribution`으로 숨김. 검증 25/25. ⚠️ 워터마크 "캔버스 밖 누출"로 보였던 것은 이미지 판독 오류 — DOM 실측(부모 overflow:hidden·rect=캔버스)으로 누출 불가 확인.
- 후속 4(사용자 요청): **L5는 라이트 토글에서도 프레임 유지** — 프레임 조건을 `l5Charcoal`→`isFrameworkMap`으로 올려 채움만 하늘↔라이트 교체, 태그/칩 20px 인셋도 상시 고정 → 다크/라이트 전환 시 버튼 위치가 안 움직임. 검증 25/25(라이트 프레임 11px·태그 20px 유지).
- 후속 5-α(사용자 톤·밀도 조정): 워터마크를 **2종 교차**(SAMSUNG BIOLOGICS ↔ Workflow 아이콘+Business Process Map 한 덩어리)로 줄이고 간격 확장(gap 24/28·10행×4열) — 밀도 완화. 하늘 알파는 80%↔94% 사이 **86%** 절충. 검증 27/27.
- 후속 5(사용자 리포트): **L5 인스펙터의 SP 지정 카드 숨김** — L5 캔버스에서도 `SubprocessInspectorCard`(Not designated)가 떠서 지정 시도→서버 422("cannot be designated")로 혼동 유발 → framework 맵이면 슬롯 자체 미렌더(노드 선택 시 L6 등록요청 CTA는 정상 기능이라 유지). 홈 목록 건 재검토 결과: **L5 제외는 `lib/word-map-home.ts splitMapsByMode`의 `mode==="framework" continue`가 정답**(설계 §9 "트리 L5 행으로만 진입") — 직전 답변("숨김 필터 없음")은 이 헬퍼를 못 찾은 오류. 단 검색 리스트(filteredMaps)는 이 분리를 안 거쳐 검색 시엔 L5가 노출됨(현행 유지, 원하면 제외 가능). 검증 27/27(L5 카드 무·일반 맵 카드 유).
