...
- home 개선 배치: 새로고침·브랜드로고 클릭 시 검색·필터 초기화(맵→복귀만 유지, reload 감지+로고 stash clear+full reload) · 빈 목록영역 클릭=선택해제·자동 첫선택 제거·상세 플레이스홀더 · "/" 검색 포커스+kbd 힌트+플레이스홀더 축약 · 최근 배지 플로팅(상단경계 안쪽·축소) · 최근밴드 초기 2/+3/접기=섹션라벨 우측 쉐브론(전부 닫힘, 라벨 행 클릭=2개부터 재펼침) + "전체 맵" 섹션 라벨 · 캐시 cap 11.
