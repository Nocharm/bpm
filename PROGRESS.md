...
- 피크 레이아웃 피드백 반영: 크기 창 비례(폭 40vw·미리보기 32vh, 상하한 클램프) — 고정 높이 폐기, 등록정보는 미리보기 아래→우측 컬럼, 빈 값은 짧은 대시(-).
- 피크 2차 피드백: Add 버튼→헤더 상시 노출·게시본 표기→미리보기 워터마크, 등록정보=아이콘+필 태그(라벨은 툴팁), 좌측에 "추가될 노드" SP 목업(캔버스 정본 색), ScopePreview에 타입 기본색 적용(resolveNodeStroke 재사용 — 조상 창·요약 모달도 실캔버스 색으로 통일).
- 피크 3차 피드백: 우측을 탭(노드/상세)으로 재편 — 목업 최상단 이동·미리보기가 좌측 전폭, 상세=아이콘+라벨+값 필 3요소, L5 캔버스의 타 L5 출신 목업은 캔버스 규칙(홈 L5 색+출처 배지) 선반영, 목업 클릭=추가/해당 맵 이동 드롭다운(이동은 openMapPrompt 확인 게이트 재사용).
- 피크 4차 피드백: 목업=전체 파라미터 버전(속성 3종+SP 파라미터 5종 칩, PARAM_ICON/formatParamValue 미러), 호버 시 현재 맵 노드 표시 설정 기준으로 스왑(캡션 안내), 클릭 드롭다운 유지. BE 라이브러리 목록에 SP 파라미터 4종(touch_time·cost_krw·cost_usd·headcount) 동봉 — 미지정 마스킹 동일(테스트 확장), 상세 탭도 8행으로 확장.
- 피크 5차 피드백: 상세 탭 — 빈 값 행도 나열하되 톤다운(opacity-40)+대시, 메타 섹션 추가(게시 버전 마커 v번호·라벨, 게시 시각=published 이벤트 KST, 업무 체계=전체 레벨 경로/없으면 톤다운 대시). 메타는 getMap 재사용하되 locked 확정 시 요청 생략(403 콘솔 스팸 방지).
- 피크 6차 피드백(누락 정정): SP 지정의 조건·인풋/아웃풋·GMP를 목업(전체 파라미터·호버 필터 동일)과 상세 탭(GMP 신호등 배지·조건 필·IO 항목 전부 나열, 빈 값 톤다운 대시)에 추가 — 캔버스 미러(Play/Flag·LogIn/LogOut·getGmpBadgeStyle/formatGmp), 소스는 getMap(잠금 시 대시).
- 피크 레이아웃 고정: 본문 행 높이=미리보기 높이(32vh) 고정 + 우측 컬럼 overflow-hidden/내부 스크롤 — 우측이 길어도 패널이 안 늘어나 미리보기 아래 공백 없음.
- 목업 IO 디자인 정정(사용자 지적): 단순 아이콘+텍스트 줄 → 실노드 NodeIoDetails 체크리스트 박스 미러(흰 박스·▸ INPUT/OUTPUT (N) 대문자 헤더·체크박스 슬롯 들여쓰기·단일 항=헤더 생략+아이콘 슬롯), 조건 줄도 실노드 규격(text-xs·12px 아이콘)으로.
