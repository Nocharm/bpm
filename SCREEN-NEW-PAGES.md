...
| S2b | 피드백 | **프론트 피드백 진입** — `lib/api.ts`(submit/list/updateStatus + 타입) + i18n `feedback.*` + **TopNav 피드백 버튼**(MessageSquare·accent) + **`feedback-side-panel.tsx`**(우측 슬라이드인·유형 세그먼트 한 행·본문·현재 route/맵# 자동첨부·제출 토스트·"모든 피드백 보기"→`/feedback` placeholder). 시안 `피드백 버튼위치.png` | lint/build✅ | 패널 렌더·열림·세그먼트·자동첨부 스크린샷✅ / 제출 e2e=인증+백엔드 | ⏳ | (this) |
