...
| S5c | 공지/피드백 | 표·모달 폴리시 묶음(사용자) — 공지 표: **아코디언 펼침/접힘 애니**(grid-rows)·에디트/삭제 **아이콘+툴팁+행호버 노출**·**20개 페이징**. 피드백 표: **20개 페이징**·등록일 **시간까지**. 피드백 모달: 메타 값 **필**·상태탭 **슬라이딩+상태별 색상**. 공용 `components/pagination`. | lint/build✅ | 아이콘/툴팁/호버·날짜필·모달 슬라이딩(작업중 amber)·메타필·피드백 시간 스크린샷✅ | ✅ | (this) |
