...
| S5d | 공통 UI | 아이콘 필 디자인 통일(사용자) — 공용 `icon-pill-filter`(비활성 아이콘·활성 라벨 우측 펼침): **S1 TopNav 탭**·**피드백 유형 필터**·**공지 필터** 적용. 피드백 표: 고정폭(유형/작성자/상태/등록일)·내용 반응형 truncate·작성자 필·**등록일 날짜/시간 2필**. 피드백 모달 버튼 **아이콘화**(수정/저장/답글/삭제). | lint/build✅ | 탭 확장·아이콘필터·표 필·모달 아이콘 스크린샷✅ | ✅ | (this) |
