...
| S4b | 공지 | 뷰어 폴리시(사용자) — 목록 메타 **시간 노출**(MM-DD HH:mm)·**제목+내용 첫줄** 전 항목 노출(읽음 포함)·선택 카드 **강조선 제거**+곡선 최소(rounded-xs, 손톱형)·**바깥 테두리 제거**(홈 맵 리스트처럼, 내부 구분선만). | lint/build✅ | 시간·첫줄·선택·무테두리 스크린샷✅ | ✅ | (this) |
