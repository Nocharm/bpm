...
| S3 | 피드백 | **전체 페이지 `/feedback`** — 집계 카드(전체/내 accent/작업중/완료 %) + 유형 필터·검색 + 목록(유형 뱃지·내용 truncate·작성자·상태·등록일) + **관리자 상태변경**(sysadmin `<select>`, 비-sysadmin은 읽기 뱃지). counts·filtered는 렌더 계산(React Compiler). + 공유 스토어 `lib/feedback-panel.ts`(TopNav 패널을 어느 화면 버튼이든 오픈). 시안 `피드백 화면.png` | lint/build✅ | 렌더·집계·유형뱃지·sysadmin select·페이지 버튼→패널 스크린샷✅ / 상태변경=API PATCH+페이지 반영(작업중1·완료1 25%) 검증 | ✅ | (this) |
