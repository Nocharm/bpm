...
| S10 | 대시보드 | **설정 '분석 > 대시보드' 진입 카드 스텁**(catAnalytics 신설) — "운영 대시보드 열기" 카드(LayoutGrid·부제·→)+"세부 지표 추후 보완" 각주, 클릭 시 상세 자리표시(설정으로 돌아가기·추후 제공 안내). 상세 지표는 별도 spec. `settings/dashboard-panel`. 시안 `대시보드 진입점.png`(4b: 전환·돌아가기·추후) | lint/build✅ | 진입 카드·클릭 전환·돌아가기 스크린샷✅ | ✅ | (this) |
