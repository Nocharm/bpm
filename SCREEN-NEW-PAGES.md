...
| S5c | 공지/피드백 | 표·모달 폴리시 묶음(사용자) — 공지 표: **아코디언 펼침/접힘 애니**(grid-rows)·에디트/삭제 **아이콘+툴팁+행호버 노출**·**20개 페이징**. 피드백 표: **20개 페이징**·등록일 **시간까지**. 피드백 모달: 메타 값 **필**·상태탭 **슬라이딩+상태별 색상**. 공용 `components/pagination`. | lint/build✅ | 아이콘/툴팁/호버·날짜필·모달 슬라이딩(작업중 amber)·메타필·피드백 시간 스크린샷✅ | ✅ | 780821d |
| S5d | 공통 UI | 아이콘 필 디자인 통일(사용자) — 공용 `icon-pill-filter`(비활성 아이콘·활성 라벨 우측 펼침): **S1 TopNav 탭**·**피드백 유형 필터**·**공지 필터** 적용. 피드백 표: 고정폭(유형/작성자/상태/등록일)·내용 반응형 truncate·작성자 필·**등록일 날짜/시간 2필**. 피드백 모달 버튼 **아이콘화**(수정/저장/답글/삭제). | lint/build✅ | 탭 확장·아이콘필터·표 필·모달 아이콘 스크린샷✅ | ✅ | 094dcaa |
...
| S6c | 공통 | 3페이지 타이틀 정렬 통일(사용자) — 맵/노티스/인박스 타이틀 동일 크기(text-body-strong)·동일 왼쪽위 기준점(mx-auto max-w-[80rem] 페이지 헤더). 노티스 타이틀+안읽음을 aside→페이지 헤더로 이동. | lint/build✅ | 타이틀 정렬 스크린샷✅ | ✅ | 4dd97df |
| S6d | 공통 | 타이틀 확대·정렬(사용자) — 3페이지 타이틀 text-body-strong→**text-tagline**. 노티스/인박스 콘텐츠(필터·카드) **좌단을 컨테이너 좌단**(맵과 동일)으로. 인박스 **탭을 타이틀 아래**로 이동. | lint/build✅ | 타이틀 확대·좌단 정렬·탭 위치 스크린샷✅ | ✅ | (this) |
| S6e | 공통 | 검색창·좌폭 통일(사용자) — 공용 `search-box`(맵 목록 디자인) + `use-slash-focus`("/" 포커스). 노티스/인박스 aside 상단(필터 위) 배치, **초성 검색**(`filterByQuery` — 노티스 title+body, 인박스 message). 좌폭 맵과 동일(aside `min-w-[18rem] flex-1` · 상세 `flex-[2]`). i18n `notices/inbox.searchPlaceholder`. + 피드백 패널 "화면 자동첨부" 안내문구 삭제(기능 없음, 제출 컨텍스트는 유지). | lint/build✅ | 노티스 초성("ㅍㄹㅅㅅ"→프로세스맵)·인박스 검색/좌폭·피드백 문구삭제 스크린샷✅ | ✅ | (this) |
| S6f | 공통 | 카드 시각 필·검색 삭제버튼·인박스 탭 이동(사용자) — 공용 `time-pills`(과거 2주 이내 상대 1필 / 그 외 날짜YYYY-MM-DD·시각 2필, nowMs 주입) 노티스·인박스 카드 적용. SearchBox: 검색어 有→**전부삭제 X**, 無→**클릭가능 "/" 버튼**(입체감·hover) + `dataId`. **홈 검색창도 SearchBox로 통일**(세 탭 동일). 인박스: 검색창 헤더 아래로 올림 + **승인대기/알림 탭을 All/안읽음 필터 행 우측정렬**(모두읽음=헤더 우). | lint/build✅ | 홈 삭제버튼·공지 상대시간·인박스 검색상단/탭우측/승인대기 안내 스크린샷✅ | ✅ | (this) |
| S6g | 공통 | 이름 우선 필·날짜 필 재활용·검색 통일(사용자) — 공용 `useDirectory`+`UserPill`(이름 필+호버 카드, id 보조): 인박스 승인/공지/피드백의 사용자 표시를 **아이디→이름 필**로. 평문 날짜를 전부 **TimePills**로(인박스·공지·매뉴얼·피드백 모달; MetaRow ReactNode화). 매뉴얼 검색을 **SearchBox(`/`)**로 통일(onEnter/className prop). **인박스 승인 대기 탭에도 검색창**(제목·맵·요청자 필터). | lint/build✅ | 승인 요청자 이름 필·상대시간·승인탭 검색·공지 이름 필·매뉴얼 / 검색 스크린샷✅ | ✅ | (this) |
| S7 | 인박스 | **승인 대기 탭** — `GET /api/inbox/approvals`(`routers/inbox.py`, 세 출처 집계: 버전승인 내담당·checkout 이전요청·권한/가시성 승인요청, kind 구분) + `InboxApprovalOut` + `test_inbox.py`(4). 프론트: `listInboxApprovals`, 승인 큐 카드(유형 필·제목·맵·요청자·시간) + 상세 승인/반려(kind별 기존 함수 재사용, 버전은 반려사유 필수)·탭 배지·승인탭 검색/필터 숨김. 시안 `Inbox Page.html` | ruff·pytest 4/4·398✅·lint/build✅ | 카드·상세·배지·승인 e2e(큐 제거) 스크린샷✅ (반려=사유게이팅 시각+엔드포인트 테스트) | ✅ | (this) |
| S8a | 매뉴얼 | **백엔드 `ManualDoc`** — 모델(단일행 id=1 upsert·format·content·updated_at/by) + `routers/manual.py`(`GET /api/manual` DB 우선·`manual.md` fallback / `PUT` sysadmin upsert) + `ManualOut`/`ManualUpdate` 스키마 + main.py 등록 + `test_manual.py`(3). | ruff✅·pytest 3/3·전체 401✅ | (백엔드) | ✅ | (this) |
| S8b | 매뉴얼 | **뷰어 `/manual`** — 좌 TOC(H2/H3 파생·클릭 스크롤·활성 강조) + 우 `MarkdownView`(코드복사 내장). 헤더: 타이틀+배포포함/업데이트 배지 · 본문검색(`Ctrl/⌘+K` 포커스·Enter 순환 점프+플래시) · 읽기폭 토글 · **본문 한정 읽기 테마**(라이트 전용·토큰 제약상 다크 대신 elevated warm paper 패널). api `getManual` · i18n `manual.*` · TopNav BookOpen 진입(로그인). html 포맷 렌더는 S9(DOMPurify)로 이연. 시안 `메뉴얼 뷰 화면.png` | lint/build✅ | TOC 스크롤·검색 점프·읽기폭/테마·nav 스크린샷✅ | ✅ | (this) |
| S9 | 매뉴얼 | **설정 '매뉴얼' 편집·게시 탭**(sysadmin, 콘텐츠 카테고리) — 포맷 세그먼트(마크다운/HTML)·`.md` 업로드·배포본 불러오기(`GET ?bundled=true`)·**미리보기**(실시간 없음, 토글)·게시(`putManual`). 편집→게시→`/manual` 뷰어 반영. **HTML 렌더**: `dompurify`+`html-view`(sanitize)로 포맷=html 뷰어·미리보기(script/onerror 제거 확인). 뷰어 TOC는 헤딩 없으면 숨김. `settings/manual-manage-panel`. 시안 `New Screens.html`(3c) | ruff·pytest 403(+bundled)·lint/build✅ | 편집기·포맷/툴바·미리보기(md)·HTML sanitize e2e 스크린샷✅ | ✅ | (this) |
| S10 | 대시보드 | **설정 '분석 > 대시보드' 진입 카드 스텁**(catAnalytics 신설) — "운영 대시보드 열기" 카드(LayoutGrid·부제·→)+"세부 지표 추후 보완" 각주, 클릭 시 상세 전환(설정으로 돌아가기). `settings/dashboard-panel`. 시안 `대시보드 진입점.png`(4b: 전환·돌아가기·추후) | lint/build✅ | 진입 카드·클릭 전환·돌아가기 스크린샷✅ | ✅ | (this) |
| S10a | 대시보드 | 상세 접속자수 실데이터(사용자: 나머지 칸·미세디자인·값은 이후 구상, **우선 접속자수**) — 상세 자리표시 → **운영 현황 3 지표 카드**(접속자수=고유 login_id·총 로그인·최근 7일). 백엔드 `GET /api/dashboard`(sysadmin·`login_records` 집계)·`DashboardMetricsOut`·`test_dashboard`(2)·api `getDashboard`. | ruff·pytest 405·lint/build✅ | 상세 지표(접속자수 5·총 7·7일 7)·돌아가기 스크린샷✅ | ✅ | (this) |
