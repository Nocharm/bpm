...
| R7a | 노드 편집 모달 | **모달 재스타일**(`NodeSummaryModal`) — 헤더 "노드 편집"(editor.nodeEdit·SquarePen)·body에 **제목/설명(신규, `NodeData.description` 프론트 배선=백엔드 무변경)**·유형·**색상(노드 타입별 프리셋 `colorsForType` + 커스텀 헥사 #RRGGBB 입력란·미리보기)**·BPM 유지·푸터(Esc=취소/⌘S=저장 힌트 + 취소·저장 버튼). **버퍼 편집**(사용자 결정: 저장 눌러야 반영·취소/Esc/바깥클릭=폐기·노드 바뀌면 버퍼 리셋). **더블클릭 충돌 없음**(기존: 일반=모달·서브프로세스=드릴·타이틀=리네임). 코멘트/하위프리뷰 보존. 이미지 `group-context-and-node-modal.png`(모달부) | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
| R7b | 노드 편집 모달 | **선행/후행 클릭 내비** — 텍스트 → 클릭 가능한 노드 칩(←선행/후행→ 2열 박스), 클릭 시 `onNavigate`→setSummaryNodeId로 그 노드 편집 전환(predecessors/successors를 `{id,label}[]`로 확장). **미저장 변경 시 확인 오버레이**(저장하고 이동/저장 안 함/취소·Esc는 오버레이부터). 신규 i18n 4키. | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
