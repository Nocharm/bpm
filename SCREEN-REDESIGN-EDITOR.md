...
| R8a | 그룹 | **타이틀바 + 박스** — `GroupTitleBar`를 그룹색 **pill**(그립·색 점→팔레트·이름·**연필 리네임**·일괄편집, 밝은 콘텐츠)로, `GroupBox` 외곽선 **dashed**(`strokeDasharray 5 4`). 색 팔레트에 현재색 선택 링. 이미지 `group-bulk-edit.png`(타이틀바부) | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
| R8b | 그룹 | **멤버 일괄 편집 모달**(`GroupBulkModal`) — 부서/담당자 드롭다운·저장 재스타일. 이미지 `group-bulk-edit.png`(하단부) | — | — | ⏳ | — |
