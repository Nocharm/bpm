...
| R8b | 그룹 | **멤버 일괄 편집 모달**(`GroupBulkModal`) 재스타일 — **적용 버튼 accent primary**(목업 바이올렛)·그룹이름/색상/속성 섹션 사이 **구분선**으로 카드형 구조화·**담당자/부서=노드와 동일 SearchSelect 피커**(eligible 로드)·**충돌 옵션 아이콘 2×2 그리드**(4개 완전 균일=nowrap+중앙정렬·아이콘 18px·직관 아이콘 Replace/Plus/SkipForward/MousePointerClick). 충돌 처리·개별 마법사·전 기능 보존. 이미지 `group-bulk-edit.png`(하단부) | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
