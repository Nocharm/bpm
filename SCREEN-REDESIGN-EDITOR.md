...
| R5a | 우측 인스펙터 | **탭 바(속성/맵/승인/활동) + 속성 탭**(빈상태·노드·엣지). `inspectorTab` 상태·접힘 시 재오픈 탭·폭 330. `InspectorPanel`+`TabProperties` 추출, page.tsx `6195–6450` 폼 배선 보존. 이미지 `inspector-properties-empty/node/edge.png` · **R5a 완료**: 신규 `inspector-panel.tsx`(탭바+**속성 빈상태**+맵요약) + **노드 폼**(제목/유형/색상/BPM 카드) + **엣지 폼**(소스→타겟·분기라벨·라벨·스타일·삭제) via `propertiesSlot`. **OLD와 우측 2배 나란히 비교**(4탭 후 OLD 제거). 보류: 설명·댓글·특수필드(OLD 유지)·터미널 라벨 "—"·연결 면 단순화. 단일 브랜치 통합 완료 | tsc/lint✅ | ✅자가 | ⏳ R5a 검토대기(노드/엣지 폼·빈상태) | R5a |
