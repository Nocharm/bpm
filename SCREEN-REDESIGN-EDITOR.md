...
| R4 | 좌측 사이드바 + **편집 툴바** | **R4a 편집 툴바(두 번째 상단바)** — 노드/정렬은 사이드바가 아니라 **메인 상단바 아래 둘째 바**에 편집기능 위주로, **편집모드(!readOnly)일 때만**. `editor-toolbar.tsx`: **＋Node 메뉴**(`add-node-menu`: 프로세스/판단/시작끝/하위프로세스→라이브러리)·**Auto layout**(dagre)·**정렬 4 + 분배 2**(기존 핸들러 재사용). page.tsx 헤더 아래 게이팅 배치. **남은 R4**: 단축키 카드(맥락 반응)·노드 검색 사이드바 이전·아웃라인 재스타일. `editor-left-sidebar.tsx`. 이미지 `topbar-add-node-menu.png`·`editor-overview.png` · 병렬 브랜치 `feat/editor-r4-left-sidebar`(R3 검토 병행) | R4a tsc/lint✅ | R4a ✅자가 | ⏳ R4a(편집툴바) 검토대기·나머지 후속 | R4a |
