...
| R6a | 컨텍스트 메뉴 | **컴포넌트 공통**(`context-menu.tsx`·openMenu 로직 보존) — 패널 라운드 `rounded-md`·여백 `py-1.5` 통일 + danger(삭제) 빨간 kbd 칩(`error` 틴트). 전 메뉴 즉시 반영, 내용/동작 무변경. | lint/build✅ | ⏳ | 🔧 반영(검토대기) | 6a8f13d |
| R6b | 컨텍스트 메뉴 | **캔버스(pane) 메뉴** — 행별 Lucide 아이콘 부여 + PNG 내보내기 `기타›`→**최상위 승격(⌘E)**. 이미지 `context-canvas.png` | — | — | ⏳ | — |
