...
| R9c | 기타 모달/오버레이 | **`node-summary-modal.tsx` 토큰 폴리시** — **`font-medium`(weight 500·규칙 위반) 3곳 제거**(대표 모달 accent 버튼은 weight 클래스 없음=400) + 인라인 `boxShadow:var(--shadow-lg)` 3곳→`shadow-lg` 클래스(대표 모달과 동일 방식, 시각 변화 없음). raw hex 없음 확인. 동작·구조 보존. | lint/build✅ | ⏳ | ⏳ | (this) |
