...
| R6b | 컨텍스트 메뉴 | **캔버스(pane) 메뉴** — 노드타입 4행 Lucide 아이콘(process/decision/start/end=Square/Diamond/Circle/CircleDot, add-node-menu 동일 매핑·신규 `NODE_TYPE_ICONS`) + PNG 내보내기 `기타›`→**최상위 승격**(Download 아이콘, 라벨 `Ctrl+⇧E`=실제 전역키 유지). 전체선택·라이브러리추가·노드추가 통합은 신규동작이라 미포함. 이미지 `context-canvas.png` | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
| R6c | 컨텍스트 메뉴 | **노드 메뉴** — 행별 아이콘 + **이름변경(F2**, 기존 `startRename`) + 색상 인라인 스와치→**서브메뉴(›)**. 이미지 `context-node.png` | — | — | ⏳ | — |
