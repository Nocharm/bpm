...
| R8a | 그룹 | **타이틀바 + 박스** — `GroupTitleBar`를 그룹색 **pill**(그립·색 점→팔레트·이름(더블클릭 리네임)·일괄편집, 밝은 콘텐츠)로, `GroupBox` 외곽선 **dashed**(`strokeDasharray 5 4`). 색 팔레트에 현재색 선택 링. (연필 버튼은 사용자 요청으로 제거.) 이미지 `group-bulk-edit.png`(타이틀바부) | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
| R8b | 그룹 | **멤버 일괄 편집 모달**(`GroupBulkModal`) 재스타일 — **적용 버튼 accent primary**(목업 바이올렛)·그룹이름/색상/속성 섹션 사이 **구분선**으로 카드형 구조화·**담당자/부서=노드와 동일 SearchSelect 피커**(eligible 로드)·**충돌 옵션 아이콘 2×2 그리드**. 충돌 처리·개별 마법사·전 기능 보존. 이미지 `group-bulk-edit.png`(하단부) | lint/build✅ | ⏳ | 🔧 반영(검토대기) | — |
