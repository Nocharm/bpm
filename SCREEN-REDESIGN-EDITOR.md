...
| R9a | 기타 모달/오버레이 | **엣지 컨텍스트 팝업 4종**(`edge-branch`/`action`/`select`/`decision`) — 커서 위치 팝업이라 대표 모달(중앙 아이콘 원) 아닌 **`context-menu.tsx`(R6a) 팝업 토큰**으로 통일(사용자 결정): 패널 `py-1.5`+`text-caption`·항목 `h-8 px-3` 풀폭 hover·캡션 uppercase tracking-wide·cancel 앞 divider. 위치/Esc/ModalBackdrop/onClose 동작 보존. | lint/build✅ | ⏳ | ⏳ | feat/editor-redesign-r9 |
| R9b | 기타 모달/오버레이 | **확장 한계 확인 다이얼로그** — `page.tsx` 인라인 `capPrompt`(`p-4`·`text-white`·인라인 shadow로 대표 모달과 이탈)를 정규 `<ConfirmDialog>`로 교체(대표 모달 재사용). `expand-invariant-modal.tsx`는 죽은 코드(사용처 0)라 미변경·보고만. `prompt-dialog.tsx`는 이미 정합→무변경. | — | — | ⏳ | — |
