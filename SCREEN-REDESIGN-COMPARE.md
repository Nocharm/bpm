...
| C4 | 속성 인스펙터 | **우측 노드 속성(읽기전용) 신규** — `Properties`+`View only`(Lock) 배지. 포커스 노드 Title/Description(회색 박스)·Type·Color(스와치)·Assignee/Department/System/Duration(`InspectorRow` divide-y, 변경 필드 `~~before~~→after`). 미선택 빈 상태. `PanelRight` 토글(`inspectorOpen`). **+헤더/캔버스 에디터화**: 좌상 `PanelLeft`(Changes 접기)·우 `PanelRight`(인스펙터 접기) 에디터 동일 위치, **제목 드롭다운**(←Editor 링크 대체·클릭 시 Editor 메뉴), **Apply To-Be 제거**, **좌상 카운트→좌하 범례 통합**·**dot-grid 제거**·**Compare View 워터마크**(read-only 워터마크 재활용). i18n `compare.{watermark,properties,viewOnly,selectNode,inspectorToggle}`. | lint/build✅ | 라이브(map 13: 인스펙터·접기·드롭다운·워터마크·범례)✅ | ✅ | (this) |
