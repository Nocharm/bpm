...
- **흐름 방향 토글 (LR/TB)** — 헤더 버튼으로 좌→우(기본)/상→하 전환. 맵이 한 축으로 길 때 반대 축 사용. 아래 후처리·핸들·아크·삭제배치가 모두 방향 파라미터(`dir`)로 일반화됨.
  ...
  - `handleSides` 우회 변 — LR passthrough=bottom·back=top / TB passthrough=right·back=left. 나머지는 `orientedSides`: cross축으로 벗어난 상대(다른 라인, >40px)는 그 cross측 우선(위→top·아래→bottom / 좌→left·우→right) → 위쪽 곁가지·아래 삭제 노드가 다음 노드에 위/아래(가로) 또는 좌/우(세로)로 진입. 나머지는 방향벡터 fallback.
