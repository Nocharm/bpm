...
| H3 | 홈 | 버전 히스토리 — 타임라인 노드 + 버전 카드 + 이벤트 칩(2줄 표시, 박스 호버 시 전체 펼침, 행위자=이름). (검토6 이미지 + 검토7 호버펼침/이름) | ✅ | ⏳ 시현 | 🔧 호버 펼침·이름 반영 | (검토6·7) |
| H2b | 홈 | 멤버 카운트=아이콘+숫자 · 상세 버전:인원=2:1 · 사이 세로선(검토6: `border-hairline`로 진하게) | ✅ | ⏳ 시현 | 🔧 구분선 진하게 반영 | (검토5·6) |
| H2c | 홈 | 팀 행=평소 멤버수만→호버 시 상위소속+상위/하위 팀 하이라이트. 유저 행=평소 이름만→호버 시 (아이디)+직급·부서 아래로 펼침 (검토6·7) | ✅ | ⏳ 시현 | 🔧 유저 호버 펼침 반영 | (검토6·7) |
