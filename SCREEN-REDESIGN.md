...
| H3 | 홈 | 버전 히스토리 — 노드 + 카드 + 칩 2줄 → **클릭 토글** 상세 행(그리드 정렬: 단계필·이름·아이디·시간). 최신순·풀 타임스탬프·다중 펼침·모두접기·ease-in-out 300ms (검토6~10) | ✅ | ⏳ 5버전맵 | 🔧 다회 반영 | (검토6~10) |
| H2b | 홈 | 멤버 카운트=아이콘+숫자 · 상세 버전:인원=2:1 · 사이 세로선(검토6: `border-hairline`로 진하게) | ✅ | ⏳ 시현 | 🔧 구분선 진하게 반영 | (검토5·6) |
| H2c | 홈 | 팀=평소 멤버수만→호버 상위소속+상위/하위 하이라이트. **유저=평소 [이름/말단부서]→호버 [이름/(아이디)/타이틀/부서레벨 작은→큰] 다행 펼침** (확정 사양) | ✅ | ⏳ 시현 | 🔧 다행 펼침 반영 | (검토6·7재) |
...
| A1 | 관리자 | DB 테이블 **무한 스크롤** — 현재 페이지 버튼(`getDbTable` page/size). 하단 80px 도달 시 다음 10행 append(420ms 스피너), sticky 헤더, "N 표시" 카운트, 끝 "모든 행 불러옴". 프론트 구현(백엔드 무변경). `admin/table-viewer.tsx` | — | — | ⏳ | — |
