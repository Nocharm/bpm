...
| H2c | 홈 | 팀=평소 멤버수만→호버 상위소속+상위/하위 하이라이트. **유저=평소 [이름/말단부서]→호버 [이름/(아이디)/타이틀/부서레벨 작은→큰] 다행 펼침**(필=각 행·투명·롤배지 고정·클릭토글) (확정 사양) | ✅ | ✅ | ✅ OK (H2c완료) | (검토6~12) |
| H4 | 홈 | 카드 허용인원 클릭 팝오버 → **호버 툴팁**(120ms 디바운스·키보드) | ✅ | ✅ | 🔧 호버 전환 반영 | (검토14) |
| H5a | 홈 | 카드 3번째 줄 메타 **소유자(id)·수정시각**(프론트 가능분) | ✅ | ✅ | 🔧 메타 반영 | (검토14) |
| H5b | 홈 | 카드 메타(노드수·버전수·소유자**명** — **백엔드 read-only 집계**, 정의 확인 중) | — | — | ⏳ 백엔드 | — |
